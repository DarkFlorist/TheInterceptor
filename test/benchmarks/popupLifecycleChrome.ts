...
import {
	launchChromeSession,
	waitForAnyExtensionServiceWorker,
	waitForServiceWorker,
	createTargetPage,
	connectTarget,
	closeTarget,
	waitForPerformanceMark,
	waitForPerformanceMarks,
	waitForRegisteredContentScripts,
	readExtensionLargeStateValue,
...
const BENCHMARK_TRANSACTION_PAGE_STATE_GLOBAL =
	'__interceptorBenchmarkTxPageState' as const
...
function printStatsGroup(
	title: string,
	entries: readonly (readonly [string, Stats | undefined])[],
) {
	console.warn(title)
...
function requireRpcMethodStats(
	result: RpcMethodStats | undefined,
	label: string,
) {
...
function extractExtensionId(url: string) {
	...
	if (match?.[1] === undefined)
		throw new Error(`Could not determine extension id from ${url}`)
	return match[1]
}

function requiredTiming(
	launchEpochMs: number,
	...
	if (delta < -50)
		throw new Error(
			`Performance mark ${markName} was stale for the measured popup open`,
		)
	return delta
}

function timingFromSnapshot(
	launchEpochMs: number | undefined,
	snapshot: PerformanceMarkSnapshot | undefined,
	markName: string,
) {
...
function stackedSetupTiming(
	setups: readonly StackedSetupSample[],
	source: 'page' | 'worker' | 'confirm',
	markName: string,
) {
	return stats(
		setups.map((setup) => {
			try {
				const snapshot =
					source === 'page'
						? setup.pageSnapshot
						: source === 'worker'
							? setup.workerSnapshot
							: setup.confirmPopupSnapshot
...
function stackedSampleTiming(
	samples: readonly Sample[],
	selector: (sample: Sample, setup: StackedSetupSample) => number | undefined,
) {
	return stats(
		samples.map((sample) => {
...
function aggregateRpcRequests(
	rpcRequests: readonly BenchmarkRpcRequestSample[],
) {
	const totalRequests = rpcRequests.length
	const totalDurationMs = rpcRequests.reduce(
	...
	const methods = [...grouped.entries()].map(([method, durations]) => {
		...
		if (result === undefined)
			throw new Error(`Missing rpc method durations for ${method}`)
		return {
			method,
...
async function getTransactionPageState(
	connection: CdpConnection,
): Promise<TransactionStackPageState | undefined> {
	const state = await connection.evaluate<
		TransactionStackPageState | undefined
	>(
		`(() => globalThis[${JSON.stringify(BENCHMARK_TRANSACTION_PAGE_STATE_GLOBAL)}] ?? undefined)()`,
	)
	return state
}

async function waitForTransactionPagePhase(
	connection: CdpConnection,
	phase: TransactionStackPageState['phase'],
	timeoutMs: number,
) {
	await waitForCondition(
		async () => {
			...
			if (state?.phase === 'error')
				throw new Error(
					`Transaction page failed: ${state.error ?? 'unknown error'}`,
				)
...
async function triggerPopupOpen(workerConnection: CdpConnection) {
	await workerConnection.evaluate(
		`(async () => {
...
async function ensureAnchorTarget(context: BenchmarkContext) {
	...
	const existingAnchor = targets.find(
		(target) => target.id === context.anchorTargetId,
	)
	if (existingAnchor === undefined) {
		context.anchorTargetId = await createTargetPage(
			context.chrome.browserConnection,
			'about:blank',
		)
		return
	}
	await context.chrome.browserConnection.send('Target.activateTarget', {
		targetId: context.anchorTargetId,
	})
}
...
async function prepareBenchmarkContext(): Promise<BenchmarkContext> {
	...
	try {
		const initialWorkerTarget = await waitForAnyExtensionServiceWorker(
			chrome.browserDebugPort,
			30_000,
		)
		const extensionId = extractExtensionId(initialWorkerTarget.url)
		const anchorTargetId = await createTargetPage(
			chrome.browserConnection,
			'about:blank',
		)
		const workerConnection = await connectTarget(
			chrome.browserDebugPort,
			initialWorkerTarget.id,
		)
		try {
			await waitForPerformanceMarks(
				workerConnection,
				['interceptor:background:loaded'],
				30_000,
			)
...
async function openPopupAndCollect(
	context: BenchmarkContext,
	scenario: ScenarioName,
): Promise<Sample> {
	const workerTarget = await waitForServiceWorker(
		context.chrome.browserDebugPort,
		context.extensionId,
		30_000,
	)
	const workerConnection = await connectTarget(
	...
	try {
		...
		const popupTarget = await waitForPopupTarget(
			context.chrome.browserDebugPort,
			context.extensionId,
			30_000,
		)
		popupTargetId = popupTarget.id
		const popupConnection = await connectTarget(
			context.chrome.browserDebugPort,
			popupTargetId,
		)
		try {
			...
			try {
				await waitForPerformanceMark(
					popupConnection,
					POPUP_REFRESH_COMPLETE_MARK,
					15_000,
				)
				popupRefreshCompleteSnapshot =
					await getPerformanceSnapshot(popupConnection)
			} catch (error: unknown) {
				console.warn(
					'Popup benchmark did not observe the refresh-complete performance mark within the timeout.',
					error,
				)
			}
			...
			try {
				await waitForPerformanceMark(
					popupConnection,
					POPUP_REFRESH_RENDERED_MARK,
					15_000,
				)
				popupRefreshRenderedSnapshot =
					await getPerformanceSnapshot(popupConnection)
			} catch (error: unknown) {
				console.warn(
					'Popup benchmark did not observe the refresh-rendered performance mark within the timeout.',
					error,
				)
			}
			const popupRefreshCompleteMs =
				absoluteTime(
					popupRefreshCompleteSnapshot,
					POPUP_REFRESH_COMPLETE_MARK,
				) === undefined
			...
			const rpcRequests =
				(await workerConnection.evaluate<
					readonly BenchmarkRpcRequestSample[] | undefined
				>(rpcRequestCollectorExpression())) ?? []
			return {
				scenario,
				launchMode: 'real-chrome-extension-page',
				popupLaunchEpochMs: launchEpochMs,
				popupScriptStartMs: requiredTiming(
					launchEpochMs,
					popupSnapshot,
					'interceptor:popup:script-start',
				),
				popupShellPaintMs: requiredTiming(
					launchEpochMs,
					popupSnapshot,
					'interceptor:popup:shell-painted',
				),
				popupHomeFirstCommitMs: requiredTiming(
					launchEpochMs,
					popupSnapshot,
					'interceptor:popup:home-first-commit',
				),
				...
				workerRefreshStartMs: requiredTiming(
					launchEpochMs,
					workerSnapshot,
					'interceptor:background:refresh-home-start',
				),
				workerRefreshEndMs: requiredTiming(
					launchEpochMs,
					workerSnapshot,
					'interceptor:background:refresh-home-end',
				),
		...
		if (popupTargetId !== undefined)
			await closeTarget(context.chrome.browserConnection, popupTargetId).catch(
				() => undefined,
			)
		workerConnection.close()
	}
...
async function prepareStackedTransaction(
	context: BenchmarkContext,
	...
	await workerConnection.evaluate(rpcRequestCollectorResetExpression())
	const launchEpochMs = performance.timeOrigin + performance.now()
	const pageTargetId = await createTargetPage(
		context.chrome.browserConnection,
		transactionPageUrl,
	)
	const pageConnection = await connectTarget(
		context.chrome.browserDebugPort,
		pageTargetId,
	)
	...
	try {
		await waitForTransactionPagePhase(
			pageConnection,
			'requesting-access',
			30_000,
		)
		const accessTarget = await waitForTargetByUrl(
			context.chrome.browserDebugPort,
			`chrome-extension://${context.extensionId}/html3/interceptorAccessV3.html`,
			30_000,
		).catch(async (error) => {
			const targets = await waitForBrowserTargets(
				context.chrome.browserDebugPort,
			)
		...
		const accessConnection = await connectTarget(
			context.chrome.browserDebugPort,
			accessTarget.id,
		)
		try {
			await waitForButtonEnabled(
				accessConnection,
				'nav.popup-button-row button.is-primary:not(.is-danger)',
				30_000,
			)
			await clickButton(
				accessConnection,
		...
		const confirmTarget = await waitForTargetByUrl(
			context.chrome.browserDebugPort,
			`chrome-extension://${context.extensionId}/html3/confirmTransactionV3.html`,
			30_000,
		)
		confirmTargetId = confirmTarget.id
		await waitForTargetGone(
			context.chrome.browserDebugPort,
			(target) => target.id === accessTargetId,
			30_000,
			'interceptor access popup to close',
		)
		const confirmConnection = await connectTarget(
			context.chrome.browserDebugPort,
			confirmTarget.id,
		)
		try {
			await waitForButtonEnabled(
				confirmConnection,
				'button.button.is-primary.button-overflow.dialog-button-right',
				120_000,
			)
			const confirmPopupSnapshot =
				await getPerformanceSnapshot(confirmConnection)
			await clickButton(
				confirmConnection,
				'button.button.is-primary.button-overflow.dialog-button-right',
			)
			...
			const workerSnapshot = await getPerformanceSnapshot(workerConnection)
			const setupRpcRequests =
				(await workerConnection.evaluate<
					readonly BenchmarkRpcRequestSample[] | undefined
				>(rpcRequestCollectorExpression())) ?? []
			const stackLength =
				(
					await readExtensionLargeStateValue<{
						readonly operations?: readonly unknown[]
					}>(workerConnection, 'interceptorTransactionStack')
				)?.operations?.length ?? 0
		...
		await waitForTargetGone(
			context.chrome.browserDebugPort,
			(target) => target.id === confirmTargetId,
			30_000,
			'confirm transaction popup to close',
		).catch(() => undefined)
		pageConnection.close()
		workerConnection.close()
		if (confirmTargetId !== undefined)
			await closeTarget(
				context.chrome.browserConnection,
				confirmTargetId,
			).catch(() => undefined)
		if (accessTargetId !== undefined)
			await closeTarget(context.chrome.browserConnection, accessTargetId).catch(
				() => undefined,
			)
	}
}

async function runStackedScenario(
	context: BenchmarkContext,
	transactionPageUrl: string,
) {
	const { stackLength, setupRpcRequests, stackedSetup } =
		await prepareStackedTransaction(context, transactionPageUrl)
	console.warn(
		`  transaction stack prepared with ${stackLength} operation${stackLength === 1 ? '' : 's'}`,
	)
...
async function runScenarioOnce(
	...
	try {
		...
		if (transactionPageUrl === undefined)
			throw new Error('Missing transaction page URL for stacked scenario')
...
async function runScenarioManyTimes(
	scenario: ScenarioName,
	iterations: number,
	transactionPageUrl?: string,
): Promise<Sample[]> {
...
async function main() {
	...
	if (!Number.isFinite(iterations) || iterations <= 0)
		throw new Error(
			`Invalid BENCH_ITERATIONS value: ${process.env.BENCH_ITERATIONS ?? '1'}`,
		)
	const transactionPageServer =
		scenarioFilter === 'all' || scenarioFilter === 'stacked'
	...
	try {
		...
		for (const scenario of scenarios) {
			const samples = await runScenarioManyTimes(
				scenario,
				iterations,
				transactionPageServer?.baseUrl,
			)
			runs.push({ scenario, samples })
		}

		for (const run of runs) {
			const popupScriptStart = stats(
				run.samples.map((sample) => sample.popupScriptStartMs),
			)
			const popupShellPaint = stats(
				run.samples.map((sample) => sample.popupShellPaintMs),
			)
			const popupHomeFirstCommit = stats(
				run.samples.map((sample) => sample.popupHomeFirstCommitMs),
			)
			const popupRefreshComplete = stats(
				run.samples.map((sample) => sample.popupRefreshCompleteMs),
			)
			const popupRefreshRendered = stats(
				run.samples.map((sample) => sample.popupRefreshRenderedMs),
			)
			const workerRefreshStart = stats(
				run.samples.map((sample) => sample.workerRefreshStartMs),
			)
			const workerRefreshEnd = stats(
				run.samples.map((sample) => sample.workerRefreshEndMs),
			)
			const rpcRequests = run.samples.flatMap((sample) => sample.rpcRequests)
			const setupRpcRequests = run.samples.flatMap(
				(sample) => sample.setupRpcRequests,
			)
			const stackedSetups = run.samples.flatMap((sample) =>
				sample.stackedSetup === undefined ? [] : [sample.stackedSetup],
			)
			...
			const summary: ScenarioStats = {
				...
				popupHomeFirstCommit: requireStats(
					popupHomeFirstCommit,
					'popup Home first commit',
				),
				popupRefreshComplete,
				popupRefreshRendered,
				workerRefreshStart: requireStats(
					workerRefreshStart,
					'worker refresh start',
				),
			...
			console.warn(
				`${summary.name} (${summary.iterations} run${summary.iterations === 1 ? '' : 's'}, ${summary.launchMode}):`,
			)
			if (stackedSetups.length > 0) {
				console.warn(
					'  note: the next three groups start when the transaction test page loads, before the main popup is opened.',
				)
				printStatsGroup(
					'  send transaction path / page load -> page milestones:',
					[
						[
							'provider ready',
							stackedSetupTiming(
								stackedSetups,
								'page',
								'interceptor:benchmark:tx:provider-ready',
							),
						],
						[
							...
							stackedSetupTiming(
								stackedSetups,
								'page',
								'interceptor:benchmark:tx:requesting-access',
							),
						],
						[
							'access granted',
							stackedSetupTiming(
								stackedSetups,
								'page',
								'interceptor:benchmark:tx:access-granted',
							),
						],
						[
							'transaction request started',
							stackedSetupTiming(
								stackedSetups,
								'page',
								'interceptor:benchmark:tx:requesting-transaction',
							),
						],
						[
							...
							stackedSetupTiming(
								stackedSetups,
								'page',
								'interceptor:benchmark:tx:submitted',
							),
						],
						[
							'balance request started',
							stackedSetupTiming(
								stackedSetups,
								'page',
								'interceptor:benchmark:tx:requesting-balance',
							),
						],
						[
							'balance fetched',
							stackedSetupTiming(
								stackedSetups,
								'page',
								'interceptor:benchmark:tx:balance-fetched',
							),
						],
					],
				)
				printStatsGroup(
					'  send transaction path / page load -> confirm popup milestones:',
					[
						[
							'confirm popup simulation started',
							stackedSetupTiming(
								stackedSetups,
								'confirm',
						...
						[
							'confirm popup simulation ready',
							stackedSetupTiming(
								stackedSetups,
								'confirm',
				...
				printStatsGroup(
					'  send transaction path / page load -> background milestones:',
					[
						[
							'background received transaction request',
							stackedSetupTiming(
								stackedSetups,
								'worker',
				...
				printStatsGroup(
					'  send transaction path / end-to-end page load -> main popup milestones:',
					[
						[
							'main popup open requested',
							stackedSampleTiming(
								run.samples,
								(sample, setup) =>
									sample.popupLaunchEpochMs - setup.launchEpochMs,
							),
						],
						[
							'main popup script start',
							stackedSampleTiming(
								run.samples,
								(sample, setup) =>
									sample.popupLaunchEpochMs -
									setup.launchEpochMs +
									sample.popupScriptStartMs,
							),
						],
						[
							'main popup shell paint',
							stackedSampleTiming(
								run.samples,
								(sample, setup) =>
									sample.popupLaunchEpochMs -
									setup.launchEpochMs +
									sample.popupShellPaintMs,
							),
						],
						[
							'main popup first commit',
							stackedSampleTiming(
								run.samples,
								(sample, setup) =>
									sample.popupLaunchEpochMs -
									setup.launchEpochMs +
									sample.popupHomeFirstCommitMs,
							),
						],
						[
							'main popup rendered with refreshed state',
							stackedSampleTiming(run.samples, (sample, setup) => {
								const rendered =
									sample.popupRefreshRenderedMs ?? sample.popupHomeFirstCommitMs
								return (
									sample.popupLaunchEpochMs - setup.launchEpochMs + rendered
								)
							}),
						],
					],
			...
			if (run.scenario === 'stacked') {
				console.warn(
					'  note: the next two groups start later, at main popup open, so they exclude the send-transaction setup path above.',
				)
			}
			printStatsGroup(
				'  main popup only / open path (starts at main popup open):',
				[
			...
			printStatsGroup(
				'  main popup only / refresh path (same main popup open):',
				[
			...
			if (setupRpcRequests.length > 0) {
				console.warn(
					`  rpc / setup (${setupRpcSummary.totalRequests} request${setupRpcSummary.totalRequests === 1 ? '' : 's'}, ${roundToTwoDecimals(setupRpcSummary.totalDurationMs)} ms cumulative):`,
				)
				if (setupRpcMethodStats.length === 0) {
					...
					for (const methodStats of setupRpcMethodStats) {
						console.warn(
							formatRpcMethodStats(
								`    ${methodStats.method}`,
								requireRpcMethodStats(methodStats, methodStats.method),
							),
						)
					}
				}
			...
			console.warn(
				`  rpc / popup (${summary.rpcTotalRequests} request${summary.rpcTotalRequests === 1 ? '' : 's'}, ${roundToTwoDecimals(summary.rpcTotalDurationMs)} ms cumulative):`,
			)
			...
			if (summary.popupRefreshComplete === undefined) {
				console.warn(
					'  popup refresh state applied was not observed within the benchmark wait window.',
				)
			}
			console.warn('')
		}
	} finally {
		if (transactionPageServer !== undefined)
			await transactionPageServer.close().catch(() => undefined)
	}
}
