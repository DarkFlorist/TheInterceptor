...
const { UpdateConfirmTransactionDialogPendingTransactions } = await import(
	'../../app/ts/types/interceptor-messages.js'
)
...
describe('ConfirmTransaction', () => {
	test('updates the simulation age when a refreshed pending transaction arrives', async () => {
		...
		const olderPendingTransaction = makePendingTransaction(
			new Date('2024-01-01T00:00:05.000Z'),
		)
		const newerPendingTransaction = makePendingTransaction(
			new Date('2024-01-01T00:00:09.000Z'),
		)
	...
	test(
		'updates the simulation age when the real refresh flow runs',
		{ timeout: 15_000 },
		async () => {
			const dom = installDomMock()
			const clock = installDateMock('2024-01-01T00:00:10.000Z')
			...
			const { refreshPopupConfirmTransactionSimulation } = await import(
				'../../app/ts/background/popupMessageHandlers.js'
			)
			const { browserStorageLocalSet2 } = await import(
				...
				async jsonRpcRequest(rpcRequest: {
					...
					switch (rpcRequest.method) {
						case 'eth_getBlockByNumber':
							return serialize(
								(await import('../../app/ts/types/wire-types.js'))
									.EthereumBlockHeader,
								fakeBlock,
							)
						...
						case 'eth_simulateV1': {
							const multicallAbi = (
								await import('../../app/ts/utils/constants.js')
							).Multicall3ABI
							const balanceResult = encodeFunctionReturn(
								multicallAbi,
								'getEthBalance',
								[0n],
							)
							const aggregate3Result = encodeFunctionReturn(
								multicallAbi,
								'aggregate3',
								[[{ success: true, returnData: balanceResult }]],
							)
							const blockStateCalls = Array.isArray(
								rpcRequest.params?.[0]?.blockStateCalls,
							)
								? rpcRequest.params[0].blockStateCalls
								: [{}]
							return serialize(
								(await import('../../app/ts/types/ethSimulate-types.js'))
									.EthSimulateV1Result,
								blockStateCalls.map((blockStateCall) => ({
									...
									calls: Array.from(
										{
											length: Array.isArray(blockStateCall.calls)
			...
			await act(async () => {
				await refreshPopupConfirmTransactionSimulation(
					simulator.ethereum,
					simulator.tokenPriceService as never,
				)
			})

			const [refreshedPendingTransaction] =
				await getPendingTransactionsAndMessages()
			if (
				refreshedPendingTransaction === undefined ||
				refreshedPendingTransaction.type !== 'Transaction'
			)
			...
			assert.equal(
				dom.document.body.textContent?.includes('Simulated 0s ago'),
				true,
			)
