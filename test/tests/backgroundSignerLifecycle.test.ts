...
describe('background eth_accounts', () => {
	...
	test('suppresses unscoped connect events for requester during signer refresh with page-level access', async () => {
		...
		assert.deepEqual(messages.filter((message) => message.method !== 'eth_accounts_reply' && message.method !== 'safe_apps_compatibility').map((message) => message.method), ['request_signer_to_eth_requestAccounts', 'accountsChanged', 'eth_accounts'])
	...
	for (const manifestVersion of [2, 3] as const) {
		test(`replays only accountsChanged before wallet_requestPermissions resolves for manifest v${ manifestVersion }`, async () => {
			...
			assert.deepEqual(messages.filter((message) => message.method !== 'eth_accounts_reply').map((message) => message.method), ['request_signer_to_eth_requestAccounts', 'safe_apps_compatibility', 'accountsChanged', 'wallet_requestPermissions'])
