...
describe('content script injection strategy', () => {
	test('keeps manifest v3 exclusions scoped to the approved scheme and exact host', async () => {
		...
		assert.equal(websiteOriginToMatchPattern('https://example.test:8443'), 'https://example.test:8443/*')
		...
		assert.equal(websiteOriginToMatchPattern('example.test:443'), '*://example.test:443/*')
