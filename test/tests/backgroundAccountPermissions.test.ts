...
describe('background eth_accounts', () => {
	...
	test('replays account state after already-approved eth_requestAccounts with cached active signer address', async () => {
		...
		assert.deepEqual(messages.filter((message) => message.method !== 'safe_apps_compatibility').map((message) => message.method), ['accountsChanged', 'eth_accounts'])
	})
	...
	test('does not advertise an approved EOA as a Safe on top-frame reload or child-frame approval', async () => {
		...
		assert.deepEqual(childMessages.filter((message) => message.method === 'safe_apps_compatibility').map((message) => message.result?.enabled), [false])
		await safeAppsCompatibilityCoordinator.refreshApprovedPorts(childConnections)
	...
	for (const { accountKind, discoveredAccount, expectedCompatibility } of [
		{ accountKind: 'owner', discoveredAccount: 0x7171717171717171717171717171717171717171n, expectedCompatibility: true },
		{ accountKind: 'non-owner', discoveredAccount: 0x7272727272727272727272727272727272727272n, expectedCompatibility: false },
	] as const) {
		test(`waits for fresh ${ accountKind } account discovery before publishing Safe compatibility`, async () => {
			...
			assert.deepEqual(messages.filter((message) => message.method === 'safe_apps_compatibility'), [])
			if (replyToDiscovery === undefined) throw new Error('Safe Apps signer-account discovery was not requested')
			await replyToDiscovery()
			await waitForPortMessageCount(messages, 'safe_apps_compatibility', 1)
			assert.deepEqual(messages.filter((message) => message.method === 'safe_apps_compatibility').map((message) => message.result?.enabled), [expectedCompatibility])
		})
	}
	...
	test('does not let an older async Safe compatibility enable overwrite a newer disable', async () => {
		...
		Object.defineProperty(browser.storage.local, 'get', {
			...
			value: async (keys?: string | string[] | Record<string, unknown> | null) => {
				...
				if (delayNextSafeAppsModeRead && Array.isArray(keys) && keys.length === 1 && keys[0] === 'safeAppsCompatibilityMode') {
		...
		const olderEnablePublication = safeAppsCompatibilityCoordinator.refreshApprovedPorts(websiteTabConnections)
	...
	test('delivers accountsChanged before an approved active-address switch resolves', async () => {
		...
		assert.deepEqual(messages.filter((message) => message.method !== 'safe_apps_compatibility').map((message) => message.method), ['accountsChanged'])
	...
	test('clears dapp accounts and finishes opening access approval when the active address is unapproved', async () => {
		...
		assert.deepEqual(messages.map((message) => message.method), ['safe_apps_compatibility', 'accountsChanged', 'disconnect'])
		assert.equal((messages[0]?.result as { enabled?: boolean } | undefined)?.enabled, false)
	...
	test('wallet_revokePermissions clears website account access and keeps the website entry', async () => {
		...
		assert.equal((messages.find((message) => message.method === 'safe_apps_compatibility')?.result as { enabled?: boolean } | undefined)?.enabled, false)
