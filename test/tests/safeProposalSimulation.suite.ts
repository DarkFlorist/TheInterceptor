...
test('atomic Safe batch preparation and simulation preserve one nonce and delegate execution as the Safe', async () => {
	...
	const params = SendTransactionParams.parse({ method: 'eth_sendTransaction', params: [{ from: addressString(activeAddress), to: addressString(SAFE_MULTI_SEND_CALL_ONLY), value: '0x0', data: dataStringWith0xStart(input), gas: '0x989680', safeOperation: '0x1' }] })
	...
	const preparation = await (await import('../../app/ts/background/safeTransactionConfirmation.js')).prepareSafeTransactionConfirmation(simulator.ethereum, params, false, activeAddress, recipientAddress)
