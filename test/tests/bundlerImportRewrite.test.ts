...
import {
	findMissingRuntimeImportsInRuntimeFiles,
	isBrowserIncompatibleRuntimeModule,
	replaceImport,
} from '../../build/bundler.mts'
...
describe('bundler import rewriting', () => {
	test('rewrites minified vendored esm imports without whitespace separators', () => {
		...
		const source =
			'import{Component as t,options as i}from"preact";import{useMemo as e}from"preact/hooks";import{Signal as r}from"@preact/signals-core";export{Signal}from"@preact/signals-core";'
	...
	test('rewrites bare side-effect imports in generated app modules', () => {
		...
		assert.equal(
			rewritten,
			'import"../../vendor/webextension-polyfill/dist/browser-polyfill.js";',
		)
	})
	...
	test('rewrites nested vendored package imports using the vendored dependency tree', () => {
		...
		assert.equal(
			rewritten,
			"import { keccak_256 as noble_keccak256 } from '../../__dependencies__/@noble/hashes/esm/sha3.js';",
		)
	})
	...
	test('rewrites vendored relative node_modules imports away from node_modules paths', () => {
		...
		assert.equal(
			rewritten,
			"import { keccak_256 } from '../../../__dependencies__/@noble/hashes/esm/sha3.js';",
		)
	})
	...
	test('ignores stale generated modules that are not reachable from runtime entrypoints', () => {
		...
		const staleFilePath = path.join(
			...
			'app',
			'js',
			'stale-build-output.js',
		)
		fs.mkdirSync(appJsDirectoryPath, { recursive: true })
		fs.writeFileSync(
			staleFilePath,
			"import '../vendor/does-not-exist/index.js';\n",
		)
