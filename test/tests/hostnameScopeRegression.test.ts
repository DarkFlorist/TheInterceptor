...
function installBrowserMock(storageState: StorageState, manifestVersion = 3) {
	const registeredContentScripts: Array<{ excludeMatches?: readonly string[] }> = []
...
describe('hostname scope regressions', () => {
	test('MV3 content script exclusions include disabled hostnames stored with ports', async () => {
		const storageState: StorageState = {
			websiteAccess: [
				{
					website: { websiteOrigin: 'localhost:3000', icon: undefined, title: 'Local app' },
					addressAccess: [],
	...
	test('removing one origin clears hostname-scoped website access entries', async () => {
		const storageState: StorageState = {
			websiteAccess: [
				{
					website: { websiteOrigin: 'localhost:3000', icon: undefined, title: 'App A' },
					addressAccess: [],
					interceptorDisabled: true,
					declarativeNetRequestBlockMode: 'block-all',
				},
				{
					website: { websiteOrigin: 'localhost:5173', icon: undefined, title: 'App B' },
					addressAccess: [],
					access: true,
				},
				{
					website: { websiteOrigin: 'otherhost.test:3000', icon: undefined, title: 'Other Host' },
					addressAccess: [],
					access: true,
				},
			],
