...
describe('narrow text layout CSS', () => {
	test('keeps checkbox labels and dropdown text constrained inside narrow containers', async () => {
		...
		assert.match(nameField, /flex\s*:\s*1\.55 1 10\.5rem\s*;/)
		...
		assert.match(css, /\.address-editor-name-field \+ \.address-editor-field\s*\{[^}]*flex\s*:\s*0\.75 1 5\.5rem\s*;/)
		assert.match(css, /\.address-editor-name-field \+ \.address-editor-field \+ \.address-editor-field\s*\{[^}]*flex\s*:\s*1\.1 1 7\.5rem\s*;/)
