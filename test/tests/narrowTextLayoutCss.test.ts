...
describe('narrow text layout CSS', () => {
	test('keeps checkbox labels and dropdown text constrained inside narrow containers', async () => {
		...
		assert.match(addressEditorFields, /display\s*:\s*flex\s*;/)
		assert.match(addressEditorFields, /flex-wrap\s*:\s*wrap\s*;/)
		...
		assert.match(nameField, /flex\s*:\s*1 1 11rem\s*;/)
		assert.match(nameField, /grid-template-columns\s*:\s*max-content minmax\(0,\s*1fr\)\s*;/)
		assert.match(nameField, /column-gap\s*:\s*0\.65rem\s*;/)
		const identitySelectors = expectRule(css, '.address-editor-identity-selectors')
		assert.match(identitySelectors, /display\s*:\s*grid\s*;/)
		assert.match(identitySelectors, /flex\s*:\s*1 1 14rem\s*;/)
		const inlineIdentityField = expectRule(css, '.address-editor-identity-selectors > .address-editor-field')
		assert.match(inlineIdentityField, /grid-template-columns\s*:\s*max-content minmax\(0,\s*1fr\)\s*;/)
		assert.match(inlineIdentityField, /align-items\s*:\s*center\s*;/)
		...
		assert.match(addressEditorSectionHeading, /flex-wrap\s*:\s*wrap\s*;/)
