...
describe('narrow text layout CSS', () => {
	test('keeps checkbox labels and dropdown text constrained inside narrow containers', async () => {
		...
		const safeSignerHomeAddress = expectRule(css, '.safe-signer-home-option > code')
		assert.match(safeSignerHomeAddress, /overflow-wrap\s*:\s*anywhere\s*;/)
		assert.match(safeSignerHomeAddress, /word-break\s*:\s*break-all\s*;/)
		...
		assert.match(identity, /display\s*:\s*flex\s*;/)
		assert.match(identity, /flex-wrap\s*:\s*wrap\s*;/)
		...
		assert.match(identitySelectors, /min-width\s*:\s*0\s*;/)
