...
describe('narrow text layout CSS', () => {
	test('keeps checkbox labels and dropdown text constrained inside narrow containers', async () => {
		...
		const safeSignerEditorAddress = expectRule(css, '.safe-signer-editor-address')
		assert.match(safeSignerEditorAddress, /grid-column\s*:\s*2\s*;/)
		assert.match(safeSignerEditorAddress, /color\s*:\s*var\(--text-color\)\s*;/)
		assert.match(safeSignerEditorAddress, /min-width\s*:\s*0\s*;/)
		assert.match(safeSignerEditorAddress, /width\s*:\s*100%\s*;/)
		assert.match(safeSignerEditorAddress, /overflow-wrap\s*:\s*anywhere\s*;/)
		assert.match(safeSignerEditorAddress, /white-space\s*:\s*normal\s*;/)
