...
describe('narrow text layout CSS', () => {
	test('keeps checkbox labels and dropdown text constrained inside narrow containers', async () => {
		...
		assert.match(css, /\.safe-signer-editor-title,\s*\.safe-signer-editor-row,\s*\.safe-signer-editor-add,\s*\.safe-signer-editor-empty,\s*\.safe-signer-editor-actions\s*\{[\s\S]*?grid-column\s*:\s*1 \/ -1\s*;/)
		...
		assert.match(safeSignerEditorAddress, /text-overflow\s*:\s*ellipsis\s*;/)
