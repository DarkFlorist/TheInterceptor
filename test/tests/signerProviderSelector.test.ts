...
test('signer provider selector renders a contained logo-led accessible dropdown', async () => {
	...
	assert.match(homeSource, /tabIndex = \{ isSelected \|\| \(selectedProvider === undefined && index === 0\) \? 0 : -1 \}/)
	assert.match(homeSource, /event\.key !== 'ArrowDown' && event\.key !== 'ArrowUp'/)
	assert.match(homeSource, /\['ArrowDown', 'ArrowUp', 'Home', 'End'\]/)
	assert.match(homeSource, /onFocusOut = \{ onDropdownFocusOut \}/)
