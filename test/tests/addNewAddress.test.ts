...
describe('add new address save flow', () => {
	...
	test('preserves known Safe owners on lookup failure and clears them when the target changes', () => {
		...
		assert.match(safeContractStateSource, /new EthereumClientService\([\s\S]*?new EthereumJSONRpcRequestHandler\(rpcEntry\.httpsRpc/)
		assert.doesNotMatch(safeContractStateSource, /createSimulationServices/)
