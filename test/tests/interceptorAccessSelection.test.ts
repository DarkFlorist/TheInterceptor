...
function createPendingAccessRequest(
	accessRequestId: string,
	signerAccount: bigint,
...
describe('InterceptorAccess modal request selection', () => {
	test('uses the accessRequestId-selected request instead of always using the first request', () => {
		const firstRequest = createPendingAccessRequest(
			'first-request',
			0x1000000000000000000000000000000000000001n,
		)
		const secondRequest = createPendingAccessRequest(
			'second-request',
			0x2000000000000000000000000000000000000002n,
		)

		const selectedRequest = getSelectedPendingAccessRequest(
			[firstRequest, secondRequest],
			secondRequest.accessRequestId,
		)
