...
describe('EIP712', () => {
	...
	test('can not verify fixedArrayOverload message', () => {
		...
		assert.deepEqual(verification, {
			valid: false,
			reason: 'Message was invalid: contents: ["Hello, Bob!","hellow cow!"] is invalid: expected array of length 1, got 2',
		})
	})
	...
	test('can not verify typeMissmatch message', () => {
		...
		assert.deepEqual(verification, {
			valid: false,
			reason: 'Message was invalid: from: {"name":"Cow","wallet":5} is invalid 5 is not address',
		})
	})
	test('can not verify smallOverFlow message', () => {
		...
		assert.deepEqual(verification, {
			valid: false,
			reason: 'Message was invalid: overflow: "0xCcCCccccCCCCcCCCCCCcCcCccCcCCCcCcccccccC" is invalid: "0xCcCCccccCCCCcCCCCCCcCcCccCcCCCcCcccccccC" is out of bounds of uint8',
		})
	})
