...
import {
	...
	validateEIP712Types,
} from '../../app/ts/utils/eip712Parsing.js'
...
import {
	...
	verifyEip712Message,
} from '../../app/ts/utils/eip712.js'
import {
	canVerifyStructArray,
	chainIdIsNotDefined,
	chainIdOverFlow,
	d2Array,
	d2ArrayFixed,
	...
	hasArray,
	hasFixedArray,
	hasTooLongFixedArray,
	missingChainId,
	openSeaWithTotalOriginalConsiderationItems,
	permit2Message,
	permit2MessageHexChainId,
	permit2MessageNumberChainId,
	primarytypeIsWrong,
	safeTx,
	smallOverFlow,
	structNotDefined,
...
describe('EIP712', () => {
	test('can convert oversized number to bigint', () => {
		assert.deepEqual(
			jsonParserWithNumbersAsStringsConverter(
				'1234567890123456789012345678901234567890',
			),
			'1234567890123456789012345678901234567890',
		)
	})
	test('can parse EIP712 message', () => {
		EIP712Message.parse(permit2Message)
	})
	test('can parse Permit2 message', () => {
		Permit2.parse(JSON.parse(permit2Message))
	})
	test('can parse safeTx message', () => {
		SafeTx.parse(JSON.parse(safeTx))
	})
	test('can verify a permit 2 message EIP712 message', () => {
		...
		getMessageAndDomainHash({
			...
			params: [0x1n, JSON.parse(permit2Message)],
		})
	})
	test('can verify a safe EIP712 message', () => {
		const verification = verifyEip712Message(JSON.parse(safeTx))
		if (verification.valid === false) throw new Error(verification.reason)
		getMessageAndDomainHash({
			method: 'eth_signTypedData_v4',
			params: [0x1n, JSON.parse(safeTx)],
		})
	})
	test('can verify an opensea message', () => {
		const verification = verifyEip712Message(
			JSON.parse(openSeaWithTotalOriginalConsiderationItems),
		)
		if (verification.valid === false) throw new Error(verification.reason)
		getMessageAndDomainHash({
			method: 'eth_signTypedData_v4',
			params: [0x1n, JSON.parse(openSeaWithTotalOriginalConsiderationItems)],
		})
	})
	test('can verify permit2MessageHexChainId message', () => {
		const verification = verifyEip712Message(
			JSON.parse(permit2MessageHexChainId),
		)
		if (verification.valid === false) throw new Error(verification.reason)
		getMessageAndDomainHash({
			method: 'eth_signTypedData_v4',
			params: [0x1n, JSON.parse(permit2MessageHexChainId)],
		})
	})
	test('can verify eip712Example message', () => {
		...
		getMessageAndDomainHash({
			method: 'eth_signTypedData_v4',
			params: [0x1n, JSON.parse(eip712Example)],
		})
	})
	test('can verify permit2MessageNumberChainId message', () => {
		const verification = verifyEip712Message(
			JSON.parse(permit2MessageNumberChainId),
		)
		if (verification.valid === false) throw new Error(verification.reason)
	})
	test('can not verify chainIdIsNotDefined message', () => {
		const verification = verifyEip712Message(JSON.parse(chainIdIsNotDefined))
		assert.deepEqual(verification, {
			valid: false,
			reason: 'EIP712Domain was invalid: Failed to find type for chainId',
		})
	})
	test('can not verify primarytypeIsWrong message', () => {
		const verification = verifyEip712Message(JSON.parse(primarytypeIsWrong))
		assert.equal(verification.valid, false)
	})
	test('can not verify eip721DomainMissing message', () => {
		const verification = verifyEip712Message(JSON.parse(eip721DomainMissing))
		assert.equal(verification.valid, false)
	})
	test('can not verify structNotDefined message', () => {
		const verification = verifyEip712Message(JSON.parse(structNotDefined))
		assert.deepEqual(verification, {
			valid: false,
			reason: 'Message was invalid: unknown type: Person2',
		})
	})
	test('can verify extraType message', () => {
	...
	test('can not verify fixedArrayOverload message', () => {
		...
		assert.deepEqual(verification, {
			valid: false,
			reason:
				'Message was invalid: contents: ["Hello, Bob!","hellow cow!"] is invalid: expected array of length 1, got 2',
		})
	})
	test('can not verify duplicateType message', () => {
		const verification = verifyEip712Message(JSON.parse(duplicateType))
		assert.deepEqual(verification, {
			valid: false,
			reason: 'Message was invalid: not unique type names',
		})
	})
	test('can not verify unknownExtraField message', () => {
		const verification = verifyEip712Message(JSON.parse(unknownExtraField))
		assert.deepEqual(verification, {
			valid: false,
			reason: 'EIP712 message should only have 4 fields',
		})
	})
	test('can not verify unknownExtraField2 message', () => {
		const verification = verifyEip712Message(JSON.parse(unknownExtraField2))
		assert.deepEqual(verification, {
			valid: false,
			reason: 'Message was invalid: Failed to find type for test',
		})
	})
	test('can not verify typeMissmatch message', () => {
		const verification = verifyEip712Message(JSON.parse(typeMissmatch))
		assert.deepEqual(verification, {
			valid: false,
			reason:
				'Message was invalid: from: {"name":"Cow","wallet":5} is invalid 5 is not address',
		})
	})
	test('can not verify smallOverFlow message', () => {
		const verification = verifyEip712Message(JSON.parse(smallOverFlow))
		assert.deepEqual(verification, {
			valid: false,
			reason:
				'Message was invalid: overflow: "0xCcCCccccCCCCcCCCCCCcCcCccCcCCCcCcccccccC" is invalid: "0xCcCCccccCCCCcCCCCCCcCcCccCcCCCcCcccccccC" is out of bounds of uint8',
		})
	})
	test('can not verify message with tuples (not supported by eip712)', () => {
		const verification = verifyEip712Message(JSON.parse(tupleSupport))
		assert.deepEqual(verification, {
			valid: false,
			reason: 'Message was invalid: unknown type: tuple(uint256, uint256)',
		})
	})
	test('can verify d2Array message', () => {
	...
	test('can extract safeTx message', async () => {
		const parsed = EIP712Message.parse(safeTx)
		const enrichedMessage = stringifyJSONWithBigInts(
			await extractEIP712Message(ethereum, undefined, parsed, false),
		)
	...
	test('can extract permit2Message message', async () => {
		const parsed = EIP712Message.parse(permit2Message)
		const enrichedMessage = stringifyJSONWithBigInts(
			await extractEIP712Message(ethereum, undefined, parsed, false),
		)
	...
	test('can extract permit2Message message with hex chain id', async () => {
	...
	test('can extract permit2Message message with number chain id', async () => {
