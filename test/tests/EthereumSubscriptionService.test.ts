...
async function loadModules() {
	return {
		...(await import(
			'../../app/ts/simulation/services/EthereumSubscriptionService.js'
		)),
...
class MockEthereumJSONRpcRequestHandler {
	...
	public readonly jsonRpcRequest = async (
		...
		switch (rpcRequest.method) {
			...
			case 'eth_simulateV1': {
				const lastCallInput =
					rpcRequest.params[0]?.blockStateCalls.at(-1)?.calls[0]?.input
...
describe('EthereumSubscriptionService', () => {
	test('removeEthereumSubscription only removes the matching socket subscription', async () => {
		...
		assert.deepEqual(
			(await getEthereumSubscriptionsAndFilters()).map(
				(entry) => entry.subscriptionOrFilterId,
			),
	...
	test('getEthFilterChanges applies the real filter payload and preserves the stored subscription list', async () => {
		...
		const matchingLog =
			simulationState.simulatedBlocks[0]?.simulatedTransactions[0]
				?.ethSimulateV1CallResult.status === 'success'
				? simulationState.simulatedBlocks[0]?.simulatedTransactions[0]
						?.ethSimulateV1CallResult.logs[0]
				: undefined
		if (matchingLog === undefined)
			throw new Error('matching simulated log missing')
		...
		await updateEthereumSubscriptionsAndFilters(() => [
			{
				...
				params: {
					...
					params: [
						{ address: matchingLog.address, topics: [matchingLog.topics[0]] },
					],
				},
		...
		const firstChanges = await getEthFilterChanges(
			'filter-1',
			ethereum,
			undefined,
		...
		const secondChanges = await getEthFilterChanges(
			'filter-1',
			ethereum,
			undefined,
			toResolvedExecutionSimulationState(simulationState),
		...
		if (updated[0]?.type !== 'eth_newFilter')
			throw new Error('Filter was not preserved')
	...
	test('eth_getFilterChanges only returns newly simulated logs once', async () => {
		...
		const ethereum = createEthereum()
		const socket = { tabId: 1, connectionName: 1n } as const
		const filterId = await createNewFilter(
		...
		const simulationState = await createExecutionSimulationState(
			ethereum,
			undefined,
			createSimulationInput(21_000n, 0n, 1n),
		)
		if (simulationState.success === false)
			throw new Error('simulation unexpectedly failed')
		...
		const storedFilter = storedFilters.find(
			(filter) =>
				filter.type === 'eth_newFilter' &&
				filter.subscriptionOrFilterId === filterId,
		)
		if (storedFilter === undefined || storedFilter.type !== 'eth_newFilter')
			throw new Error('stored filter missing')
	...
	test('newHeads emits each simulated execution block after a gas split', async () => {
		installBrowserMock()
		const { createEthereumSubscription, sendSubscriptionMessagesForNewBlock } =
			await loadModules()
		const ethereum = createEthereum()
		const socket = { tabId: 1, connectionName: 1n } as const
		...
		await createEthereumSubscription(
		...
		const simulationState = await createExecutionSimulationState(
			ethereum,
			undefined,
			splitSimulationInput,
		)
		if (simulationState.success === false)
			throw new Error('simulation unexpectedly failed')

		await sendSubscriptionMessagesForNewBlock(
			blockNumber,
			ethereum,
			true,
			websiteTabConnections,
			async () => toResolvedExecutionSimulationState(simulationState),
		)
		...
		const emittedBlockNumbers = postedMessages.flatMap((message) => {
			if (
				message.type !== 'result' ||
				!('method' in message) ||
				message.method !== 'newHeads'
			)
				return []
			if (!('result' in message) || !('result' in message.result))
				throw new Error('wrong subscription payload')
			if (message.result.result === null)
				throw new Error('missing simulated block payload')
			return [message.result.result.number]
		})
		assert.equal(emittedBlockNumbers.length, 3)
		assert.deepEqual(emittedBlockNumbers, [
			blockNumber,
			blockNumber + 1n,
			blockNumber + 2n,
		])
	...
	test('newHeads skips stale subscriptions without suppressing later live subscribers', async () => {
		...
		const {
			...
			sendSubscriptionMessagesForNewBlock,
			updateEthereumSubscriptionsAndFilters,
		} = await loadModules()
		const ethereum = createEthereum()
	...
	test('newHeads reuses one simulated state computation across multiple live subscribers', async () => {
		...
		const emittedMessages = postedMessages.filter(
			(message) =>
				message.type === 'result' &&
				'method' in message &&
				message.method === 'newHeads',
