...
const getCodeInterface = new ethers.Interface(['function at(address) returns (bytes)'])
const getCodeSelector = getCodeInterface.getFunction('at').selector
				...
				case 'eth_simulateV1': {
					...
					if (lastCallInput !== undefined && dataStringWith0xStart(lastCallInput).startsWith(getCodeSelector)) {
						...
						return createMockEthSimulateV1ResultWithCustomLastBlock(blockStateCallCount, {
							...
							calls: [{
								...singleCall,
								returnData: getCodeInterface.encodeFunctionResult('at', ['0x1234']),
							}],
						})
					}
