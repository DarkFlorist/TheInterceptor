...
import {
	...
	groupEthSimulateV1ResultByInputBlocks,
	mockSignTransaction,
...
describe('SimulationModeEthereumClientService', () => {
	...
	test('typed transaction v values 0x0 and 0x1 normalize to parity', async () => {
		...
		const evenTransaction = EthereumSignedTransactionToSignedTransaction(
			EthereumSignedTransaction.parse({
				...base7702Transaction,
				v: '0x0',
			}),
		)
		...
		const oddTransaction = EthereumSignedTransactionToSignedTransaction(
			EthereumSignedTransaction.parse({
				...base7702Transaction,
				v: '0x1',
				hash: testBytes32('32'),
	...
	test('typed transaction parsers reject legacy-style v values', async () => {
		...
		assert.throws(() =>
			EthereumSignedTransactionWithBlockData.parse({
				...base7702Transaction,
				gasPrice: '0x1',
				data: '0x',
