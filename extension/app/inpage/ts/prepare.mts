...
const directoryOfThisFileTsVersion = path.join(directoryOfThisFile, '../ts')
...
async function injectJsString() {
	const contents = await fs.readFile(path.join(directoryOfThisFileTsVersion, injection), 'utf8')
	const escaped = replaceAll(replaceAll(contents,'`', '\\`'), '\\$', '\\$')
	const fileToInjectContents = await fs.readFile(path.join(directoryOfThisFileTsVersion, fileToInject), 'utf8')
