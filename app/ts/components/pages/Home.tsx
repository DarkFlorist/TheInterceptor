...
function isSignerAvailable(tabState: TabState | undefined) {
	return (
		tabState !== undefined &&
		(tabState.signerConnected || tabState.signerAccounts.length > 0)
	)
}

function SignerExplanation(param: SignerExplanationParams) {
	if (
		param.activeAddress.value !== undefined ||
		param.tabState.value === undefined ||
		param.tabState.value.signerAccountError !== undefined
	)
		return <></>
	if (!isSignerAvailable(param.tabState.value)) {
		if (
			...
			return (
				<ErrorComponent text="No signer installed. You need to install a signer, eg. Metamask." />
			)
		return (
			<ErrorComponent text="The page you are looking at has NOT CONNECTED to a wallet." />
		)
	}
	return (
		<ErrorComponent
			text={`No account connected (or wallet is locked) in ${param.tabState.value.signerName === 'NoSigner' ? 'signer' : getPrettySignerName(param.tabState.value.signerName)}.`}
		/>
	)
}
	...
	return (
		<>
			<header
				...
				style={{
					display: 'grid',
					gridTemplateColumns: 'max-content max-content minmax(0, max-content)',
			...
			>
				<div>
					<ToolTip content={tabIconReason}>
						<img
							...
							src={param.tabIconDetails.value.icon}
							width="48"
							height="48"
							style={{ display: 'block', width: '3rem', height: '3rem' }}
						/>
					</ToolTip>
				</div>
				<div>
					<div
						...
						style="border-style: solid; border-color: var(--primary-color); border-radius: 6px; padding: 1px; border-width: 1px; display: inline-flex; margin-bottom: 0;"
					>
						<button
				...
				<RpcSelector
					...
					changeRpc={param.changeActiveRpc}
				/>
			</header>
		</>
	)
...
function InterceptorDisabledButton({
	disableInterceptorToggle,
	interceptorDisabled,
	website,
}: InterceptorDisabledButtonParams) {
	return (
		<button
			disabled={website.value === undefined}
			class={`button is-small ${interceptorDisabled.value ? 'is-success' : 'is-primary'}`}
			onClick={() => disableInterceptorToggle(!interceptorDisabled.value)}
		>
			{interceptorDisabled.value ? (
				<>
					<span class="icon">
						{' '}
						<img src={ICON_ACTIVE} width="24" height="24" />{' '}
					</span>
					<span> Enable</span>
				</>
			) : (
				<>
					<span class="icon">
						{' '}
						<img src={ICON_INTERCEPTOR_DISABLED} width="24" height="24" />{' '}
					</span>
					<span> Disable</span>
				</>
			)}
...
function RichList({
	...
	async function enableMakeCurrentAddressRich(enabled: boolean) {
		sendPopupMessageToBackgroundPage({
			method: 'popup_modifyMakeMeRich',
			data: { add: enabled, address: 'CurrentAddress' },
		})
		makeCurrentAddressRich.value = enabled
	}
	async function modifyRichList(
		addressBookEntry: AddressBookEntry,
		makeRich: boolean,
	) {
		richList.value = [
			...richList.value.filter(
				(x) => x.addressBookEntry.address !== addressBookEntry.address,
			),
			...(makeRich
				? [
						{
		...
		sendPopupMessageToBackgroundPage({
			method: 'popup_modifyMakeMeRich',
			data: { add: makeRich, address: addressBookEntry.address },
		})
	}
	...
	const activeAddressSetAsRichViaFixedAddressList = useComputed(() =>
		richList.value
			.filter((element) => element.makingRich)
			.some(
				(element) =>
					element.addressBookEntry.address === activeAddress.value?.address,
			),
	)
	const visibleRichList = useComputed(() => {
		...
		if (
			richList.value.some(
				(element) =>
					element.addressBookEntry.address === peekedActiveAddress.address,
			)
		)
			return richList.value
		return [
			...richList.value,
			{
				addressBookEntry: peekedActiveAddress,
				makingRich: false,
				type: 'CurrentActiveAddress' as const,
			},
		]
	})

	const numberOfRichAddresses = useComputed(
		() => richList.value.filter((element) => element.makingRich).length,
	)
		...
		<>
			...
			>
				<p
					...
					style="font-weight: unset; font-size: 0.8em; padding: 0 0.5rem;"
				>
					<label
						class="form-control"
						style="grid-template-columns: 1em min-content; width: min-content;"
						onClick={(event) => {
					...
					>
						<input
							type="checkbox"
							checked={makeCurrentAddressRich.value}
							onInput={(e) => {
								if (e.target instanceof HTMLInputElement && e.target !== null) {
									enableMakeCurrentAddressRich(e.target.checked)
								}
							}}
							onClick={(event) => {
								event.stopPropagation()
						...
						/>
						<p class="paragraph checkbox-text" style="white-space: nowrap;">
							{' '}
				...
				<div class="card-header-icon noselect" style="cursor: pointer;">
					{numberOfRichAddresses.value === 0 ? (
						<></>
					) : (
						<p
							class="paragraph checkbox-text"
							style="white-space: nowrap; color: gray; padding-right: 10px;"
						>
							{' '}
							(+{numberOfRichAddresses.value} rich address
							{numberOfRichAddresses.value > 1 ? 'es' : ''})
						</p>
					)}
					<span class="icon">
						<ChevronIcon />
			...
			{!showList.value ? (
				<>
					{' '}
					{!activeAddressSetAsRichViaFixedAddressList.value ||
									...
									<input
										...
										onInput={(e) => {
											if (
												...
												modifyRichList(activeAddress.value, e.target.checked)
											}
										}}
									/>
									<SmallAddress
					...
					<div style={{ display: 'flex', flexDirection: 'column' }}>
						...
						{visibleRichList.value.map((richListElement) => (
							<label
								class="form-control"
								style="gap: 1em;"
								key={richListElement.addressBookEntry.address.toString()}
							>
								<input
									type="checkbox"
									...
									onInput={(e) => {
										if (
											...
											modifyRichList(
												richListElement.addressBookEntry,
												e.target.checked,
											)
										}
									}}
								/>
								<SmallAddress
									addressBookEntry={richListElement.addressBookEntry}
									renameAddressCallBack={renameAddressCallBack}
								/>
							</label>
						))}
...
function FirstCard(param: FirstCardParams) {
	...
	const timeSelectorOnChange = () => {
		...
		if (blockTimeManipulation.type === 'No Delay')
			return sendPopupMessageToBackgroundPage({
				method: 'popup_changePreSimulationBlockTimeManipulation',
				data: { blockTimeManipulation: DEFAULT_BLOCK_MANIPULATION },
			})
		return sendPopupMessageToBackgroundPage({
			method: 'popup_changePreSimulationBlockTimeManipulation',
			data: { blockTimeManipulation },
		})
	}
	...
	if (
		param.tabState.value?.signerName === 'NoSigner' &&
		param.simulationMode.value === false
	) {
		return (
			<>
				<section class="card" style="margin: 10px;">
					...
					<div class="card-content">
						<DinoSays
							text={
								'No signer connnected. You can use Interceptor in simulation mode without a signer, but signing mode requires a browser wallet.'
							}
						/>
					</div>
				</section>
				...
				<div class="card-content">
						...
						<p style="color: var(--text-color); text-align: left; padding-bottom: 10px">
							...
							param.tabState.value?.signerName === 'NoSigner' ? (
								...
								<>
									Retrieving from&nbsp;
									<SignersLogoName
										signerName={param.tabState.value.signerName}
									/>
								</>
							)}
							{signerAvailable.value ? (
								...
								<span style="float: right; color: var(--negative-color);">
									NOT CONNECTED
								</span>
							)}
						</p>
					...
					{!param.simulationMode.value ? (
						<>
							...
							param.tabIconDetails.value.icon !==
								ICON_NOT_ACTIVE_WITH_SHIELD ? (
								<div style="margin-top: 5px">
									<button
										...
										onClick={() =>
											sendPopupMessageToBackgroundPage({
												method: 'popup_requestAccountsFromSigner',
												data: true,
											})
										}
									>
...
export const isEmptySimulation = (
	...
	if (simulationStateInput === undefined)
		return isEmptySimulationAndVisualisationResults(
			simulationAndVisualisationResults,
		)
	return !simulationStateInput.some(
		(block) => block.transactions.length > 0 || block.signedMessages.length > 0,
	)
}
		...
		<div style="display: grid; grid-template-columns: auto auto; padding-left: 10px; padding-right: 10px">
			...
			<div class="log-cell" style="justify-content: right; gap: 10px;">
				<button
					...
					onClick={param.openImportSimulation}
				>
					<span
						style={{
							marginRight: '0.25rem',
				...
				{param.disableReset === undefined ||
				param.resetSimulation === undefined ? (
					<></>
				) : (
					<button
						class="btn is-small is-danger"
						disabled={param.disableReset.value}
						onClick={param.resetSimulation}
					>
						<span
							style={{
								marginRight: '0.25rem',
...
function PopupVisualisation(param: SimulationStateParam) {
	const isEmpty = useComputed(() => {
		if (param.simulationAndVisualisationResults.value.kind === 'passthrough')
			return true
		return isEmptySimulation(
			param.simulationAndVisualisationResults.value.value,
		)
	})

	const computedAddressBookEntries = useComputed(() =>
		param.simulationAndVisualisationResults.value.kind === 'simulated'
			? param.simulationAndVisualisationResults.value.value.addressBookEntries
			: [],
	)
	...
	if (
		isEmpty.value &&
		(param.simulationUpdatingState.value === 'updating' ||
			param.simulationUpdatingState.value === undefined)
	) {
		return (
			<div style="display: grid; place-items: center; height: 250px;">
				<Spinner height="3em" />
			</div>
		)
	}
		...
		return (
			<div>
				...
				<div style="padding: 10px">
					<DinoSays text={'Give me some transactions to munch on!'} />
				</div>
			</div>
		)
	}
	...
	return (
		<div>
			<SimulationResultsHeader
				openImportSimulation={param.openImportSimulation}
				disableReset={param.disableReset}
				resetSimulation={param.resetSimulation}
			/>

			{resolvedResults.visualizedSimulationState.success === false ? (
				<>
					<ErrorComponent
						text={`Failed to simulate the stack due to error: "${resolvedResults.visualizedSimulationState.jsonRpcError.error.message}". Please modify the stack to make it simutable.`}
					/>
				...
				<>
					{isEmpty.value ? (
						...
						<>
							<div
								class={
									param.simulationResultState.value === 'invalid' ||
									param.simulationUpdatingState.value === 'failed'
										? 'blur'
										: ''
								}
							>
...
export function Home(param: HomeParams) {
	...
	const removedTransactionOrSignedMessages = useSignal<
		readonly TransactionOrMessageIdentifier[]
	>([])
	...
	const activeSimulationAddress = useComputed(() =>
		param.activeSimulationAddress.value !== undefined
			? getActiveAddressEntry(
	...
	const activeSigningAddress = useComputed(() =>
		param.activeSigningAddress.value !== undefined
			? getActiveAddressEntry(
					param.activeSigningAddress.value,
					param.activeAddresses.value,
				)
			: undefined,
	)
	const currentActiveAddress = useComputed(() =>
		param.simulationMode.value
			...
			: activeSigningAddress.value,
	)

	useEffect(() => {
		if (
			!param.simulationMode.value ||
			activeSimulationAddress.value === undefined
		) {
	...
	async function removeTransactionOrSignedMessage(
		...
		return await sendPopupMessageToBackgroundPage({
	...
	async function disableInterceptorToggle() {
		...
		sendPopupMessageToBackgroundPage({
			method: 'popup_setDisableInterceptor',
			data: {
				interceptorDisabled: newValue,
				website: param.tabState.value.website,
			},
		})
	}
	...
	return (
		<>
			{param.rpcNetwork.value.httpsRpc === undefined ? (
				<ErrorComponent
					text={`${param.rpcNetwork.value.name} is not a supported network. The Interceptor is disabled while you are using ${param.rpcNetwork.value.name}.`}
				/>
			) : (
				<></>
			)}
			...
			activeSimulationAddress.value !== undefined ? (
				showPopupVisualisation.value ? (
					<PopupVisualisation
						...
						renameAddressCallBack={param.renameAddressCallBack}
						editEnsNamedHashCallBack={param.editEnsNamedHashCallBack}
						removedTransactionOrSignedMessages={
							removedTransactionOrSignedMessages.value
						}
						rpcConnectionStatus={param.rpcConnectionStatus}
					...
					<section
						class="card"
						style="margin: 10px; min-height: 250px; display: grid; place-items: center;"
					>
						<Spinner height="3em" />
					</section>
				)
				...
				<>
					...
					>
						<div style="display: grid; grid-template-columns: auto auto; padding-left: 10px; padding-right: 10px">
							...
							<div
								class="log-cell"
								style="justify-content: right; padding-left: 20px"
							>
								<InterceptorDisabledButton
									website={tabWebsite}
									disableInterceptorToggle={disableInterceptorToggle}
									interceptorDisabled={param.interceptorDisabled}
								/>
							</div>
						</div>
					</div>
