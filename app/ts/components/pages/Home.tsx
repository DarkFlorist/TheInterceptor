...
function RichList({ makeCurrentAddressRich, activeAddress, richList, renameAddressCallBack }: RichListParams) {
	...
	const activeAddressSetAsRichViaFixedAddressList = useComputed(() => richList.value.filter((element) => element.makingRich).some((element) => element.addressBookEntry.address === activeAddress.value?.address))
			...
			>
				...
				<div class="card-header-icon noselect" style="cursor: pointer;">
					{numberOfRichAddresses.value === 0 ? (
						...
						<p class="paragraph checkbox-text" style="white-space: nowrap; color: gray; padding-right: 10px;">
							{' '}
							...
							<div class="card-content-header" style="font-size: 0.8em;">
								<label class="form-control" style="gap: 1em;">
									<input
										...
										checked={true}
										onInput={(e) => {
											if (e.target instanceof HTMLInputElement && e.target !== null && activeAddress.value !== undefined) {
						...
						{visibleRichList.value.map((richListElement) => (
							<label class="form-control" style="gap: 1em;" key={richListElement.addressBookEntry.address.toString()}>
								<input
									...
									onInput={(e) => {
										if (e.target instanceof HTMLInputElement && e.target !== null) {
											modifyRichList(richListElement.addressBookEntry, e.target.checked)
										}
									}}
								/>
		...
		<>
			<section class="card" style="margin: 10px;">
				...
				<div class="card-content">
					{param.useSignersAddressAsActiveAddress.value || !param.simulationMode.value ? (
						<p style="color: var(--text-color); text-align: left; padding-bottom: 10px">
							{param.tabState.value === undefined || param.tabState.value?.signerName === 'NoSigner' ? (
								...
								<>
									Retrieving from&nbsp;
									<SignersLogoName signerName={param.tabState.value.signerName} />
								</>
							)}
					...
					<ActiveAddressComponent activeAddress={param.activeAddress} buttonText={'Change'} disableButton={!param.simulationMode.value} changeActiveAddress={param.changeActiveAddress} renameAddressCallBack={param.renameAddressCallBack} />
					{!param.simulationMode.value ? (
						...
						<div style="justify-content: space-between; padding-top: 10px;">
							...
							<TimePicker startText="Delay first transaction" mode={timeSelectorMode} absoluteTime={timeSelectorAbsoluteTime} deltaValue={timeSelectorDeltaValue} deltaUnit={timeSelectorDeltaUnit} onChangedCallBack={timeSelectorOnChange} removeNoDelayOption={true} />
						</div>
					)}
				</div>
			</section>
						...
						<>
							<div class={param.simulationResultState.value === 'invalid' || param.simulationUpdatingState.value === 'failed' ? 'blur' : ''}>
								...
								{param.removedTransactionOrSignedMessages.length > 0 ? (
									...
									<SimulationSummary simulationAndVisualisationResults={param.simulationAndVisualisationResults} currentBlockNumber={param.currentBlockNumber} activeAddress={param.activeSimulationAddress} renameAddressCallBack={param.renameAddressCallBack} rpcConnectionStatus={param.rpcConnectionStatus} />
								)}
							</div>
						</>
					)}
