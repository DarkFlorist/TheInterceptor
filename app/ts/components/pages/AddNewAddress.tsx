...
function IncompleteAddressIcon({ addressInput, logoUri }: IncompleteAddressIconParams) {
	return <AddressIcon address={stringToAddress(addressInput)} logoUri={logoUri} isBig={true} backgroundColor={'var(--text-color)'} />
}
...
function NameInput({ nameInput, setNameInput, disabled }: NameInputParams) {
	...
	return <input class="input title is-5 is-spaced" type="text" value={nameInput} placeholder={'What should we call this address?'} onInput={(e) => setNameInput((e.target as HTMLInputElement).value)} maxLength={42} ref={ref} style={'width: 100%'} disabled={disabled} />
}
		...
		<div class="media">
			...
			<div class="media-content" style="overflow-y: unset; overflow-x: unset;">
				<div class="container" style="margin-bottom: 10px;">
					<span class="log-table" style="column-gap: 5px; row-gap: 5px; grid-template-columns: max-content auto;">
						...
						<div style={{ justifyContent: 'right', display: 'flex' }}>
							{' '}
							<DropDownMenu selected={selectedAddresBookEntryType} dropDownOptions={addressBookEntryOptions} onChangedCallBack={onTypeChangedCallBack} buttonClassses={'btn btn--outline is-small'} />{' '}
						</div>
						<CellElement element={<Text text={'Chain: '} />} />
						<div style={{ justifyContent: 'right', display: 'flex' }}>
							{' '}
							<ChainSelector rpcEntries={rpcEntries} chainId={selectedChainId} changeChain={setChain} buttonClassses={'btn btn--outline is-small'} />{' '}
						</div>
								...
								<CellElement
									element={
										<input
											...
											style="width: 100%"
											type="text"
											value={modifyAddressWindowState.value.incompleteAddressBookEntry.symbol}
											placeholder={'...'}
											onInput={(e) => {
				...
				<label class="form-control">
					<input
						type="checkbox"
						checked={modifyAddressWindowState.value.incompleteAddressBookEntry.useAsActiveAddress}
						onInput={(e) => {
							if (e.target instanceof HTMLInputElement && e.target !== null) {
				...
				<label class="form-control">
					<input
						type="checkbox"
						checked={!modifyAddressWindowState.value.incompleteAddressBookEntry.askForAddressAccess}
						onInput={(e) => {
							if (e.target instanceof HTMLInputElement && e.target !== null) {
				...
				<label class="form-control">
					<input
						type="checkbox"
						checked={'declarativeNetRequestBlockMode' in modifyAddressWindowState.value.incompleteAddressBookEntry && modifyAddressWindowState.value.incompleteAddressBookEntry.declarativeNetRequestBlockMode === 'block-all'}
						onInput={(e) => {
							if (e.target instanceof HTMLInputElement && e.target !== null) {
								setDeclarativeNetRequestBlockMode(e.target.checked ? 'block-all' : 'disabled')
							}
						}}
					/>
...
export function AddNewAddress(param: AddAddressParam) {
	...
	}, [param.modifyAddressWindowState.value.windowStateId, param.modifyAddressWindowState.value.incompleteAddressBookEntry.address, param.activeAddress])
	...
	function getCompleteAddressBookEntry(): AddressBookEntry | { type: 'error'; error: string } {
		...
		const base = {
			...
			...(incompleteAddressBookEntry.declarativeNetRequestBlockMode !== undefined
				? {
	...
	const isSubmitButtonDisabled = useComputed(() => {
		return !areInputsValid.value || param.modifyAddressWindowState.value.errorState?.blockEditing || (showOnChainVerificationErrorBox.value && !onChainInformationVerifiedByUser.value)
	})
		...
		<>
			...
			<div class="modal-card">
				...
				<section class="modal-card-body">
					<div class="card" style="margin: 10px;">
						<div class="card-content">
							<RenderIncompleteAddressBookEntry modifyAddressWindowState={param.modifyAddressWindowState} rpcEntries={param.rpcEntries} canFetchFromEtherScan={canFetchFromEtherScan} fetchAbiAndNameFromBlockExplorer={fetchAbiAndNameFromBlockExplorer} />
						</div>
					</div>
				...
				<footer class="modal-card-foot window-footer" style="border-bottom-left-radius: unset; border-bottom-right-radius: unset; border-top: unset; padding: 10px;">
					{param.setActiveAddressAndInformAboutIt === undefined || param.modifyAddressWindowState.value.incompleteAddressBookEntry === undefined || activeAddress.value === stringToAddress(param.modifyAddressWindowState.value.incompleteAddressBookEntry.address) ? (
						<></>
					) : (
						<button class="button is-success is-primary" onClick={createAndSwitch} disabled={!areInputsValid.value}>
							{param.modifyAddressWindowState.value.incompleteAddressBookEntry.addingAddress ? 'Create and switch' : 'Modify and switch'}
						</button>
					)}
					<button class="button is-success is-primary" onClick={modifyOrAddEntry} disabled={isSubmitButtonDisabled.value}>
						{' '}
						{param.modifyAddressWindowState.value.incompleteAddressBookEntry.addingAddress ? 'Create' : 'Modify'}{' '}
					</button>
					<button class="button is-primary" style="background-color: var(--negative-color)" onClick={param.close}>
						Cancel
					</button>
				</footer>
			</div>
