...
import {
	requestPopupAbiAndNameFromBlockExplorer,
	requestPopupIdentifyAddress,
...
import {
	isBlockExplorerAvailableForChain,
	isValidAbi,
...
function IncompleteAddressIcon({
	addressInput,
	logoUri,
}: IncompleteAddressIconParams) {
	return (
		<AddressIcon
...
function RenderIncompleteAddressBookEntry({
	...
	const disableDueToSource =
		modifyAddressWindowState.value.incompleteAddressBookEntry.entrySource ===
			'DarkFloristMetadata' ||
		modifyAddressWindowState.value.incompleteAddressBookEntry.entrySource ===
			'Interceptor'
	const logoUri =
		modifyAddressWindowState.value.incompleteAddressBookEntry.addingAddress ===
			false &&
		'logoUri' in modifyAddressWindowState.value.incompleteAddressBookEntry
			? modifyAddressWindowState.value.incompleteAddressBookEntry.logoUri
			: undefined
	const selectedChainId = useComputed(
		() =>
			modifyAddressWindowState.value.incompleteAddressBookEntry.chainId || 1n,
	)
	const blockExplorerAvailable = useComputed(() =>
		isBlockExplorerAvailableForChain(selectedChainId.value, rpcEntries.value),
	)

	const selectedAddresBookEntryType = useSignal<AddressBookEntryType>(
		modifyAddressWindowState.value.incompleteAddressBookEntry.type,
	)
	const addressBookEntryOptions = useSignal<readonly AddressBookEntryType[]>([
		'contact',
		'contract',
		'ERC20',
		'ERC1155',
	...
	const onTypeChangedCallBack = (type: AddressBookEntryType) => {
		...
		updateIncompleteAddressBookEntry((previousEntry) =>
			modifyObject(previousEntry, { type }),
		)
	}

	type ModifyEntry =
		typeof modifyAddressWindowState.peek extends () => infer State
			? State extends { incompleteAddressBookEntry: infer Entry }
				? Entry
				: never
			: never
	const updateIncompleteAddressBookEntry = async (
		updateEntry: (previousEntry: ModifyEntry) => ModifyEntry,
	) => {
		const previousState = modifyAddressWindowState.peek()
		modifyAddressWindowState.value = modifyObject(previousState, {
			incompleteAddressBookEntry: updateEntry(
				previousState.incompleteAddressBookEntry,
			),
		})
		try {
			await sendPopupMessageToBackgroundPage({
				method: 'popup_changeAddOrModifyAddressWindowState',
				data: {
					windowStateId: modifyAddressWindowState.value.windowStateId,
					newState: modifyAddressWindowState.value,
				},
			})
		} catch (e) {
			console.error(e)
	...
	const setAddress = async (address: string) =>
		updateIncompleteAddressBookEntry((previousEntry) =>
			modifyObject(previousEntry, { address }),
		)
	const setName = async (name: string) =>
		updateIncompleteAddressBookEntry((previousEntry) =>
			modifyObject(previousEntry, { name }),
		)
	const setChain = async (chainEntry: ChainEntry) =>
		updateIncompleteAddressBookEntry((previousEntry) =>
			modifyObject(previousEntry, { chainId: chainEntry.chainId }),
		)
	const setAbi = async (abi: string) =>
		updateIncompleteAddressBookEntry((previousEntry) =>
			modifyObject(previousEntry, {
				abi: abi.trim().length === 0 ? undefined : abi,
			}),
		)
	const setSymbol = async (symbol: string) =>
		updateIncompleteAddressBookEntry((previousEntry) =>
			modifyObject(previousEntry, { symbol }),
		)
	const setDecimals = async (inputEvent: Event) =>
		updateIncompleteAddressBookEntry((previousEntry) => {
			if (
	...
	const setUseAsActiveAddress = async (useAsActiveAddress: boolean) =>
		updateIncompleteAddressBookEntry((previousEntry) =>
			modifyObject(previousEntry, { useAsActiveAddress }),
		)
	const setDeclarativeNetRequestBlockMode = async (
		declarativeNetRequestBlockMode: DeclarativeNetRequestBlockMode,
	) =>
		updateIncompleteAddressBookEntry((previousEntry) =>
			modifyObject(previousEntry, { declarativeNetRequestBlockMode }),
		)
	const setAskForAddressAccess = async (askForAddressAccess: boolean) =>
		updateIncompleteAddressBookEntry((previousEntry) =>
			modifyObject(previousEntry, { askForAddressAccess }),
		)

	const decimals = useComputed(() =>
		modifyAddressWindowState.value.incompleteAddressBookEntry.decimals !==
		undefined
			? modifyAddressWindowState.value.incompleteAddressBookEntry.decimals.toString()
			: undefined,
	)
	return (
		<div class="media">
			<div class="media-left">
				<figure class="image">
					<IncompleteAddressIcon
						addressInput={
							modifyAddressWindowState.value.incompleteAddressBookEntry.address
						}
						logoUri={logoUri}
					/>
				</figure>
			...
			<div class="media-content" style="overflow-y: unset; overflow-x: unset;">
				<div class="container" style="margin-bottom: 10px;">
					...
					>
						...
						<div style={{ justifyContent: 'right', display: 'flex' }}>
							{' '}
							<DropDownMenu
								selected={selectedAddresBookEntryType}
								dropDownOptions={addressBookEntryOptions}
								onChangedCallBack={onTypeChangedCallBack}
						...
						<CellElement
							element={
								<AddressInput
									disabled={
										modifyAddressWindowState.value.incompleteAddressBookEntry
											.addingAddress === false || disableDueToSource
									}
									addressInput={
										modifyAddressWindowState.value.incompleteAddressBookEntry
											.address
								...
								<>
									...
									<button
										...
										disabled={
											stringToAddress(
												modifyAddressWindowState.value
													.incompleteAddressBookEntry.address,
											) === undefined ||
											!canFetchFromEtherScan.value ||
											!blockExplorerAvailable.value
										}
										onClick={async () => {
											fetchAbiAndNameFromBlockExplorer()
										}}
					...
					<input
						...
						onInput={(e) => {
							if (e.target instanceof HTMLInputElement && e.target !== null) {
								setUseAsActiveAddress(e.target.checked)
							}
						}}
					/>
				...
				<label class="form-control">
					<input
						...
						onInput={(e) => {
							if (e.target instanceof HTMLInputElement && e.target !== null) {
								setAskForAddressAccess(!e.target.checked)
							}
						}}
					/>
					<p class="paragraph checkbox-text">
						Don't request for an access when used as active address(insecure)
					</p>
				</label>
				<label class="form-control">
					<input
						type="checkbox"
						checked={
							'declarativeNetRequestBlockMode' in
								modifyAddressWindowState.value.incompleteAddressBookEntry &&
							modifyAddressWindowState.value.incompleteAddressBookEntry
						...
						onInput={(e) => {
							if (e.target instanceof HTMLInputElement && e.target !== null) {
								setDeclarativeNetRequestBlockMode(
					...
					/>
					<p class="paragraph checkbox-text">
						Block all external requests on site when this address is active (not
						recommended).
					</p>
				</label>
			</div>
...
export function AddNewAddress(param: AddAddressParam) {
	...
	useEffect(() => {
		const popupMessageListener = (msg: unknown): false => {
			...
			if (parsed.method === 'popup_addOrModifyAddressWindowStateInformation') {
				if (
					parsed.data.windowStateId !==
					param.modifyAddressWindowState.value.windowStateId
				)
					return false
				param.modifyAddressWindowState.value = modifyObject(
					param.modifyAddressWindowState.value,
					{ errorState: parsed.data.errorState },
				)
			}
			return false
		}
	...
	function getCompleteAddressBookEntry():
		| AddressBookEntry
		| { type: 'error'; error: string } {
		const incompleteAddressBookEntry =
			param.modifyAddressWindowState.peek().incompleteAddressBookEntry
		if (
			incompleteAddressBookEntry.name !== undefined &&
			incompleteAddressBookEntry.name.length > 42
		)
			return { type: 'error', error: 'Name is not valid' }
		const inputedAddressBigInt = stringToAddress(
			incompleteAddressBookEntry.address,
		)
		if (inputedAddressBigInt === undefined)
			return { type: 'error', error: 'Address is not valid' }
		const name = incompleteAddressBookEntry.name
			? incompleteAddressBookEntry.name
			: checksummedAddress(inputedAddressBigInt)
		if (
			incompleteAddressBookEntry.abi !== undefined &&
			!isValidAbi(incompleteAddressBookEntry.abi)
		)
			return { type: 'error', error: 'Abi is not valid' }
		const base = {
			name,
			...
			...(incompleteAddressBookEntry.declarativeNetRequestBlockMode !==
			undefined
				? {
						declarativeNetRequestBlockMode:
							incompleteAddressBookEntry.declarativeNetRequestBlockMode,
				...
				: {}),
			...(incompleteAddressBookEntry.useAsActiveAddress !== undefined
				? { useAsActiveAddress: incompleteAddressBookEntry.useAsActiveAddress }
				: {}),
			...(incompleteAddressBookEntry.logoUri !== undefined
		...
		switch (incompleteAddressBookEntry.type) {
			case 'ERC721': {
				if (incompleteAddressBookEntry.symbol === undefined)
					return { type: 'error', error: 'Symbol is missing' }
				return {
					...base,
			...
			case 'ERC20': {
				if (incompleteAddressBookEntry.symbol === undefined)
					return { type: 'error', error: 'Symbol is missing' }
				if (incompleteAddressBookEntry.decimals === undefined)
					return { type: 'error', error: 'Decimals are missing' }
				return {
					...base,
					type: 'ERC20' as const,
					symbol: incompleteAddressBookEntry.symbol,
			...
			default:
				assertUnreachable(incompleteAddressBookEntry.type)
		}
	}
	...
	async function createAndSwitch() {
		...
		if (param.setActiveAddressAndInformAboutIt !== undefined)
			await param.setActiveAddressAndInformAboutIt(inputedAddressBigInt)
	}
	...
	const areInputsValid = useComputed(
		() => completeAddressBookEntryOrError.value.type !== 'error',
	)
	...
	async function fetchAbiAndNameFromBlockExplorer() {
		...
		param.modifyAddressWindowState.value = modifyObject(
			param.modifyAddressWindowState.value,
			{
				incompleteAddressBookEntry: modifyObject(
					param.modifyAddressWindowState.value.incompleteAddressBookEntry,
					{
						abi: reply.data.abi,
						name:
	...
	const showOnChainVerificationErrorBox = useComputed(() => {
		const incompleteAddressBookEntry =
			param.modifyAddressWindowState.value.incompleteAddressBookEntry
		return (
			incompleteAddressBookEntry.entrySource === 'OnChain' &&
			(incompleteAddressBookEntry.type === 'ERC20' ||
				incompleteAddressBookEntry.type === 'ERC721')
		)
	})
	...
	const isSubmitButtonDisabled = useComputed(() => {
		return (
			...
			(showOnChainVerificationErrorBox.value &&
				!onChainInformationVerifiedByUser.value)
		)
	})
	...
	function getCardTitle() {
		...
		const name =
			incompleteAddressBookEntry.name !== undefined
				? `${alleged}${incompleteAddressBookEntry.name}`
				: readableAddressType[incompleteAddressBookEntry.type]
		return `Modify ${name}`
	}
	const incompleteAddressBookEntry = useComputed(
		() => param.modifyAddressWindowState.value.incompleteAddressBookEntry,
	)
	return (
		<>
				...
				<section class="modal-card-body">
					...
					<div style="padding-left: 10px; padding-right: 10px; margin-bottom: 10px; min-height: 80px">
						{completeAddressBookEntryOrError.value.type !== 'error' ? (
							...
							<ErrorText text={completeAddressBookEntryOrError.value.error} />
						)}

						{param.modifyAddressWindowState.value.errorState === undefined ? (
							<></>
						) : (
							<ErrorText
								text={param.modifyAddressWindowState.value.errorState.message}
							/>
						)}
						{!showOnChainVerificationErrorBox.value ? (
							<></>
