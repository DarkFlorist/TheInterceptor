...
const WebsiteAccessProvider = ({ children }: { children: ComponentChildren }) => {
	...
	const retrieveWebsiteAccess = () => {
		sendPopupMessageToBackgroundPage({
			method: 'popup_retrieveWebsiteAccess',
			data: { query: '' },
		})
	}
				...
				>
					...
					<div class="flexy" style={{ textAlign: 'left', flex: '1', '--pad-y': 0 }}>
						<div style={{ flex: 1 }}>
							...
							>
								&lrm;{websiteAccess.website.websiteOrigin}
							</p>
						</div>
	...
	return (
		<div class="flexy flexy-sm" style={{ '--gap-x': '1rem', flex: 1 }}>
			{websiteIcon === undefined ? (
				...
				<figure>
					<img width="34" height="34" src={websiteIcon} />
				</figure>
			)}
			<div style={{ flex: 1 }}>
				<h2
				...
				>
					{websiteAccess.deepValue.website.title}
				</h2>
				<p>
					<span
						class="truncate"
					...
					>
						&lrm;{websiteAccess.deepValue.website.websiteOrigin}
					</span>
				</p>
...
const NoAccessPrompt = ({ websiteAccess, hostScopeDetails }: { websiteAccess: OptionalSignal<WebsiteAccess>; hostScopeDetails: ReadonlySignal<HostScopeDetails | undefined> }) => {
	...
	const confirmOrRejectRemoval = async (returnValue: string) => {
		...
		await sendPopupMessageToBackgroundPage({
			...
			data: { websiteOrigin: websiteAccess.deepValue.website.websiteOrigin },
		})
		...
		>
			...
			<Modal>
				...
				<Modal.Dialog class="dialog" style={{ textAlign: 'center', color: 'var(--disabled-text-color)' }} onModalClose={confirmOrRejectRemoval}>
					...
					>
						<Modal.Close class="btn btn--outline" value="reject">
							Cancel
						</Modal.Close>
						<Modal.Close class="btn btn--destructive" value="confirm">
							Confirm
						</Modal.Close>
					</div>
				</Modal.Dialog>
...
const AddressAccessCard = ({ website, addressAccess, renameAddressCallBack }: { website: ReadonlySignal<Website | undefined>; addressAccess: WebsiteAddressAccess; renameAddressCallBack: RenameAddressCallBack }) => {
	...
	const setAddressAccess = (event: Event) => {
		...
		sendPopupMessageToBackgroundPage({
			method: 'popup_allowOrPreventAddressAccessForWebsite',
			data: {
				website: currentWebsite,
				address: addressAccess.address,
				allowAccess: event.target.checked,
			},
		})
	}
	...
	return (
		<div
			style={{
				display: 'grid',
				gridTemplateColumns: 'minmax(0,1fr) min-content min-content',
				columnGap: '1rem',
				alignItems: 'center',
			}}
		>
...
const RemoveAddressConfirmation = ({ website, addressBookEntry }: { addressBookEntry: AddressBookEntry; website: ReadonlySignal<Website | undefined> }) => {
	const removeAddressAccessForWebsite = async () => {
		...
		if (currentWebsite === undefined) return
		sendPopupMessageToBackgroundPage({
			method: 'popup_removeWebsiteAddressAccess',
			data: {
				websiteOrigin: currentWebsite.websiteOrigin,
...
const BlockRequestSetting = ({ websiteAccess, hostScopeDetails }: { websiteAccess: OptionalSignal<WebsiteAccess>; hostScopeDetails: ReadonlySignal<HostScopeDetails | undefined> }) => {
	const setWebsiteExternalRequestBlocking = async (shouldBlock: boolean) => {
		...
		sendPopupMessageToBackgroundPage({
			method: 'popup_blockOrAllowExternalRequests',
			data: { website: websiteAccess.deepValue.website, shouldBlock },
		})
	}
					...
					{requestBlockMode.value === 'block-all' ? (
						...
						<Modal>
							...
							<Modal.Dialog
								...
								onModalClose={confirmOrRejectRequestBlocking}
							>
								<h2
									style={{
										fontWeight: 600,
								...
								>
									Confirm Blocking External Requests For Host
								</h2>
								<p></p>
...
const DisableProtectionSetting = ({ websiteAccess, hostScopeDetails }: { websiteAccess: OptionalSignal<WebsiteAccess>; hostScopeDetails: ReadonlySignal<HostScopeDetails | undefined> }) => {
	const disableWebsiteProtection = async (shouldDisable = true) => {
		...
		sendPopupMessageToBackgroundPage({
			...
			data: {
				website: websiteAccess.deepValue.website,
				interceptorDisabled: shouldDisable,
			},
		})
	}
	...
	return (
		<article class="flexy flexy-lg">
			<figure>
				<i class="status-lg status-danger">
					<InterceptorDisabledIcon />
				</i>
			</figure>
			<section class="flexy" style={{ flex: 1, '--pad-y': 0 }}>
					...
					{isInterceptorDisabled.value ? (
						...
						<Modal>
							...
							<Modal.Dialog
								...
								onModalClose={confirmOrRejectDialog}
							>
								<h2
									style={{
										fontWeight: 600,
								...
								>
									Disable Interceptor Protection For Host
								</h2>
								<p></p>
								<p style={{ marginBlock: '0.5rem' }}>
...
const RemoveWebsiteSetting = ({ websiteAccess, hostScopeDetails }: { websiteAccess: OptionalSignal<WebsiteAccess>; hostScopeDetails: ReadonlySignal<HostScopeDetails | undefined> }) => {
	...
	return (
		<article class="flexy flexy-lg">
			<figure>
				<i
					class="status-lg status-outline"
					style={{
						'--fg-color': 'var(--status-danger-outline-color)',
						'--outline': '1px solid var(--status-danger-outline-color)',
					}}
				>
					<TrashIcon />
				</i>
			...
			<section class="flexy" style={{ flex: 1, '--pad-y': 0 }}>
				...
				<aside>
					<Modal>
						<Modal.Open class="btn btn--destructive">
							<span style={{ whiteSpace: 'nowrap' }}>Remove Host</span>
						</Modal.Open>
						<Modal.Dialog
							class="dialog"
							style={{
							...
							onModalClose={confirmOrRejectUpdate}
						>
							<h2
								style={{
									fontWeight: 600,
