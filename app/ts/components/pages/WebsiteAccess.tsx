	...
	return (
		<section style = { { paddingBlock: '1rem' } }>
			...
			{ websiteAccessList.value.length < 1 ? <EmptyAccessList /> : <>
				<ul role = 'listbox'>{ websiteAccessList.value.map((access, index) => <WebsiteAccessOverview key = { access.website.websiteOrigin } websiteAccess = { access } checked = { index === 0 } />) }</ul>
				<input type = 'submit' style = { { display: 'none' } } />
			</> }
		</section>
	)
...
const WebsiteAccessOverview = ({ websiteAccess, checked }: WebsiteAccessOverviewProps) => {
	...
	return (
		<li role = 'option'>
			...
			<label for = { websiteAccess.website.websiteOrigin } style = { { cursor: 'pointer' } } onClick = { handleChange }>
