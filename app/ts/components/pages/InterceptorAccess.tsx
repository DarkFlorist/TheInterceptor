...
function Title({ icon, title }: { icon: string | undefined; title: string }) {
	...
	return (
		<span style="font-weight: 900; line-height: 48px">
			{websiteIcon === undefined ? <></> : <img src={websiteIcon} width="48" height="48" style="width: 48px; height: 48px; vertical-align: bottom; margin-right: 10px;" />}
			{title}
		</span>
	)
}
		...
		<>
			{accessRequest.requestAccessToAddress === undefined ? (
				...
				<>
					<div class="notification" style="background-color: var(--importance-box-color); color: var(--text-color)">
						...
						<div class="notification" style="padding: 10px; background-color: var(--alpha-015); justify-content: center; ">
							{accessRequest.simulationMode ? (
								<ActiveAddressComponent activeAddress={accessRequest.requestAccessToAddress} renameAddressCallBack={renameAddressCallBack} changeActiveAddress={changeActiveAddress} buttonText={'Change'} disableButton={false} />
							) : (
								<>
					...
					<AssociatedTogether associatedAddresses={accessRequest.associatedAddresses} renameAddressCallBack={renameAddressCallBack} />
				</>
			)}
		</>
	)
...
export function InterceptorAccess() {
	...
	const selectedPendingAccessRequest = getSelectedPendingAccessRequest(pendingAccessRequests.value, appPage.value.page === 'Home' ? undefined : appPage.value.accessRequestId)
			...
			<Hint>
				...
				<div class="block popup-block">
					<div class="popup-block-scroll">
						<AccessRequests changeActiveAddress={changeActiveAddress} renameAddressCallBack={renameAddressCallBack} pendingAccessRequests={pendingAccessRequests.value} refreshActiveAddress={refreshActiveAddress} approve={approve} reject={reject} informationChangedRecently={informationChangedRecently} />
					</div>
				</div>
			</Hint>
		</main>
