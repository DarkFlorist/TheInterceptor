...
function Title({ icon, title }: { icon: string | undefined; title: string }) {
	...
	return (
		<span style="font-weight: 900; line-height: 48px">
			{websiteIcon === undefined ? (
				...
				<img
					...
					width="48"
					height="48"
					style="width: 48px; height: 48px; vertical-align: bottom; margin-right: 10px;"
				/>
			)}
			{title}
		</span>
...
function AssociatedTogether({
	associatedAddresses,
	renameAddressCallBack,
}: {
	associatedAddresses: AddressBookEntries
				...
				{!showLogs.value ? (
					...
					>
						{associatedAddresses.length <= 1 ? (
							<DinoSays
								text={
									"Given its size, a tiny dinosaur wouldn't be expected to know any..."
								}
							/>
						) : (
							<ul>
								{associatedAddresses.map((info, index) => (
									<li
										...
										style={`margin: 0px; margin-bottom: ${index < associatedAddresses.length - 1 ? '10px;' : '0px'}`}
									>
										<BigAddress
		...
		<>
			{accessRequest.requestAccessToAddress === undefined ? (
				<div style="margin: 10px">
					...
					>
						<Title
							icon={accessRequest.website.icon}
							title={
								accessRequest.website.title === undefined
									? accessRequest.website.websiteOrigin
									: accessRequest.website.title
							}
						/>
						<br />
				...
				<>
					<div
						class="notification"
						style="background-color: var(--importance-box-color); color: var(--text-color)"
					>
						<p
							class="title is-3"
						...
						<div
							class="notification"
							style="padding: 10px; background-color: var(--alpha-015); justify-content: center; "
						>
							{accessRequest.simulationMode ? (
								...
								<>
									...
									/>
									<p
										style="color: var(--subtitle-text-color); white-space: normal;"
										class="subtitle is-7"
									>
				...
				<>
					<div class="card" style="margin-bottom: 10px;">
						...
						<div class="card-content" style="padding-bottom: 5px;">
							<AccessRequest
								renameAddressCallBack={(entry: AddressBookEntry) =>
									param.renameAddressCallBack(
										pendingRequest.accessRequestId,
										entry,
									)
								}
								accessRequest={pendingRequest}
								changeActiveAddress={() =>
									param.changeActiveAddress(pendingRequest.accessRequestId)
								}
								refreshActiveAddress={() =>
									param.refreshActiveAddress(pendingRequest.accessRequestId)
								}
							/>
						</div>
						...
						<nav class="popup-button-row">
							<div style="display: flex; flex-direction: row;">
								<button
									...
									onClick={() => param.reject(pendingRequest.accessRequestId)}
									disabled={param.informationChangedRecently.value}
								>
									Deny Access
								</button>
								<button
									...
									style="flex-grow: 1; margin-left: 5px; margin-right: 5px;"
									onClick={() => param.approve(pendingRequest.accessRequestId)}
									disabled={param.informationChangedRecently.value}
								>
									Grant Access
								</button>
...
type Page =
	| { page: 'Home'; accessRequestId: string }
	| {
...
export function getSelectedPendingAccessRequest(
	pendingAccessRequests: PendingAccessRequests,
	accessRequestId: string | undefined,
) {
	if (accessRequestId === undefined) return pendingAccessRequests[0]
	return pendingAccessRequests.find(
		(request) => request.accessRequestId === accessRequestId,
	)
}
...
export function InterceptorAccess() {
	...
	useEffect(() => {
		function popupMessageListener(msg: unknown): false {
			...
			if (
				parsed.method === 'popup_interceptorAccessDialog' ||
				parsed.method === 'popup_interceptor_access_dialog_pending_changed'
			) {
				if (
					parsed.method === 'popup_interceptor_access_dialog_pending_changed'
				) {
					if (pendingAccessRequests.value.length > 0)
	...
	async function approve(accessRequestId: string) {
		const accessRequest = pendingAccessRequests.value.find(
			(request) => request.accessRequestId === accessRequestId,
		)
		...
		const data = {
			...
			originalRequestAccessToAddress:
				accessRequest.originalRequestAccessToAddress?.address,
			accessRequestId: accessRequest.accessRequestId,
		}
		informationUpdatedTimestamp.value = Date.now()
		if (pendingAccessRequests.value.length === 1)
			await tryFocusingTabOrWindow({
				type: 'tab',
				id: accessRequest.socket.tabId,
			})
	...
	function renameAddressCallBack(
		accessRequestId: string,
		entry: AddressBookEntry,
	) {
		appPage.value = {
			page: 'ModifyAddress',
			state: new Signal(addressEditEntry(entry)),
			accessRequestId,
		}
	}

	const changeActiveAddress = (accessRequestId: string) => {
		appPage.value = { accessRequestId, page: 'ChangeActiveAddress' }
	}

	async function refreshMetadata() {
		await sendPopupMessageToBackgroundPage({
			method: 'popup_refreshInterceptorAccessMetadata',
		})
	}
	...
	async function refreshActiveAddress(accessRequestId: string) {
		...
		if (accessRequest === undefined) throw Error('accessRequest is undefined')
		await sendPopupMessageToBackgroundPage({
			method: 'popup_interceptorAccessRefresh',
			data: {
	...
	async function setActiveAddressAndInformAboutIt(
		accessRequestId: string,
		address: bigint | 'signer',
	) {
		const accessRequest = pendingAccessRequests.value.find(
			(request) => request.accessRequestId === accessRequestId,
		)
	...
	const isModalActive =
		appPage.value.page !== 'Home' && selectedPendingAccessRequest !== undefined
		...
		<main>
			<Hint>
				<div class={`modal ${isModalActive ? 'is-active' : ''}`}>
					...
					selectedPendingAccessRequest !== undefined ? (
						<AddNewAddress
							setActiveAddressAndInformAboutIt={(address: bigint | 'signer') =>
								setActiveAddressAndInformAboutIt(
									appPage.value.accessRequestId,
									address,
								)
							}
							modifyAddressWindowState={appPage.value.state}
							close={() => {
								appPage.value = { page: 'Home', accessRequestId: '' }
							}}
							activeAddress={
								selectedPendingAccessRequest.requestAccessToAddress?.address
							}
					...
					selectedPendingAccessRequest !== undefined ? (
						<ChangeActiveAddress
							...
							renameAddressCallBack={(entry: AddressBookEntry) =>
								renameAddressCallBack(appPage.value.accessRequestId, entry)
							}
