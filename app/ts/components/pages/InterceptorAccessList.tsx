...
export function InterceptorAccessList(param: InterceptorAccessListParams) {
	...
	function hasChanged(state: EditableAccess) {
		if (state.removed || state.access !== state.websiteAccess.access || state.interceptorDisabled !== state.websiteAccess.interceptorDisabled || state.declarativeNetRequestBlockMode !== state.websiteAccess.declarativeNetRequestBlockMode) return true
										...
										<div class="card">
											<div class="card-header">
												...
												<div class="card-header-title" style="width: 13em">
													<CopyToClipboard content={access.websiteAccess.website.websiteOrigin} copyMessage="Website address copied!">
												...
												<>
													<label class="form-control" style="margin: auto">
														...
														<p class="paragraph checkbox-text" style="white-space: nowrap;">
															Disable Interceptor for the host (not recommended).{' '}
														</p>
													</label>
													<label class="form-control" style="margin: auto">
														<input
															type="checkbox"
															checked={access.declarativeNetRequestBlockMode === 'block-all'}
															onInput={(e) => {
																if (e.target instanceof HTMLInputElement) {
																	setWebsiteAccess(accessListIndex, {
																		declarativeNetRequestBlockMode: e.target.checked ? 'block-all' : 'disabled',
																	})
																}
															}}
														/>
														<p class="paragraph checkbox-text" style="white-space: nowrap;">
															Block all external requests made by the host (not recommended).{' '}
														</p>
													</label>
													{access.addressAccess.length === 0 ? (
														<p class="paragraph"> No individual address accesses given </p>
													) : (
														<>
																...
																<li key={addressString(websiteAccessAddress.address)} style={`margin: 0px; margin-bottom: ${addressIndex < access.addressAccessModified.length - 1 ? '10px;' : '0px'}`}>
																	{websiteAccessAddress.removed ? (
																		...
																		<div style="display: flex; width: 100%; overflow: hidden;">
																			<SmallAddress
																				addressBookEntry={
																					metadata.value.get(addressString(websiteAccessAddress.address)) || {
																						type: 'contact',
																			...
																			<div style="margin-left: auto; flex-shrink: 0; display: flex">
																				<label class="form-control" style="margin: auto">
																					<input
																						type="checkbox"
																						checked={websiteAccessAddress.access}
																						onInput={(e) => {
																							if (e.target instanceof HTMLInputElement) {
																								setAddressAccess(accessListIndex, addressIndex, {
																									access: e.target.checked,
																								})
																							}
																						}}
