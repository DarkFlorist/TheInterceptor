...
export function SignerLogoText(param: {
	signerName: SignalOrValue<SignerName>
	text: SignalOrValue<string>
}) {
	const signerLogo = getSignerLogo(resolveSignal(param.signerName))
	return (
		<p style="line-height: 24px; display: inline-block;">
			{signerLogo ? (
				<img
					style="width: 24px; height: 24px; vertical-align: text-top;"
					width="24"
					height="24"
					src={signerLogo}
				/>
...
export function SignersLogoName(param: {
	signerName: SignalOrValue<SignerName>
}) {
	...
	return (
		<SignerLogoText
			...
			text={getPrettySignerName(signerName)}
		/>
	)
}
