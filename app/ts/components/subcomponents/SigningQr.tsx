...
export function SigningQrScanner({ onFrame }: { onFrame: (frame: string) => Promise<boolean> }) {
	...
	const start = async () => {
		...
		try {
			...
			const stream = await navigator.mediaDevices.getUserMedia({ video: { width: { ideal: 640 }, height: { ideal: 480 }, facingMode: 'environment' }, audio: false })
			let stopped = false
			let timer: ReturnType<typeof setTimeout> | undefined
			stop.current = () => { stopped = true; clearTimeout(timer); for (const track of stream.getTracks()) track.stop(); if (mounted.current) setRunning(false) }
			if (!mounted.current || video.current === null) { stop.current(); return }
			...
			const scan = async () => {
				...
				try {
					...
					for (const frame of frames) {
						...
						if (await onFrame(frame.rawValue)) { stop.current?.(); return }
					}
					if (!stopped) timer = setTimeout(() => { void scan() }, 150)
				} catch (failure) { stop.current?.(); setError(failure instanceof Error ? failure.message : 'Unable to scan QR code') }
			}
			await scan()
		} catch (failure) { stop.current?.(); setError(failure instanceof Error ? failure.message : 'Camera permission was denied or the camera is unavailable') }
	}
	return <section>
