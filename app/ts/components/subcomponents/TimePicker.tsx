...
export const getTimeManipulatorFromSignals = (timeSelectorMode: TimePickerMode, timeSelectorAbsoluteTime: Date | undefined, timeSelectorDeltaValue: bigint, timeSelectorDeltaUnit: DeltaUnit) => {
	switch (timeSelectorMode) {
		...
		case 'For':
			return {
				type: 'AddToTimestamp',
				deltaToAdd: timeSelectorDeltaValue,
	...
	return (
		<div>
			<div style="display: flex; justify-content: space-between">
				<p class="paragraph" style="align-content: center;">
					{' '}
					{startText}{' '}
				</p>
				<div style="display: grid; grid-template-columns: auto auto auto; column-gap: 10px; padding-left: 5px">
					...
					<TimePickerModeViews
						...
						deltaValue={temporaryDeltaValue}
						deltaUnit={temporaryDeltaUnit}
						timePickerDeltaOptionsSignal={timePickerDeltaOptionsSignal}
						changeDeltaUnit={changeDeltaUnit}
						absoluteTimeChanged={absoluteTimeChanged}
						changeDeltaValue={changeDeltaValue}
					/>

					<button
						class="btn is-small is-primary"
						onClick={commitOptions}
						style={{
							visibility: hasValuesChanged.value ? 'visible' : 'hidden',
						}}
					>
						Commit
					</button>
