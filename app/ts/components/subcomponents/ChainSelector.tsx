...
export function RpcSelector(params: RpcSelectorParams) {
	const options = useComputed(() =>
		params.rpcEntries.value.map((rpcEntry) => rpcEntry.name),
	)
	const selected = useComputed(
		() => params.rpcNetwork.value?.name || 'No RPC Selected',
	)
	const onChangedCallBack = (rpcName: string) => {
		const newEntry = params.rpcEntries.value.find(
			(rpcEntry) => rpcEntry.name === rpcName,
		)
		if (newEntry === undefined)
			throw new Error(`Tried to change rpc that does not exist: ${rpcName}`)
...
export function ChainSelector(params: ChainSelectorParams) {
	const chains = useComputed(() =>
		rpcEntriesToChainEntriesWithAllChainsEntry(params.rpcEntries.value),
	)
	const options = useComputed(() => chains.value.map((entry) => entry.name))
	const selected = useComputed(
		() =>
			chains.value.find(
				(chainEntry) => chainEntry.chainId === params.chainId.value,
			)?.name || 'No Chain Selected',
	)
	const onChangedCallBack = (rpcName: string) => {
		const newEntry = params.rpcEntries.value.find(
			(rpcEntry) => rpcEntry.name === rpcName,
		...
		if (newEntry === undefined)
			throw new Error(`Tried to change chain that does not exist: ${rpcName}`)
	...
	return (
		<DropDownMenu
			...
			dropDownOptions={options}
			onChangedCallBack={onChangedCallBack}
			buttonClassses={params.buttonClassses}
		/>
	)
}
