...
const ConfigureRpcForm = ({ defaultValues, onCancel, onSave, onRemove }: ConfigureRpcFormProps) => {
	...
	const parseRpcFormData = (formData: FormData) => {
		...
		const newRpcEntry = {
			...
			...(isBlockExplorerDefined
				? {
						blockExplorer: {
							apiUrl: blockExplorerUrlForm || '',
							apiKey: blockExplorerApiKeyForm,
						},
					}
				: {}),
			minimized: true,
					...
					<>
						...
						{defaultValues && onRemove ? (
							<button
								type="button"
								class="btn btn--ghost"
								style="--area: 1 / 1; --btn-text-color: var(--negative-color)"
								onClick={() => {
...
const RpcUrlField = ({ defaultValue }: { defaultValue?: string }) => {
	...
	return (
		<TextInput
			...
			onInput={(e) => deferredQueryAnRpcUrl(e.currentTarget.value)}
			statusIcon={<StatusIcon state={rpcQuery.value.state} />}
			style="--area: 1 / span 2"
			required
			autoComplete="off"
			autoFocus={defaultValue === undefined}
...
const StatusIcon = ({ state }: { state: AsyncStates }) => {
	switch (state) {
		...
		case 'rejected':
			return (
				<i style={{ color: 'var(--negative-color)' }}>
					<XMarkIcon />
				</i>
			)
		case 'resolved':
...
const SpinnerIcon = () => <svg class="spin" width="1em" height="1em" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const CheckIcon = () => <svg width="1em" height="1em" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
		<path d="M15 3L5.64686 12.5524L1 7.84615" stroke="var(--positive-color, currentColor)" strokeWidth={2} />
	</svg>

const Trash = () => <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 32 32">
