...
const enrichEnsEvent = (
	event: ParsedEnsEvent,
	ens: {
		ensNameHashes: MaybeENSNameHashes
		ensLabelHashes: MaybeENSLabelHashes
	},
	addressMetaData: Map<string, AddressBookEntry>,
) => {
	const getNameHash = (node: bigint) =>
		ens.ensNameHashes.find((nameHash) => nameHash.nameHash === node) ?? {
			nameHash: node,
			name: undefined,
		}
	const getLabelHash = (labelHash: bigint) =>
		ens.ensLabelHashes.find((nameHash) => nameHash.labelHash === labelHash) ?? {
			labelHash: labelHash,
			label: undefined,
		}
	...
	switch (event.subType) {
		case 'ENSNameWrapped': {
			...
			return {
				...event,
				logInformation: {
					...event.logInformation,
					owner,
					node: getNameHash(event.logInformation.node),
				},
			}
		}
		case 'ENSFusesSet':
		...
		case 'ENSNewOwner': {
			...
			return {
				...
				logInformation: {
					...event.logInformation,
					node: getNameHash(event.logInformation.node),
					labelHash: getLabelHash(event.logInformation.labelHash),
					owner,
				},
			}
		}
		...
		case 'ENSBaseRegistrarNameRenewed':
			return {
				...event,
				logInformation: {
					...event.logInformation,
...
export function formSimulatedAndVisualizedTransactions(
	simulatedTransactions: readonly SimulatedTransaction[],
	eventsForEachTransaction: readonly EnrichedEthereumEvents[],
	rpcNetwork: RpcNetwork,
	parsedInputData: readonly EnrichedEthereumInputData[],
	protectorResults: readonly ProtectorResults[],
	addressBookEntries: readonly AddressBookEntry[],
