	...
	return (
		<div class="notification transaction-importance-box">
			...
			<div class="box" style="background-color: var(--alpha-005); box-shadow: unset; margin-bottom: 0px;">
				<BigAddress addressBookEntry={param.approval.to} renameAddressCallBack={param.renameAddressCallBack} />
			</div>
			<span style="grid-template-columns: auto auto; display: grid;">
				<p class="paragraph" style="font-size: 28px; font-weight: 500; justify-self: right;">
					{' '}
				...
				<span class="log-table" style="justify-content: center; column-gap: 5px;">
					<div class="log-cell" style="justify-content: right;">
						{param.approval.type === 'NFT All approval' ? (
							<AllApproval {...param.approval} style={{ 'font-weight': '500', color: textColor }} fontSize="big" />
						) : (
							<>
								{' '}
								{'amount' in param.approval && param.approval.amount >= 2n ** 96n - 1n ? (
									...
									<TokenAmount amount={param.approval.amount} tokenEntry={param.approval.token} style={{ 'font-weight': '500', color: textColor }} fontSize="big" />
								) : (
									<></>
								)}{' '}
							</>
					...
					<div class="log-cell">
						<TokenSymbol {...tokenEventToTokenSymbolParams(param.approval)} useFullTokenName={false} style={{ 'font-weight': '500', color: textColor }} renameAddressCallBack={param.renameAddressCallBack} fontSize="big" />
					</div>
				</span>
			</div>
