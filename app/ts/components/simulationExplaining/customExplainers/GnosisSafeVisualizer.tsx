...
const requestToSimulate = (gnosisSafeMessage: VisualizedPersonalSignRequestSafeTx) =>
...
const ShowSuccessOrFailure = ({ simulateExecutionReply, activeAddress, renameAddressCallBack, editEnsNamedHashCallBack, gnosisSafeMessage }: ShowSuccessOrFailureParams) => {
	...
	if (simulateExecutionReply.value === undefined) {
		return (
			<div style="display: flex; justify-content: center;">
				<button class={'button is-primary'} onClick={() => requestToSimulate(gnosisSafeMessage)} disabled={false}>
					Simulate execution
				</button>
			</div>
		)
	...
	return (
		<div style="display: grid; grid-template-rows: max-content">
			<Transaction simTx={simTx.value} simulationAndVisualisationResults={results.value} removeTransactionOrSignedMessage={undefined} activeAddress={activeAddress} renameAddressCallBack={renameAddressCallBack} editEnsNamedHashCallBack={editEnsNamedHashCallBack} addressMetaData={addressMetaData} />
		</div>
	)
}
		...
		<>
			<div class="notification transaction-importance-box">
				<span class="log-table" style="justify-content: center; grid-template-columns: auto auto auto">
					...
					<div class="log-cell">
						{' '}
						<SmallAddress addressBookEntry={param.gnosisSafeMessage.verifyingContract} renameAddressCallBack={param.renameAddressCallBack} />{' '}
					</div>
					<div class="log-cell">
						{' '}
			...
			<div class="notification dashed-notification">
				...
				<ShowSuccessOrFailure gnosisSafeMessage={param.gnosisSafeMessage} simulateExecutionReply={simulateExecutionReply} renameAddressCallBack={param.renameAddressCallBack} editEnsNamedHashCallBack={param.editEnsNamedHashCallBack} activeAddress={activeAddress} />
			</div>
			{simulateExecutionReply.value === undefined ? (
				<></>
			) : (
				<div class="log-cell" style="justify-content: right; margin-top: 10px;">
					<button class={'button is-primary is-small'} onClick={() => requestToSimulate(param.gnosisSafeMessage)}>
						Refresh simulation
					</button>
				</div>
			)}
