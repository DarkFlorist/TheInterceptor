...
const ShowSuccessOrFailure = ({ simulateExecutionReply, simTx, activeAddress, renameAddressCallBack, editEnsNamedHashCallBack }: ShowSuccessOrFailureParams) => {
		...
		return (
			<div style="display: flex; justify-content: center;">
				{simTx.value.transaction.to !== undefined && 'abi' in simTx.value.transaction.to && simTx.value.transaction.to.abi !== undefined ? (
					<button class={'button is-primary'} onClick={() => simulateGovernanceVote(simTx.value!.transactionIdentifier)} disabled={false}>
						Simulate execution on a passing vote
					</button>
				) : (
					<>
						{' '}
						<MissingAbi errorMessage={missingAbiText} addressBookEntry={simTx.value.transaction.to} renameAddressCallBack={renameAddressCallBack} />
					</>
				)}
			</div>
		)
	...
	if (simulateExecutionReply.value.data.success === false) {
		return (
			<div style="display: grid; grid-template-rows: max-content">
				{simulateExecutionReply.value.data.errorType === 'MissingAbi' ? <MissingAbi errorMessage={missingAbiText} addressBookEntry={simulateExecutionReply.value.data.errorAddressBookEntry} renameAddressCallBack={renameAddressCallBack} /> : <ErrorComponent text={errorText} />}
			</div>
		)
	}
	...
	return (
		<div style="display: grid; grid-template-rows: max-content">
			<Transaction simTx={govSimTx.value} simulationAndVisualisationResults={results.value} removeTransactionOrSignedMessage={undefined} activeAddress={activeAddress} renameAddressCallBack={renameAddressCallBack} editEnsNamedHashCallBack={editEnsNamedHashCallBack} addressMetaData={addressMetaData} />
		</div>
	)
}
	...
	return (
		<>
				...
				<span class="log-table" style="padding-bottom: 10px; grid-template-columns: auto auto;">
					...
					<div class="log-cell" style="justify-content: right;">
						{simulateExecutionReply.value === undefined ? (
							...
							<button class={'button is-primary is-small'} onClick={() => simulateGovernanceVote(simTx.value!.transactionIdentifier)}>
								Refresh
							</button>
						)}
					</div>
			...
			<div class="notification dashed-notification">
				<ShowSuccessOrFailure simulateExecutionReply={simulateExecutionReply} renameAddressCallBack={param.renameAddressCallBack} editEnsNamedHashCallBack={param.editEnsNamedHashCallBack} simTx={simTx} activeAddress={activeAddress} />
			</div>
		</>
	)
}
