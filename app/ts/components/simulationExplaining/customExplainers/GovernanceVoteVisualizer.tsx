...
import {
	MessageToPopup,
	type GovernanceVoteInputParameters,
			...
			<div style="display: flex; justify-content: center; padding-top: 10px">
				{params.addressBookEntry === undefined ? (
					...
					<button
						...
						onClick={() =>
							params.addressBookEntry !== undefined &&
							params.renameAddressCallBack(params.addressBookEntry)
						}
					>
						Add Abi
					</button>
...
function VotePanel({
	...
	inputParams: SignalOrValue<GovernanceVoteInputParameters | undefined>
}) {
			...
			resolvedInputParams.params !== undefined ? (
				<>
					<div class="container">
						<span
							class="log-table"
							style="justify-content: center; column-gap: 5px; row-gap: 5px; grid-template-columns: auto auto"
						>
							{resolvedInputParams.reason !== undefined ? (
								<>
							...
							{resolvedInputParams.signature !== undefined ? (
								<>
									...
									<CellElement
										text={dataStringWith0xStart(resolvedInputParams.signature)}
									/>
								</>
							) : (
								<></>
							...
							{resolvedInputParams.voter !== undefined ? (
								<>
									...
									<CellElement
										text={checksummedAddress(resolvedInputParams.voter)}
									/>
								</>
							) : (
								<></>
							...
							{resolvedInputParams.params !== undefined ? (
								<>
									...
									<CellElement
										text={dataStringWith0xStart(resolvedInputParams.params)}
									/>
								</>
							) : (
								<></>
...
const simulateGovernanceVote = (transactionIdentifier: EthereumQuantity) =>
	sendPopupMessageToBackgroundPage({
		method: 'popup_simulateGovernanceContractExecution',
...
const ShowSuccessOrFailure = ({
	...
	const missingAbiText =
		'The governance contract is missing an ABI. Add an ABI to simulate execution of this proposal.'
	...
	if (simulateExecutionReply.value === undefined) {
		return (
			<div style="display: flex; justify-content: center;">
				{simTx.value.transaction.to !== undefined &&
				'abi' in simTx.value.transaction.to &&
				simTx.value.transaction.to.abi !== undefined ? (
					<button
						class={'button is-primary'}
						onClick={() =>
							simulateGovernanceVote(simTx.value!.transactionIdentifier)
						}
						disabled={false}
					>
	...
	if (simulateExecutionReply.value.data.success === false) {
		return (
			<div style="display: grid; grid-template-rows: max-content">
				{simulateExecutionReply.value.data.errorType === 'MissingAbi' ? (
					<MissingAbi
						errorMessage={missingAbiText}
						addressBookEntry={
							simulateExecutionReply.value.data.errorAddressBookEntry
						}
						renameAddressCallBack={renameAddressCallBack}
					/>
				) : (
...
export function GovernanceVoteVisualizer(
	param: GovernanceVoteVisualizerParams,
) {
	const simulateExecutionReply = useSignal<SimulateExecutionReply | undefined>(
		undefined,
	)
	const governanceVoteInputParameters = useSignal<
		GovernanceVoteInputParameters | undefined
	>(undefined)
	const simTx = useSignal<SimulatedAndVisualizedTransaction | undefined>(
		undefined,
	)
	...
	useEffect(() => {
		...
		param.simTx.transactionIdentifier,
		param.governanceVoteInputParameters,
		param.activeAddress.value,
	])

	if (
		governanceVoteInputParameters.value === undefined ||
		simTx.value === undefined ||
	...
	return (
		<>
			...
			<div style="display: grid; grid-template-rows: max-content max-content">
				<span
					class="log-table"
					style="padding-bottom: 10px; grid-template-columns: auto auto;"
				>
					<div class="log-cell">
						<p class="paragraph">
