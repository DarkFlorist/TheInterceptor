...
export function AddressBeforeAfter({
	address,
	beforeAndAfter,
	renameAddressCallBack,
	tokenOrEtherDefinition,
}: TransferAddressParams) {
	return (
		<>
			<BigAddress
				addressBookEntry={address}
				renameAddressCallBack={renameAddressCallBack}
				style={{ '--bg-color': 'var(--address-muted-bg-color)' }}
			/>
			{beforeAndAfter === undefined ? (
				...
				<span style="grid-template-columns: auto auto; display: grid; justify-content: space-between; margin-top: 10px">
					<span style="grid-template-columns: auto; display: grid;">
						...
						<TokenOrEth
							{...{ ...tokenOrEtherDefinition, amount: beforeAndAfter.before }}
						/>
					</span>
					<span style="grid-template-columns: auto; display: grid;">
						<p class="paragraph" style="color: var(--subtitle-text-color);">
						...
						<TokenOrEth
							{...{ ...tokenOrEtherDefinition, amount: beforeAndAfter.after }}
						/>
					</span>
				</span>
			)}
...
export const getAsset = (
	transfer: TokenVisualizerResultWithMetadata,
	renameAddressCallBack: RenameAddressCallBack,
) => {
	switch (transfer.type) {
		case 'ERC1155':
			return {
				...tokenEventToTokenSymbolParams(transfer),
...
export function SimpleTokenTransferVisualisation({
	simTx,
	renameAddressCallBack,
}: {
	simTx: SimulatedAndVisualizedSimpleTokenTransferTransaction
	renameAddressCallBack: RenameAddressCallBack
}) {
	...
	const receiverAfter = simTx.tokenBalancesAfter.find(
		(change) =>
			change.owner === transfer.to.address &&
			change.token === asset.tokenEntry.address &&
			change.tokenId === asset.tokenId,
	)?.balance
	const senderGasFees =
	...
	const receiverGasFees =
		asset.tokenEntry.address === ETHEREUM_LOGS_LOGGER_ADDRESS &&
		asset.tokenEntry.type === 'ERC20' &&
		transfer.to.address === simTx.transaction.from.address
			? simTx.gasSpent * simTx.realizedGasPrice
			: 0n
	...
	return (
		<SimpleSend
			...
			sender={{
				...
				beforeAndAfter:
					senderAfter === undefined || !('amount' in asset)
						? undefined
						: {
								before: senderAfter + asset.amount + senderGasFees,
								after: senderAfter,
							},
			}}
			receiver={{
				...
				beforeAndAfter:
					receiverAfter === undefined || !('amount' in asset)
						? undefined
						: {
								before: receiverAfter + receiverGasFees - asset.amount,
								after: receiverAfter,
							},
			}}
			renameAddressCallBack={renameAddressCallBack}
