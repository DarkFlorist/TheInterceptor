import {
	type Erc1155OperatorChange,
	Erc20ApprovalChanges,
	type Erc721OperatorChange,
...
import {
	extractEnsEvents,
	extractTokenEvents,
} from '../../../background/metadataUtils.js'
...
export function tokenEventToTokenSymbolParams(
	tokenEvent: TokenVisualizerResultWithMetadata,
) {
...
function SendOrReceiveTokensImportanceBox(
	param: SendOrReceiveTokensImportanceBoxParams,
) {
	return (
		<>
			{param.tokenVisualizerResults
				...
				.map((tokenEvent, index) => (
					<div
						key={`${tokenEvent.token.address.toString()}-${index}`}
						class="vertical-center"
					>
						<div
							class={`box token-box ${param.sending ? 'negative-box' : 'positive-box'} vertical-center`}
							style="display: inline-block"
						>
							<table class="log-table">
								<div class="log-cell">
									<p
										class="ellipsis paragraph"
										style={`color: ${param.textColor}; margin-bottom: 0px; display: inline-block`}
									>
								...
								<div class="log-cell">
									<SmallAddress
										addressBookEntry={
											param.sending ? tokenEvent.to : tokenEvent.from
										}
										renameAddressCallBack={param.renameAddressCallBack}
									/>
								</div>
...
export function CatchAllVisualizer(param: CatchAllVisualizerParams) {
	...
	const tokenSendersAndReceivers = deduplicateByFunction(
		tokenResults.flatMap((tokenResult) => [
			...(tokenResult.from.address === msgSender ||
			tokenResult.from.useAsActiveAddress
				? [tokenResult.from]
				: []),
			...(tokenResult.to.address === msgSender ||
			tokenResult.to.useAsActiveAddress
				? [tokenResult.to]
				: []),
		]),
		(element: AddressBookEntry) => addressString(element.address),
	)
	const eventTypesForEachAccount = tokenSendersAndReceivers.map(
		(currentAddress) => {
			const sendingTokenResults = tokenResults.filter(
				(x) => x.from.address === currentAddress.address,
			)
			const receivingTokenResults = tokenResults.filter(
				(x) => x.to.address === currentAddress.address,
			)
			const erc20TokenApprovalChanges: ERC20TokenApprovalChange[] =
				sendingTokenResults
					.filter(
						(x): x is TokenVisualizerErc20Event =>
							x.isApproval && x.type === 'ERC20',
					)
					.map((entry) => {
						return {
							...entry.token,
			...
			const operatorChanges: (Erc721OperatorChange | Erc1155OperatorChange)[] =
				sendingTokenResults
					.filter(
						(x): x is TokenVisualizerNFTAllApprovalEvent =>
							x.type === 'NFT All approval',
					)
					.map((entry) => {
						return {
							...entry.token,
							operator:
								'allApprovalAdded' in entry && entry.allApprovalAdded
									? entry.to
									: undefined,
						}
					})
			...
			const tokenIdApprovalChanges: Erc721TokenApprovalChange[] =
				sendingTokenResults
					.filter(
						(x): x is TokenVisualizerErc721Event =>
							'tokenId' in x && x.isApproval,
					)
					.map((entry) => {
						return {
	...
	if (
		...
		return (
			<div class="notification transaction-importance-box">
				<p class="paragraph">
					{' '}
					{param.simTx.events.length === 0
						? 'The transaction does no visible important changes to your accounts.'
						: `The transaction does no visible important changes to your accounts, HOWEVER, it produces ${param.simTx.events.length} event${param.simTx.events.length === 1 ? '' : 's'}.`}
				</p>
			</div>
		)
	}
	...
	return (
		<div class="notification transaction-importance-box">
			<div style="display: grid; grid-template-rows: max-content max-content">
				...
				<div class="log-cell" style="justify-content: left; display: grid;">
					{param.simTx.transaction.to !== undefined ? (
						...
						<p class="paragraph">
							{' '}
							{`A contract is deployed to address ${addressString(getDeployedContractAddress(param.simTx.transaction.from.address, param.simTx.transaction.nonce))}`}
						</p>
					)}
				</div>
				{/* ENS events */}
			...
			{(param.simTx.transaction.to === undefined || ensEvents.length > 0) &&
			eventTypesForEachAccount.length > 0 ? (
			...
			{eventTypesForEachAccount.map((eventsGrouped, index) => (
				<div key={addressString(eventsGrouped.currentAddress.address)}>
					...
					<div style="display: grid; grid-template-rows: max-content max-content">
						<div class="log-cell" style="justify-content: left; display: grid;">
							<SendOrReceiveTokensImportanceBox
								tokenVisualizerResults={eventsGrouped.sendingTokenResults.filter(
									(x) => !x.isApproval,
								)}
								sending={true}
								textColor={textColor}
						...
						<div class="log-cell" style="justify-content: left; display: grid;">
							<Erc20ApprovalChanges
								erc20TokenApprovalChanges={
									eventsGrouped.erc20TokenApprovalChanges
								}
								textColor={textColor}
						...
						<div class="log-cell" style="justify-content: left; display: grid;">
							<Erc721TokenIdApprovalChanges
								Erc721TokenIdApprovalChanges={
									eventsGrouped.tokenIdApprovalChanges
								}
								textColor={textColor}
								negativeColor={textColor}
								isImportant={useComputed(() => true)}
						...
						<div class="log-cell" style="justify-content: left; display: grid;">
							<SendOrReceiveTokensImportanceBox
								tokenVisualizerResults={eventsGrouped.receivingTokenResults.filter(
									(x) => !x.isApproval,
								)}
								sending={false}
								textColor={textColor}
					...
					{index + 1 !== eventTypesForEachAccount.length ? (
						<div
							class="is-divider"
