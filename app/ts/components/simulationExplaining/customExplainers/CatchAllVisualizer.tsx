	...
	switch (tokenEvent.type) {
		case 'ERC1155':
			return {
				...
				tokenIdName: tokenEvent.tokenIdName,
			}
		case 'ERC20':
			return { tokenEntry: tokenEvent.token }
		case 'ERC721':
			return { tokenEntry: tokenEvent.token, tokenId: tokenEvent.tokenId }
		case 'NFT All approval': {
			if (tokenEvent.token.type === 'ERC1155')
				return {
					tokenEntry: tokenEvent.token,
					tokenId: undefined,
					...
					<div key={`${tokenEvent.token.address.toString()}-${index}`} class="vertical-center">
						<div class={`box token-box ${param.sending ? 'negative-box' : 'positive-box'} vertical-center`} style="display: inline-block">
							<table class="log-table">
								...
								<div class="log-cell">{'amount' in tokenEvent ? <TokenAmount amount={tokenEvent.amount} {...('tokenId' in tokenEvent ? { tokenId: tokenEvent.tokenId } : {})} tokenEntry={tokenEvent.token} style={{ color: param.textColor }} fontSize="normal" /> : <></>}</div>
								<div class="log-cell">
								...
								<div class="log-cell">
									<SmallAddress addressBookEntry={param.sending ? tokenEvent.to : tokenEvent.from} renameAddressCallBack={param.renameAddressCallBack} />
								</div>
							</table>
						</div>
					</div>
...
export function CatchAllVisualizer(param: CatchAllVisualizerParams) {
	...
	const tokenSendersAndReceivers = deduplicateByFunction(
		tokenResults.flatMap((tokenResult) => [...(tokenResult.from.address === msgSender || tokenResult.from.useAsActiveAddress ? [tokenResult.from] : []), ...(tokenResult.to.address === msgSender || tokenResult.to.useAsActiveAddress ? [tokenResult.to] : [])]),
	...
	const eventTypesForEachAccount = tokenSendersAndReceivers.map((currentAddress) => {
		...
		const erc20TokenApprovalChanges: ERC20TokenApprovalChange[] = sendingTokenResults
			.filter((x): x is TokenVisualizerErc20Event => x.isApproval && x.type === 'ERC20')
			.map((entry) => {
				return {
					...entry.token,
		...
		const operatorChanges: (Erc721OperatorChange | Erc1155OperatorChange)[] = sendingTokenResults
			.filter((x): x is TokenVisualizerNFTAllApprovalEvent => x.type === 'NFT All approval')
			.map((entry) => {
				return {
					...entry.token,
		...
		const tokenIdApprovalChanges: Erc721TokenApprovalChange[] = sendingTokenResults
			.filter((x): x is TokenVisualizerErc721Event => 'tokenId' in x && x.isApproval)
			.map((entry) => {
				return {
	...
	return (
		<div class="notification transaction-importance-box">
			<div style="display: grid; grid-template-rows: max-content max-content">
				...
				{/* ENS events */}
				<div class="log-cell" style="justify-content: left; display: grid;">
					<EnsEventsExplainer ensEvents={ensEvents} textColor={textColor} renameAddressCallBack={param.renameAddressCallBack} editEnsNamedHashCallBack={param.editEnsNamedHashCallBack} rpcNetwork={param.rpcNetwork.value} />
				</div>
			</div>
			...
			{eventTypesForEachAccount.map((eventsGrouped, index) => (
				<div key={addressString(eventsGrouped.currentAddress.address)}>
					...
					<div style="display: grid; grid-template-rows: max-content max-content">
						<div class="log-cell" style="justify-content: left; display: grid;">
							<SendOrReceiveTokensImportanceBox tokenVisualizerResults={eventsGrouped.sendingTokenResults.filter((x) => !x.isApproval)} sending={true} textColor={textColor} renameAddressCallBack={param.renameAddressCallBack} />
						</div>

						{/* us approving other addresses */}
						<div class="log-cell" style="justify-content: left; display: grid;">
							<Erc20ApprovalChanges erc20TokenApprovalChanges={eventsGrouped.erc20TokenApprovalChanges} textColor={textColor} negativeColor={textColor} isImportant={useComputed(() => true)} renameAddressCallBack={param.renameAddressCallBack} />
						</div>
						<div class="log-cell" style="justify-content: left; display: grid;">
							<Erc721or1155OperatorChanges erc721or1155OperatorChanges={eventsGrouped.operatorChanges} textColor={textColor} negativeColor={textColor} isImportant={useComputed(() => true)} renameAddressCallBack={param.renameAddressCallBack} />
						</div>
						<div class="log-cell" style="justify-content: left; display: grid;">
							<Erc721TokenIdApprovalChanges Erc721TokenIdApprovalChanges={eventsGrouped.tokenIdApprovalChanges} textColor={textColor} negativeColor={textColor} isImportant={useComputed(() => true)} renameAddressCallBack={param.renameAddressCallBack} />
						</div>

						{/* receiving tokens */}
						<div class="log-cell" style="justify-content: left; display: grid;">
							<SendOrReceiveTokensImportanceBox tokenVisualizerResults={eventsGrouped.receivingTokenResults.filter((x) => !x.isApproval)} sending={false} textColor={textColor} renameAddressCallBack={param.renameAddressCallBack} />
						</div>
					</div>
