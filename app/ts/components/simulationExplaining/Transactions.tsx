...
export function Transaction(param: TransactionVisualizationParameters) {
	...
	const remove =
		removeTransactionOrSignedMessage === undefined
			? undefined
			: () => {
					return removeTransactionOrSignedMessage({
						type: 'Transaction',
	...
	return (
		<div class="card">
			...
			<div class="card-content" style="padding-bottom: 5px;">
				...
				{visualizedPersonalSignRequest === undefined ? (
					<></>
				) : (
					<div style="margin-top: 10px;">
						<SignatureCard
...
const TransactionOrMessageWithBlockTimeManipulator = ({
	...
	useEffect(() => {
		switch (blockTimeManipulation.type) {
			...
			default:
				assertNever(blockTimeManipulation)
		}
	...
	const timeSelectorOnChange = (
		...
		return sendPopupMessageToBackgroundPage({
			...
			data: { transactionOrMessageIdentifier, blockTimeManipulation },
		})
	}
	return (
		<>
...
export function ParsedInputData(params: ParsedInputDataParams) {
	...
	if (params.inputData.type === 'NonParsed') {
		return (
			<div class="textbox">
				<p class="paragraph" style="color: var(--subtitle-text-color)">
					{dataStringWith0xStart(params.inputData.input)}
				</p>
			</div>
		)
	}
