...
const hasKnownCommonTokenMetadata = (address: EthereumAddress) => {
	...
	return (
		tokenMetadata.has(normalizedAddress) ||
		erc721Metadata.has(normalizedAddress) ||
		erc1155Metadata.has(normalizedAddress)
	)
}

export async function getCodeOrError(
	ethereum: EthereumClientService,
	requestAbortController: AbortController | undefined,
	simulationState: SimulationState,
	address: EthereumAddress,
) {
	const code = await getSimulatedCode(
		ethereum,
		requestAbortController,
		toResolvedSimulationState(simulationState),
		address,
	)
...
export async function commonTokenOops(
	...
	const to = await identifyAddress(
		ethereum,
		requestAbortController,
		transferInfo.arguments.to,
	)
