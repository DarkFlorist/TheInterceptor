...
export async function chainIdMismatch(
	...
	if (transaction.chainId !== connectedChainId)
		return `This transaction is for a different chain (${getChainName(transaction.chainId)}) than what you are currently connected to (${getChainName(connectedChainId)}).`
	return
}
