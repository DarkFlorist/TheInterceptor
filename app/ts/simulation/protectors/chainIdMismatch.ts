...
export async function chainIdMismatch(transaction: EthereumUnsignedTransaction, ethereum: EthereumClientService, _simulationState: SimulationState) {
	...
	if (transaction.chainId !== connectedChainId) return `This transaction is for ${ getChainName(transaction.chainId) } while we are currently connected to ${ getChainName(connectedChainId) }.`
	return
}
