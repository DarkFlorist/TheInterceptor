...
export async function eoaApproval(transaction: EthereumUnsignedTransaction, ethereum: EthereumClientService, requestAbortController: AbortController | undefined, simulationState: SimulationState) {
	...
	if (approvalInfo.name === 'approve') {
		...
		const code = await getCodeOrError(ethereum, requestAbortController, simulationState, approvalInfo.arguments.spender)
	...
	if (approvalInfo.name === 'setApprovalForAll') {
		...
		const code = await getCodeOrError(ethereum, requestAbortController, simulationState, approvalInfo.arguments.operator)
		if (code.statusCode === 'failure') return code.message
		if (code.getCodeReturn.length > 0) return
