...
export function handleERC20TransferLog(
	eventLog: EthereumEvent,
): TokenVisualizerResult[] {
	if (eventLog.topics[1] === undefined || eventLog.topics[2] === undefined)
...
export function handleApprovalLog(
	eventLog: EthereumEvent,
): TokenVisualizerResult[] {
	if (eventLog.topics[1] === undefined || eventLog.topics[2] === undefined)
		throw new Error('unknown log')
	...
	return [
		{ ...data, ...{ amount: bytesToUnsigned(eventLog.data), type: 'ERC20' } },
	]
}

export function handleErc721ApprovalForAllLog(
	eventLog: EthereumEvent,
): TokenVisualizerResult[] {
	if (eventLog.topics[1] === undefined || eventLog.topics[2] === undefined)
		throw new Error('unknown log')
...
export function handleDepositLog(
	eventLog: EthereumEvent,
): TokenVisualizerResult[] {
...
export function handleWithdrawalLog(
	eventLog: EthereumEvent,
): TokenVisualizerResult[] {
	if (eventLog.topics[1] === undefined) throw new Error('unknown log')
	return [
...
export function handleERC1155TransferBatch(
	...
	if (eventLog.topics.length !== 4)
		throw new Error('Malformed ERC1155 TransferBatch Event')
	const parsed = decodeEventStrict(Erc1155ABI, {
		data: dataStringWith0xStart(eventLog.data),
		topics: eventLog.topics
			.map((topic) => EthereumBytes32.serialize(topic))
			.filter((topic): topic is `0x${string}` => topic !== undefined),
	})
	...
	return [...Array(_ids.length)].map((_, index) => {
		if (
			_ids[index] === undefined ||
			_values[index] === undefined ||
			eventLog.topics[1] === undefined ||
			eventLog.topics[2] === undefined ||
			eventLog.topics[3] === undefined
		)
			throw new Error('Malformed ERC1155 TransferBatch Event')
		const tokenId = _ids[index]
		const amount = _values[index]
...
export function handleERC1155TransferSingle(
	...
	if (
		...
		throw new Error('Malformed ERC1155 TransferSingle Event')
	return [
		{
			type: 'ERC1155',
			operator: eventLog.topics[1],
...
export function handleEnsAddressChanged(eventLog: ParsedEvent) {
	if (
		eventLog.args[0]?.typeValue.type !== 'fixedBytes' ||
		eventLog.args[1]?.typeValue.type !== 'unsignedInteger' ||
		eventLog.args[2]?.typeValue.type !== 'bytes'
	)
		throw new Error('Malformed ENS AddressChanged Event')
	return {
		node: EthereumBytes32.parse(
			dataStringWith0xStart(eventLog.args[0].typeValue.value),
		),
...
export function handleEnsAddrChanged(eventLog: ParsedEvent) {
	if (
		eventLog.args[0]?.typeValue.type !== 'fixedBytes' ||
		eventLog.args[1]?.typeValue.type !== 'address'
	)
		throw new Error('Malformed ENS AddrChanged Event')
	return {
		node: EthereumBytes32.parse(
			dataStringWith0xStart(eventLog.args[0].typeValue.value),
		),
...
export function handleEnsControllerNameRenewed(eventLog: ParsedEvent) {
	if (
		eventLog.args[0]?.typeValue.type !== 'string' ||
		eventLog.args[1]?.typeValue.type !== 'fixedBytes' ||
		eventLog.args[2]?.typeValue.type !== 'unsignedInteger' ||
		eventLog.args[3]?.typeValue.type !== 'unsignedInteger'
	)
		throw new Error('Malformed ENS NameRenewed Event')
	return {
...
export function handleControllerNameRegistered(eventLog: ParsedEvent) {
	if (
		eventLog.args[0]?.typeValue.type !== 'string' ||
		eventLog.args[1]?.typeValue.type !== 'fixedBytes' ||
		eventLog.args[2]?.typeValue.type !== 'address' ||
		eventLog.args[3]?.typeValue.type !== 'unsignedInteger' ||
		eventLog.args[4]?.typeValue.type !== 'unsignedInteger'
	)
		throw new Error('Malformed ENS Name Registered Event')
	return {
		name: eventLog.args[0].typeValue.value,
		labelHash: bytesToUnsigned(eventLog.args[1].typeValue.value),
...
export function handleBaseRegistrarNameRenewed(eventLog: ParsedEvent) {
	if (
		eventLog.args[0]?.typeValue.type !== 'unsignedInteger' ||
		eventLog.args[1]?.typeValue.type !== 'unsignedInteger'
	)
		throw new Error('Malformed ENS Name Renewed Event')
	return {
...
export function handleEnsTextChanged(eventLog: ParsedEvent) {
	if (
		eventLog.args[0]?.typeValue.type !== 'fixedBytes' ||
		eventLog.args[1]?.typeValue.type !== 'fixedBytes' ||
		eventLog.args[2]?.typeValue.type !== 'string'
	)
		throw new Error('Malformed ENS TextChanged Event')
	return {
		node: EthereumBytes32.parse(
			dataStringWith0xStart(eventLog.args[0].typeValue.value),
		),
...
export function handleEnsTransfer(eventLog: ParsedEvent) {
	if (
		...
		throw new Error('Malformed ENS Transfer Event')
	return {
		node: EthereumBytes32.parse(
			dataStringWith0xStart(eventLog.args[0].typeValue.value),
		),
...
export function handleEnsNewOwner(eventLog: ParsedEvent) {
	if (
		...
		throw new Error('Malformed ENS New Owner Event')
	return {
		node: EthereumBytes32.parse(
			dataStringWith0xStart(eventLog.args[0].typeValue.value),
		),
...
export function handleEnsNewResolver(eventLog: ParsedEvent) {
	if (
		...
		throw new Error('Malformed ENS New Resolver Event')
	return {
		node: EthereumBytes32.parse(
			dataStringWith0xStart(eventLog.args[0].typeValue.value),
		),
...
export function handleEnsTextChangedKeyValue(eventLog: ParsedEvent) {
	if (
		...
		eventLog.args[1]?.typeValue.type !== 'fixedBytes' ||
		eventLog.args[2]?.typeValue.type !== 'string' ||
		eventLog.args[3]?.typeValue.type !== 'string'
	)
		throw new Error('Malformed ENS Text changed Event')
	return {
		node: EthereumBytes32.parse(
			dataStringWith0xStart(eventLog.args[0].typeValue.value),
		),
...
export function handleEnsContentHashChanged(eventLog: ParsedEvent) {
	if (
		eventLog.args[0]?.typeValue.type !== 'fixedBytes' ||
		eventLog.args[1]?.typeValue.type !== 'bytes'
	)
		throw new Error('Malformed ENS Content Hash Changed Event')
	return {
		node: EthereumBytes32.parse(
			dataStringWith0xStart(eventLog.args[0].typeValue.value),
		),
...
export function handleEnsFusesSet(eventLog: ParsedEvent) {
	if (
		...
		throw new Error('Malformed ENS Fuses Set Event')
	return {
		node: EthereumBytes32.parse(
			dataStringWith0xStart(eventLog.args[0].typeValue.value),
		),
...
export function handleEnsNameUnWrapped(eventLog: ParsedEvent) {
	if (
		...
		throw new Error('Malformed ENS Name Unwrapped Event')
	return {
		node: EthereumBytes32.parse(
			dataStringWith0xStart(eventLog.args[0].typeValue.value),
		),
...
export function handleEnsNameChanged(eventLog: ParsedEvent) {
	if (
		eventLog.args[0]?.typeValue.type !== 'fixedBytes' ||
		eventLog.args[1]?.typeValue.type !== 'string'
	)
		throw new Error('Malformed ENS Name Unwrapped Event')
	return {
		node: EthereumBytes32.parse(
...
export function handleEnsReverseClaimed(eventLog: ParsedEvent) {
	if (
		eventLog.args[0]?.typeValue.type !== 'address' ||
		eventLog.args[1]?.typeValue.type !== 'fixedBytes'
	)
		throw new Error('Malformed ENS Name Unwrapped Event')
	return {
		...
		node: EthereumBytes32.parse(
			dataStringWith0xStart(eventLog.args[1].typeValue.value),
		),
	}
}
...
export function handleEnsNewTtl(eventLog: ParsedEvent) {
	if (
		...
		throw new Error('Malformed ENS New TTL Event')
	return {
		node: EthereumBytes32.parse(
			dataStringWith0xStart(eventLog.args[0].typeValue.value),
		),
...
export function handleEnsExpiryExtended(eventLog: ParsedEvent) {
	if (
		...
		throw new Error('Malformed ENS ExpiryExtended Event')
	return {
		node: EthereumBytes32.parse(
			dataStringWith0xStart(eventLog.args[0].typeValue.value),
		),
