...
export function handleERC1155TransferBatch(eventLog: EthereumEvent): TokenVisualizerResult[] {
	...
	return [...Array(_ids.length)].map((_, index) => {
		...
		if (tokenId === undefined || amount === undefined) throw new Error('Malformed ERC1155 TransferBatch Event')
		return {
...
export function handleControllerNameRegistered(eventLog: ParsedEvent) {
	if (eventLog.args[0]?.typeValue.type !== 'string' || eventLog.args[1]?.typeValue.type !== 'fixedBytes' || eventLog.args[2]?.typeValue.type !== 'address' || eventLog.args[3]?.typeValue.type !== 'unsignedInteger' || eventLog.args[4]?.typeValue.type !== 'unsignedInteger')
...
export function handleNameWrapped(eventLog: ParsedEvent) {
	if (eventLog.args[0]?.typeValue.type !== 'fixedBytes' || eventLog.args[1]?.typeValue.type !== 'bytes' || eventLog.args[2]?.typeValue.type !== 'address' || eventLog.args[3]?.typeValue.type !== 'unsignedInteger' || eventLog.args[4]?.typeValue.type !== 'unsignedInteger')
