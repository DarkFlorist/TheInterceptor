...
function getBlockExplorer(
	...
	const primaryRpc = rpcEntries.find(
		(rpc) => rpc.chainId === chainId && rpc.primary,
	)
	if (primaryRpc !== undefined && primaryRpc.blockExplorer !== undefined)
		return primaryRpc.blockExplorer
	return getDefaultBlockExplorer()
}

export const isBlockExplorerAvailableForChain = (
	chainId: ChainIdWithUniversal,
	rpcEntries: RpcEntries,
) => getBlockExplorer(chainId, rpcEntries) !== undefined

async function fetchAbi(
	contractAddress: EthereumAddress,
	maybeExplorer: BlockExplorer | undefined,
	chainId: bigint,
): Promise<Result<EtherscanSourceCodeResult>> {
	const normalizedAddressString = addressString(contractAddress)
	let bestResult: Result<EtherscanSourceCodeResult> = {
		success: false,
		...
		if (maybeExplorer !== undefined) {
			try {
				const result = await fetch(
					`${maybeExplorer.apiUrl}?chainId=${chainId.toString()}&module=contract&action=getsourcecode&address=${normalizedAddressString}&apiKey=${maybeExplorer.apiKey}`,
				)
				bestResult = EtherscanSourceCodeResult.safeParse(await result.json())
				if (bestResult.success) return bestResult
			} catch (error: unknown) {
				console.error(
					`Failed to retrieve ABI for ${normalizedAddressString} from ${maybeExplorer.apiUrl}`,
				)
				console.error(error)
			}
		}
		const result = await fetch(
			`https://repo.sourcify.dev/contracts/full_match/${chainId.toString(10)}/${normalizedAddressString}/metadata.json`,
		)
		if (result.status === 404)
			return { success: false, message: 'No source available' } as const
...
export async function fetchAbiFromBlockExplorer(
	contractAddress: EthereumAddress,
	chainId: ChainIdWithUniversal,
) {
	const api = getBlockExplorer(chainId, await getRpcList())

	const parsedSourceCode = await fetchAbi(
		contractAddress,
		api,
		chainId === 'AllChains' ? 1n : chainId,
	)

	// Extract ABI from getSourceCode request if not proxy, otherwise attempt to fetch ABI of implementation
	if (
		parsedSourceCode.success === false ||
		parsedSourceCode.value.status !== 'success'
	)
		return {
			success: false as const,
			error: 'Could not get ABI for the contract.',
		}

	if (
		api !== undefined &&
		parsedSourceCode.value.result[0].Proxy === 'yes' &&
		parsedSourceCode.value.result[0].Implementation !== ''
	) {
		const implReq = await fetchJson(
			`${api.apiUrl}?chainId=${chainId.toString()}&module=contract&action=getabi&address=${addressString(parsedSourceCode.value.result[0].Implementation)}&apiKey=${api.apiKey}`,
		)
		if (!implReq.success) return implReq
		const implResult = EtherscanGetABIResult.safeParse(implReq.result)

		const sourceCodeResult = await fetchJson(
			`${api.apiUrl}?chainId=${chainId.toString()}&module=contract&action=getsourcecode&address=${addressString(parsedSourceCode.value.result[0].Implementation)}&apiKey=${api.apiKey}`,
		)
		if (!sourceCodeResult.success) return sourceCodeResult
		const implementationName = EtherscanSourceCodeResult.safeParse(
			sourceCodeResult.result,
		)

		if (!implResult.success || !implementationName.success)
			return {
				success: false as const,
				error: 'Failed to parse Etherscan results.',
			}
		if (!isValidAbi(implResult.value.result))
			return {
				success: false as const,
				error: 'Etherscan returned an invalid ABI',
			}
		return {
