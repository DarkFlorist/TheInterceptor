...
const isEmptySimulationInput = (simulationStateInput: SimulationStateInput | SimulationStateInputMinimalData) => simulationStateInput.length === 0 || (simulationStateInput.length === 1 && simulationStateInput[0]?.transactions.length === 0 && simulationStateInput[0]?.signedMessages.length === 0)
...
const createPreparedSimulationExecutionContext = async (
	...
	const resolvedSimulationInput = 'kind' in simulationStateInput ? (simulationStateInput.kind === 'passthrough' ? undefined : simulationStateInput.value) : simulationStateInput
...
const resolveSimulationBlockTag = (baseBlockNumber: bigint, executionBlockCount: number, blockTag: EthereumBlockTag = 'latest') => {
	if (blockTag === 'latest' || blockTag === 'pending') return baseBlockNumber + BigInt(executionBlockCount)
	return blockTag
}

const canQueryNodeDirectlyFromInput = (baseBlockNumber: bigint, executionBlockCount: number, blockTag: EthereumBlockTag = 'latest') => {
...
const getExecutionBlockIndexForTag = (baseBlockNumber: bigint, executionBlockCount: number, blockTag: EthereumBlockTag = 'latest') => {
...
const getExecutionBlocksUpToTag = (context: PreparedSimulationExecutionContext, blockTag: EthereumBlockTag = 'latest') => {
	...
	const includedBlockCount = min(BigInt(context.executionBlocks.length), resolvedBlockTag - context.parentBlock.number)
...
const simulateBlockCallOnTopOfSimulationInput = async (
	...
	return await simulateBlockCallWithPreparedInputContext(ethereumClientService, requestAbortController, await createPreparedSimulationExecutionContext(ethereumClientService, requestAbortController, simulationStateInput), transaction, extraOverrides, simulateWithZeroBaseFee)
}
...
export const simulateEstimateGas = async (
	...
	if (simulationState.kind === 'passthrough')
		return {
			gas: await ethereumClientService.estimateGas(data, requestAbortController),
		}
	const currentState = simulationState.value
	if (currentState.success === false) throw new JsonRpcResponseError(currentState.jsonRpcError)
		...
		if (error instanceof JsonRpcResponseError) {
			...
			return {
				error: {
					...
					data: safeParsedData.success ? dataStringWith0xStart(safeParsedData.value) : '0x',
				},
			}
		}
		throw error
...
export const inspectSimulationInput = async (ethereumClientService: EthereumClientService, requestAbortController: AbortController | undefined, simulationStateInput: SimulationStateInput): Promise<SimulationInputInspection> => {
...
const createExecutionSimulationBlocks = (simulationStateInput: SimulationStateInput, groupedEthSimulateV1CallResult: GroupedEthSimulateV1Result): SuccessfulExecutionSimulationState['simulatedBlocks'] => {
...
export const createExecutionSimulationState = async (ethereumClientService: EthereumClientService, requestAbortController: AbortController | undefined, simulationStateInput: SimulationStateInput): Promise<ExecutionSimulationState> => {
		...
		executionSimulationState.simulatedBlocks.map((block) => ({
			inputBlock: {
				...
				transactions: block.simulatedTransactions.map((transaction) => ({
					signedTransaction: transaction.preSimulationTransaction.signedTransaction,
				})),
...
const createPreparedSimulatedExecutionBlocks = async (ethereumClientService: EthereumClientService, requestAbortController: AbortController | undefined, simulationState: SuccessfulExecutionSimulationState): Promise<readonly PreparedSimulatedExecutionBlock[]> => {
	const context = await createPreparedSimulationExecutionContext(ethereumClientService, requestAbortController, { kind: 'simulated', value: simulationState.simulationStateInput }, simulationState.blockNumber)
		...
		for (const [transactionIndex, callResult] of block.calls.entries()) {
			...
			const fixTransaction = async () => {
				...
				return modifyObject(preSimulationTransaction, {
					signedTransaction: modifyObject(preSimulationTransaction.signedTransaction, { nonce: newNonce }),
				})
			}
	...
	return preSimulationTransactions.map((transaction) => {
		...
		return modifyObject(transaction, {
			signedTransaction: modifyObject(transaction.signedTransaction, {
				maxFeePerGas: parentBaseFeePerGas * 2n + transaction.signedTransaction.maxPriorityFeePerGas,
			}),
		})
	})
}
...
const canQueryNodeDirectly = async (simulationState: SimulationState, blockTag: EthereumBlockTag = 'latest') => {
	if (blockTag === 'finalized' || (simulationState.success && simulationState.simulatedBlocks.length === 0) || (simulationState.success && typeof blockTag === 'bigint' && blockTag <= simulationState.blockNumber)) {
		return true
	}
	return false
}
...
const getSimulatedMockBlockFromPreparedContext = async (context: PreparedSimulationExecutionContext, blockIndex: number) => {
...
const getSimulatedTransactionCountFromPreparedInputContext = async (ethereumClientService: EthereumClientService, requestAbortController: AbortController | undefined, context: PreparedSimulationExecutionContext | undefined, address: bigint, blockTag: EthereumBlockTag = 'latest') => {
...
export const getSimulatedTransactionCountFromInput = async (ethereumClientService: EthereumClientService, requestAbortController: AbortController | undefined, simulationStateInput: ResolvedSimulationInput, address: bigint, blockTag: EthereumBlockTag = 'latest') => {
...
export const getSimulatedBlockNumberFromInput = async (ethereumClientService: EthereumClientService, requestAbortController: AbortController | undefined, simulationStateInput: ResolvedSimulationInput) => {
	const context = await createPreparedSimulationExecutionContext(ethereumClientService, requestAbortController, simulationStateInput)
...
export const getSimulatedTransactionByHashFromInput = async (ethereumClientService: EthereumClientService, requestAbortController: AbortController | undefined, simulationStateInput: ResolvedSimulationInput, hash: bigint): Promise<EthereumSignedTransactionWithBlockData | null> => {
	const context = await createPreparedSimulationExecutionContext(ethereumClientService, requestAbortController, simulationStateInput)
	...
	for (const executionBlock of context.executionBlocks) {
		for (const [transactionIndex, transaction] of executionBlock.inputBlock.transactions.entries()) {
			...
			const gasPrice = 'gasPrice' in transaction.signedTransaction ? transaction.signedTransaction.gasPrice : calculateRealizedEffectiveGasPrice(transaction.signedTransaction, executionBlock.baseFeePerGas || 0n)
			return {
...
const simulatedCallWithPreparedInputContext = async (
	...
	if (blockTag === 'finalized') {
		try {
			return {
				result: await ethereumClientService.call(params, 'finalized', requestAbortController),
			}
		} catch (error: unknown) {
			if (error instanceof JsonRpcResponseError) {
				const safeParsedData = EthereumData.safeParse(error.data)
...
export const simulatedCallFromInput = async (
	...
	return await simulatedCallWithPreparedInputContext(ethereumClientService, requestAbortController, await createPreparedSimulationExecutionContext(ethereumClientService, requestAbortController, simulationStateInput), params, blockTag)
}

export const getSimulatedCodeFromInput = async (ethereumClientService: EthereumClientService, requestAbortController: AbortController | undefined, simulationStateInput: ResolvedSimulationInput, address: bigint, blockTag: EthereumBlockTag = 'latest') => {
	const context = await createPreparedSimulationExecutionContext(ethereumClientService, requestAbortController, simulationStateInput)
	if (context === undefined || canQueryNodeDirectlyFromInput(context.parentBlock.number, context.executionBlocks.length, blockTag)) {
		return {
			statusCode: 'success',
...
export const getSimulatedBalanceFromInput = async (ethereumClientService: EthereumClientService, requestAbortController: AbortController | undefined, simulationStateInput: ResolvedSimulationInput, address: bigint, blockTag: EthereumBlockTag = 'latest'): Promise<bigint> => {
	const context = await createPreparedSimulationExecutionContext(ethereumClientService, requestAbortController, simulationStateInput)
	if (context === undefined || canQueryNodeDirectlyFromInput(context.parentBlock.number, context.executionBlocks.length, blockTag)) {
		return await ethereumClientService.getBalance(address, blockTag, requestAbortController)
	}
...
const getLogsOfPreparedSimulatedExecutionBlock = (executionBlock: PreparedSimulatedExecutionBlock, logFilter: EthGetLogsRequest): EthGetLogsResponse => {
	const events: EthGetLogsResponse =
		executionBlock.simulatedTransactions.reduce((acc, sim, transactionIndex) => {
	...
	return events.filter((x) => (logFilter.address === undefined || x.address === logFilter.address || (Array.isArray(logFilter.address) && logFilter.address.includes(x.address))) && includeLogByTopic(x.topics, logFilter.topics))
}
...
export const getSimulatedLogs = async (ethereumClientService: EthereumClientService, requestAbortController: AbortController | undefined, simulationState: ResolvedExecutionSimulationState, logFilter: EthGetLogsRequest): Promise<EthGetLogsResponse> => {
	...
	const nodeLogs =
		fromBlockNum <= currentState.blockNumber
			...
			: []
	const simulatedLogs = executionBlocks.filter((block) => block.blockNumber >= fromBlockNum && block.blockNumber <= toBlockNum).flatMap((block) => getLogsOfPreparedSimulatedExecutionBlock(block, logFilter))
...
export const getSimulatedBlockNumber = async (ethereumClientService: EthereumClientService, requestAbortController: AbortController | undefined, simulationState: ResolvedExecutionSimulationState) => {
	if (simulationState.kind === 'simulated')
		return await getSimulatedBlockNumberFromInput(ethereumClientService, requestAbortController, {
			kind: 'simulated',
...
export const getTokenBalancesAfterForTransaction = async (ethereumClientService: EthereumClientService, requestAbortController: AbortController | undefined, simulationStateInput: SimulationStateInput, callResult: EthSimulateV1CallResult, sender: EthereumAddress): Promise<TokenBalancesAfter> => {
...
export const getTokenBalancesAfter = async (ethereumClientService: EthereumClientService, requestAbortController: AbortController | undefined, ethSimulateV1Result: GroupedEthSimulateV1Result, simulationStateInput: SimulationStateInput): Promise<TokenBalancesBlocksAfter> => {
	const tokenBalancesAfterArray = await promiseAllMapAbortSafe(Array.from(simulationStateInput.entries()), async ([inputBlockIndex, inputBlock]) => {
		...
		return await promiseAllMapAbortSafe(Array.from(inputBlock.transactions.entries()), async ([inputTransactionIndex, inputTransaction]) => {
			...
			return getTokenBalancesAfterForTransaction(ethereumClientService, requestAbortController, inputStateJustAfterTransaction, simulateResultTransaction, sender)
		})
	})
					...
					reward: [
						rewardPercentiles.map((percentile) => {
							...
							const getDataPoint = (tx: EthereumBlockHeaderTransaction) => {
								if ('maxPriorityFeePerGas' in tx && 'maxFeePerGas' in tx && 'gas' in tx)
									return {
										dataPoint: min(tx.maxPriorityFeePerGas, tx.maxFeePerGas - (newestBlockBaseFeePerGas ?? 0n)),
										weight: tx.gas,
									}
