...
type GroupedEthSimulateV1BlockResult = {
	...
	timestamp: bigint
	blockHeader: EthSimulateV1BlockHeader | undefined
	...
	for (const [blockDelta, block] of simulationState.simulatedBlocks.entries()) {
		for (const [transactionIndex, simulatedTransaction] of block.simulatedTransactions.entries()) {
			...
			if (hash === simulatedTransaction.preSimulationTransaction.signedTransaction.hash) {
				const simulatedBlockNumber = getSimulationBlockNumber(simulationState, blockDelta)
				return {
...
async function getSimulatedMockBlock(ethereumClientService: EthereumClientService, requestAbortController: AbortController | undefined, simulationState: SimulationState, blockDelta: number) {
	...
	return {
		...
		totalDifficulty: blockHeaderTemplate?.totalDifficulty ?? ((parentBlock.totalDifficulty ?? 0n) + parentBlock.difficulty), // The difficulty increases about the same amount as previously
	...
	const events: EthGetLogsResponse = block.simulatedTransactions.reduce((acc, sim, transactionIndex) => {
		...
		return [
			...
			...sim.ethSimulateV1CallResult.logs.map((event, logIndex) => ({
				...
				blockHash: getHashOfSimulatedBlock(simulationState, blockDelta),
				blockNumber: getSimulationBlockNumber(simulationState, blockDelta),
...
export const getSimulatedLogs = async (ethereumClientService: EthereumClientService, requestAbortController: AbortController | undefined, simulationState: SimulationState | undefined, logFilter: EthGetLogsRequest): Promise<EthGetLogsResponse> => {
	...
	if (simulationState && (toBlock === 'latest' || toBlock > simulationState.blockNumber)) {
		const logParamsToNode = fromBlock === 'latest' || (typeof fromBlock === 'bigint' && fromBlock > simulationState.blockNumber)
			? undefined
			: { ...logFilter, toBlock: simulationState.blockNumber }

		const latestSimulatedBlockNumber = simulationState.blockNumber + BigInt(simulationState.simulatedBlocks.length)
		const fromBlockNum = fromBlock === 'latest' ? latestSimulatedBlockNumber : fromBlock
		const toBlockNum = toBlock === 'latest' ? latestSimulatedBlockNumber : toBlock
		const blockDeltas = simulationState.simulatedBlocks.map((_block, blockDelta) => {
			const thisBlockNum = getSimulationBlockNumber(simulationState, blockDelta)
		...
		const logsFromNode = logParamsToNode === undefined ? [] : await ethereumClientService.getLogs(logParamsToNode, requestAbortController)
		return [...logsFromNode, ...blockDeltas.flatMap((blockDelta) => getLogsOfSimulatedBlock(simulationState, blockDelta, logFilter))]
	}
...
export const getSimulatedBlockNumber = async (ethereumClientService: EthereumClientService, requestAbortController: AbortController | undefined, simulationState: SimulationState | undefined) => {
	if (simulationState === undefined) return await ethereumClientService.getBlockNumber(requestAbortController)
	if (simulationState.success === false) throw new JsonRpcResponseError(simulationState.jsonRpcError)
	return simulationState.blockNumber + BigInt(simulationState.simulatedBlocks.length)
}
...
// use the node-provided simulated block hash when available, and fall back to a deterministic synthetic hash for grouped logical blocks
const getSyntheticHashOfSimulatedBlock = (simulationState: SimulationState, blockDelta: number) => BigInt(simulationState.simulationConductedTimestamp.getTime() * 100000 + blockDelta)
const getHashOfSimulatedBlock = (simulationState: SimulationState, blockDelta: number) => getSimulatedBlockHeaderTemplate(simulationState, blockDelta)?.hash ?? getSyntheticHashOfSimulatedBlock(simulationState, blockDelta)
