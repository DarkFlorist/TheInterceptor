import {
	type EthereumClientService,
	getNextBlockTimeStampOverride,
} from './EthereumClientService.js'
import type { PreparedEthSimulateV1Input } from './EthereumClientService.js'
import {
	type EthereumUnsignedTransaction,
	type EthereumSignedTransactionWithBlockData,
	type EthereumBlockTag,
	EthereumAddress,
	type EthereumBlockHeader,
	type EthereumBlockHeaderWithTransactionHashes,
	EthereumData,
	EthereumQuantity,
	type EthereumBytes32,
	type EthereumSendableSignedTransaction,
	type EthereumBlockHeaderTransaction,
...
import {
	CANNOT_SIMULATE_OFF_LEGACY_BLOCK,
	ERROR_INTERCEPTOR_GAS_ESTIMATION_FAILED,
	ETHEREUM_LOGS_LOGGER_ADDRESS,
	ETHEREUM_EIP1559_BASEFEECHANGEDENOMINATOR,
	ETHEREUM_EIP1559_ELASTICITY_MULTIPLIER,
	MOCK_ADDRESS,
	MULTICALL3,
	Multicall3ABI,
...
import type {
	...
	SimulationStateInputMinimalData,
	SimulationStateInputMinimalDataBlock,
	BlockTimeManipulationDeltaUnit,
	ExecutionSimulatedTransaction,
	ExecutionSimulationState,
	ResolvedExecutionSimulationState,
...
import type {
	PersonalSignParams,
	SignMessageParams,
} from '../../types/jsonRpc-signing-types.js'
import type {
...
import {
	...
	encodeFunctionCall,
	type AbiLike,
} from '../../utils/abiRuntime.js'
import { Erc20ABI, Erc1155ABI } from '../../utils/abi.js'

type SuccessfulExecutionSimulationState = Extract<
	ExecutionSimulationState,
	{ success: true }
>
...
export const getWebsiteCreatedEthereumUnsignedTransactions = (
	...
	return simulatedTransactions.map((simulatedTransaction) => ({
		...
		originalRequestParameters:
			simulatedTransaction.preSimulationTransaction.originalRequestParameters,
		transactionIdentifier:
			simulatedTransaction.preSimulationTransaction.transactionIdentifier,
		success: true as const,
	}))
}

const transactionQueueTotalGasLimit = (
	simulatedTransactions: readonly SimulatedTransaction[],
) => {
	return simulatedTransactions.reduce(
		(a, b) => a + b.preSimulationTransaction.signedTransaction.gas,
		0n,
	)
}

const transactionQueueTotalGasUsed = (
	simulatedTransactions: readonly SimulatedTransaction[],
) => {
	return simulatedTransactions.reduce(
		(totalGasUsed, simulatedTransaction) =>
			totalGasUsed + simulatedTransaction.ethSimulateV1CallResult.gasUsed,
		0n,
	)
}

const transactionQueueTotalGasLimitFromInput = (
	block: SimulationStateInputMinimalDataBlock | undefined,
) => {
	if (block === undefined) return 0n
	return block.transactions.reduce(
		(totalGasUsed, transaction) =>
			totalGasUsed + transaction.signedTransaction.gas,
		0n,
	)
}

const isEmptySimulationInput = (
	simulationStateInput: SimulationStateInput | SimulationStateInputMinimalData,
) =>
	simulationStateInput.length === 0 ||
	(simulationStateInput.length === 1 &&
		simulationStateInput[0]?.transactions.length === 0 &&
		simulationStateInput[0]?.signedMessages.length === 0)

const getSimulationBlockNumber = (
	simulationState: SimulationState,
	blockDelta: number,
) => simulationState.blockNumber + BigInt(blockDelta) + 1n

const getHashOfSimulatedBlockFromInput = (
	simulationStateInput: SimulationStateInput | SimulationStateInputMinimalData,
	blockDelta: number,
) => {
	return BigInt(
		keccak256(
			stringToBytes(
				`${getSimulationInputHash(simulationStateInput)}:${blockDelta}`,
			),
		),
	)
}
...
const createPreparedSimulationExecutionContext = async (
	...
	const resolvedSimulationInput =
		'kind' in simulationStateInput
	...
	const prepared = await ethereumClientService.prepareEthSimulateV1Input(
		resolvedSimulationInput,
		parentBlock.number,
		requestAbortController,
	)
	...
	const executionBlocks = prepared.rpcBlocks.map((inputBlock, blockIndex) => {
		...
		if (blockOverride === undefined)
			throw new Error('missing block override for prepared simulation block')
		if (blockOverride.time === undefined)
			throw new Error('missing timestamp for prepared simulation block')
		const gasUsed = transactionQueueTotalGasLimitFromInput(inputBlock)
		const baseFeePerGas =
			previousBaseFeePerGas === undefined
				? undefined
				: getNextBaseFeePerGas(
						previousGasUsed,
						parentBlock.gasLimit,
						previousBaseFeePerGas,
					)
		const blockHash = getHashOfSimulatedBlockFromInput(
			resolvedSimulationInput,
			blockIndex,
		)
...
const resolveSimulationBlockTag = (
	...
	if (blockTag === 'latest' || blockTag === 'pending')
		return baseBlockNumber + BigInt(executionBlockCount)
	return blockTag
}
...
const getExecutionBlockIndexForTag = (
	...
	const resolvedBlockTag = resolveSimulationBlockTag(
		baseBlockNumber,
		executionBlockCount,
		blockTag,
	)
	...
	if (resolvedBlockTag > baseBlockNumber + BigInt(executionBlockCount))
		return undefined
...
const getExecutionBlocksUpToTag = (
	...
	const resolvedBlockTag = resolveSimulationBlockTag(
		context.parentBlock.number,
		context.executionBlocks.length,
		blockTag,
	)
	if (
		typeof resolvedBlockTag !== 'bigint' ||
...
export const simulationGasLeft = (
	simulationStateBlock: SimulationStateBlock | undefined,
	blockHeader: EthereumBlockHeader,
) => {
	if (blockHeader === null) throw new Error('The latest block is null')
	if (simulationStateBlock === undefined)
		return (blockHeader.gasLimit * 1023n) / 1024n
	return max(
		(blockHeader.gasLimit * 1023n) / 1024n -
			transactionQueueTotalGasLimit(simulationStateBlock.simulatedTransactions),
		0n,
	)
}

export function getInputFieldFromDataOrInput(
	request: { input?: Uint8Array } | { data?: Uint8Array } | {},
) {
...
export const getSimulatedTransactionCount = async (
	ethereumClientService: EthereumClientService,
	requestAbortController: AbortController | undefined,
	...
	if (blockTag === 'finalized' || simulationState.kind === 'passthrough')
		return await ethereumClientService.getTransactionCount(
			address,
			blockTag,
			requestAbortController,
		)
	...
	if (currentState.success === false)
		throw new JsonRpcResponseError(currentState.jsonRpcError)
	const blockNumToUseForSim =
		blockTag === 'latest' || blockTag === 'pending'
			? currentState.blockNumber + BigInt(currentState.simulatedBlocks.length)
			: blockTag
	const blockNumToUseForChain =
		blockTag === 'latest' || blockTag === 'pending'
			? blockTag
			: min(
					blockTag,
					await ethereumClientService.getBlockNumber(requestAbortController),
				)
	let addedTransactions = 0n
	if (
		blockTag === 'latest' ||
		blockTag === 'pending' ||
		blockTag > currentState.blockNumber
	) {
		...
		for (const block of currentState.simulatedBlocks) {
			...
			for (const signed of block.simulatedTransactions) {
				if (signed.preSimulationTransaction.signedTransaction.from === address)
					addedTransactions += 1n
			}
			index++
		}
	}
	return (
		(await ethereumClientService.getTransactionCount(
			address,
			blockNumToUseForChain,
			requestAbortController,
		)) + addedTransactions
	)
}

type Simulated1559BlockCall = Pick<
	IUnsignedTransaction1559,
	| 'from'
	| 'chainId'
	| 'nonce'
	| 'maxFeePerGas'
	...
	| 'input'
> &
	Partial<Pick<IUnsignedTransaction1559, 'gasLimit' | 'accessList'>>

const createSimulated1559BlockCall = (
	transaction: Simulated1559BlockCall,
): SimulateBlockCalls['calls'][number] => {
	const {
		gasLimit,
		maxFeePerGas,
		accessList,
...
const simulateBlockCallWithPreparedInputContext = async (
	...
	const parentBlock =
		context?.parentBlock ??
		(await ethereumClientService.getBlock(requestAbortController))
	if (parentBlock === null) throw new Error('The latest block is null')
	const previousBlockOverride =
		context?.prepared.blockOverrides[context.prepared.blockOverrides.length - 1]
	const previousBlockTime = previousBlockOverride?.time ?? parentBlock.timestamp
	const baseFeePerGas =
		parentBlock.baseFeePerGas === undefined
			? 15_000_000n
			: parentBlock.baseFeePerGas
	const blockStateCalls: readonly SimulateBlockCalls[] = [
		...(context?.prepared.request.params[0].blockStateCalls ?? []),
		{
			...
			blockOverrides: {
				...
				time: getNextBlockTimeStampOverride(
					previousBlockTime,
	...
	const simulationResult = await ethereumClientService.ethSimulateV1(
		blockStateCalls,
		parentBlock.number,
		requestAbortController,
	)
...
const simulateBlockCallOnTopOfSimulationInput = async (
	...
	return await simulateBlockCallWithPreparedInputContext(
		...
		requestAbortController,
		await createPreparedSimulationExecutionContext(
			ethereumClientService,
			requestAbortController,
			simulationStateInput,
		),
...
export const simulateEstimateGas = async (
	ethereumClientService: EthereumClientService,
	requestAbortController: AbortController | undefined,
	simulationState: ResolvedSimulationState,
	...
	if (simulationState.kind === 'passthrough')
		return {
			gas: await ethereumClientService.estimateGas(
				data,
				requestAbortController,
			),
		}
	...
	const transactionCount = getSimulatedTransactionCount(
		ethereumClientService,
		requestAbortController,
		simulationState,
		sendAddress,
	...
	const simulatedBlockIncrement =
		blockDelta === undefined
			? currentState.simulatedBlocks.length || 0
			: blockDelta
	const maxGas = simulationGasLeft(
		currentState.simulatedBlocks[simulatedBlockIncrement] || undefined,
		block,
	)
	...
	try {
		const lastResult = await simulateBlockCallOnTopOfSimulationInput(
			ethereumClientService,
			requestAbortController,
			currentState.simulationStateInput,
			estimateGasTransaction,
			{},
			true,
		)
		...
		if (error instanceof JsonRpcResponseError) {
			...
			return {
				error: {
					...
					data: safeParsedData.success
						? dataStringWith0xStart(safeParsedData.value)
						: '0x',
				},
			}
		}
...
export const calculateRealizedEffectiveGasPrice = (
	transaction: EthereumUnsignedTransaction,
	blocksBaseFeePerGas: bigint,
) => {
	if ('gasPrice' in transaction) return transaction.gasPrice
	return min(
		blocksBaseFeePerGas + transaction.maxPriorityFeePerGas,
		transaction.maxFeePerGas,
	)
}

export const mockSignTransaction = (
	transaction: EthereumUnsignedTransaction,
): EthereumSendableSignedTransaction => {
	const unsignedTransaction =
		EthereumUnsignedTransactionToUnsignedTransaction(transaction)
	if (unsignedTransaction.type === 'legacy') {
		const signatureParams = { r: 0n, s: 0n, v: 0n }
		const hash = EthereumQuantity.parse(
			keccak256(
				serializeSignedTransactionToBytes({
					...unsignedTransaction,
					...signatureParams,
	...
	if (unsignedTransaction.type === '7702') {
		...
		const authorizationList = unsignedTransaction.authorizationList.map(
			(element) => ({ ...element, ...signatureParams }),
		)
		const hash = EthereumQuantity.parse(
			keccak256(
				serializeSignedTransactionToBytes({
	...
	if (transaction.type === 'legacy' || transaction.type === '7702')
...
export const getAddressToMakeRich = async () => {
	...
	return (await getMakeCurrentAddressRich())
		? settings.activeSimulationAddress
		: undefined
}

export const getBlockTimeManipulationSeconds = (
	...
	switch (deltaUnit) {
		case 'Seconds':
			return deltaToAdd
		case 'Minutes':
			return deltaToAdd * 60n
		case 'Hours':
			return deltaToAdd * 60n * 60n
		case 'Days':
			return deltaToAdd * 60n * 60n * 24n
		case 'Weeks':
			return deltaToAdd * 60n * 60n * 24n * 7n
		case 'Months':
			return deltaToAdd * 60n * 60n * 24n * 30n
		case 'Years':
			return deltaToAdd * 60n * 60n * 24n * 365n
		default:
			assertNever(deltaUnit)
	}
}

export const groupEthSimulateV1ResultByInputBlocks = (
	prepared: PreparedEthSimulateV1Input,
	result: EthSimulateV1Result,
): GroupedEthSimulateV1Result => {
	let rpcBlockIndex = 0
	const groupedResults = prepared.inputBlocks.map((preparedInputBlock) => {
		const resultBlocks = result.slice(
			rpcBlockIndex,
			rpcBlockIndex + preparedInputBlock.rpcBlockCount,
		)
		if (resultBlocks.length !== preparedInputBlock.rpcBlockCount)
			throw new Error(
				'multicall length does not match in createSimulationState',
			)
		rpcBlockIndex += preparedInputBlock.rpcBlockCount
		const firstResultBlock = resultBlocks[0]
		if (firstResultBlock === undefined)
			throw new Error('grouped result block was undefined')
		const blockHeader =
			preparedInputBlock.rpcBlockCount === 1
				? (() => {
	...
	if (rpcBlockIndex !== result.length)
		throw new Error('multicall length does not match in createSimulationState')
...
export const inspectSimulationInput = async (
	...
	if (isEmptySimulationInput(simulationStateInput)) {
		...
		return {
			...
			groupedEthSimulateV1CallResult: simulationStateInput.map((inputBlock) => {
				previousTimestamp = getNextBlockTimeStampOverride(
					previousTimestamp,
					inputBlock.blockTimeManipulation || DEFAULT_BLOCK_MANIPULATION,
				)
				return {
					inputBlock,
	...
	try {
		const { prepared, result: ethSimulateV1CallResult } =
			await ethereumClientService.simulatePrepared(
				simulationStateInput,
				parentBlock.number,
				requestAbortController,
			)
		const groupedEthSimulateV1CallResult =
			groupEthSimulateV1ResultByInputBlocks(prepared, ethSimulateV1CallResult)
		return {
			success: true,
			base: {
				...base,
		...
		if (error instanceof JsonRpcResponseError)
			return { success: false, base, jsonRpcError: error.serialize() }
		throw error
	}
}

const getExecutionSimulationStateBlockBase = (
	callResult: GroupedEthSimulateV1BlockResult,
) => ({
	...
	blockTimeManipulation:
		callResult.inputBlock.blockTimeManipulation || DEFAULT_BLOCK_MANIPULATION,
...
const createExecutionSimulationBlocks = (
	...
	return groupedEthSimulateV1CallResult.map((callResult, blockIndex) => ({
		simulatedTransactions: callResult.calls.map(
			(singleResult, transactionIndex) => {
				const signedTx =
					simulationStateInput[blockIndex]?.transactions[transactionIndex]
				if (signedTx === undefined) throw Error('invalid transaction index')
				return {
					ethSimulateV1CallResult: singleResult,
					realizedGasPrice: calculateRealizedEffectiveGasPrice(
						signedTx.signedTransaction,
						callResult.baseFeePerGas,
					),
...
export const createExecutionSimulationState = async (
	...
	const simulationInspection = await inspectSimulationInput(
		ethereumClientService,
		requestAbortController,
		simulationStateInput,
	)
	if (simulationInspection.success === false)
		return {
			...simulationInspection.base,
			success: false,
			jsonRpcError: simulationInspection.jsonRpcError,
		}
	const { base, groupedEthSimulateV1CallResult } = simulationInspection
	return {
		success: true,
		simulatedBlocks: createExecutionSimulationBlocks(
			simulationStateInput,
			groupedEthSimulateV1CallResult,
		),
		...base,
...
export const createSimulationState = async (
	ethereumClientService: EthereumClientService,
	requestAbortController: AbortController | undefined,
	simulationStateInput: SimulationStateInput,
	...
	if (executionSimulationState.success === false)
		return executionSimulationState
	if (isEmptySimulationInput(simulationStateInput)) {
		return {
			...executionSimulationState,
			simulatedBlocks: executionSimulationState.simulatedBlocks.map(
				(block) => ({ ...block, simulatedTransactions: [] }),
			),
		}
	}
	const tokenBalancesAfter = await getTokenBalancesAfter(
		...
		executionSimulationState.simulatedBlocks.map((block) => ({
			inputBlock: {
				...
				transactions: block.simulatedTransactions.map((transaction) => ({
					signedTransaction:
			...
			calls: block.simulatedTransactions.map(
				(transaction) => transaction.ethSimulateV1CallResult,
			),
		})),
		simulationStateInput,
	)
	return {
		...
		simulatedBlocks: executionSimulationState.simulatedBlocks.map(
			(block, blockIndex) => ({
				...block,
				simulatedTransactions: block.simulatedTransactions.map(
					(simulatedTransaction, transactionIndex) => {
						const tokenBalancesAfterForIndex =
							tokenBalancesAfter.blocks[blockIndex]?.transactions[
								transactionIndex
							]?.tokenBalancesAfter
						if (tokenBalancesAfterForIndex === undefined)
							throw Error('invalid tokenBalancesAfterForIndex index')
						return {
							...simulatedTransaction,
...
const createPreparedSimulatedExecutionBlocks = async (
	...
	return context.prepared.inputBlocks.flatMap(
		(preparedInputBlock, inputBlockIndex) => {
			const simulatedBlock = simulationState.simulatedBlocks[inputBlockIndex]
			if (simulatedBlock === undefined)
				throw new Error(
					'missing simulated block while splitting execution blocks',
				)
			let transactionOffset = 0
			const executionBlocksForInputBlock = Array.from(
				{ length: preparedInputBlock.rpcBlockCount },
				() => {
					...
					if (executionBlock === undefined)
						throw new Error(
							'missing prepared execution block while splitting simulation results',
						)
					if (rpcBlock === undefined)
						throw new Error(
							'missing prepared rpc block while splitting simulation results',
						)
					const transactionCount = rpcBlock.transactions.length
					const simulatedTransactions =
						simulatedBlock.simulatedTransactions.slice(
							transactionOffset,
							transactionOffset + transactionCount,
						)
					if (simulatedTransactions.length !== transactionCount)
						throw new Error(
							'prepared rpc block transaction count did not match simulated transactions',
						)
					transactionOffset += transactionCount
					return {
						...executionBlock,
			...
			if (transactionOffset !== simulatedBlock.simulatedTransactions.length)
				throw new Error(
					'simulated transactions remained after splitting execution blocks',
				)
...
export const getPreSimulated = (
	simulatedTransactions: readonly SimulatedTransaction[],
) =>
	simulatedTransactions.map(
		(transaction) => transaction.preSimulationTransaction,
	)

export const appendTransactionsToInput = (
	simulationStateInput: SimulationStateInput,
	...
	const mergeStateSets = (
		oldOverrides: StateOverrides,
		newOverrides: StateOverrides,
	) => {
...
export const appendTransactionToInputAndSimulate = async (
	ethereumClientService: EthereumClientService,
	requestAbortController: AbortController | undefined,
	oldSimulatedInput: SimulationStateInput,
	transactions: PreSimulationTransaction[],
	blockDelta: number | undefined = undefined,
	stateOverrides: StateOverrides = {},
): Promise<SimulationState> => {
	const simulationStateInput = appendTransactionsToInput(
		oldSimulatedInput,
		transactions,
		blockDelta,
		stateOverrides,
	)
	return await createSimulationState(
		ethereumClientService,
		requestAbortController,
		simulationStateInput,
	)
...
export const getNonceFixedSimulationStateInput = async (
	ethereumClientService: EthereumClientService,
	requestAbortController: AbortController | undefined,
	simulationStateInput: SimulationStateInput,
) => {
	const isFixableNonceError = (
	...
	if (simulationInspection.success === false)
		return { nonceFixed: false, simulationStateInput }
	...
	const areThereNonceIssues = () => {
		const nonceFixable = blocks.find((block, blockIndex) =>
			block.calls.find((callResult, transactionIndex) => {
				const transaction =
					simulationStateInput[blockIndex]?.transactions[transactionIndex]
	...
	for (const [blockIndex, block] of blocks.entries()) {
		...
		for (const [transactionIndex, callResult] of block.calls.entries()) {
			const preSimulationTransaction =
				simulationStateInput[blockIndex]?.transactions[transactionIndex]
			if (preSimulationTransaction === undefined)
				throw new Error('missing transaction when checking for nonces')
			const fromString = addressString(
				preSimulationTransaction.signedTransaction.from,
			)
			const fixTransaction = async () => {
				if (!isFixableNonceError(preSimulationTransaction, callResult))
					return preSimulationTransaction
				...
				const newNonce =
					prevNonce === undefined
						...
						: prevNonce + 1n
				return modifyObject(preSimulationTransaction, {
					signedTransaction: modifyObject(
						preSimulationTransaction.signedTransaction,
						{ nonce: newNonce },
					),
				})
			}
			...
			knownPreviousNonce.set(
				fromString,
				fixedTransaction.signedTransaction.nonce,
			)
		}
		const oldBlock = simulationStateInput[blockIndex]
		if (oldBlock === undefined)
			throw new Error('missing block when checking for nonces')
		simulationInputBlocks.push({
			...oldBlock,
...
export const getBaseFeeAdjustedTransactions = (
	...
	return preSimulationTransactions.map((transaction) => {
		...
		return modifyObject(transaction, {
			signedTransaction: modifyObject(transaction.signedTransaction, {
				maxFeePerGas:
					parentBaseFeePerGas * 2n +
...
const canQueryNodeDirectly = async (
	simulationState: SimulationState,
	blockTag: EthereumBlockTag = 'latest',
) => {
	if (
		...
		(simulationState.success &&
			...
			blockTag <= simulationState.blockNumber)
	) {
		return true
	}
	return false
...
export const getDeployedContractAddress = (
	...
	return BigInt(
		`0x${keccak256(rlpEncode([stripLeadingZeros(bigintToUint8Array(from, 20)), stripLeadingZeros(bigintToUint8Array(nonce, 32))])).slice(26)}`,
	)
}

export const getSimulatedTransactionReceipt = async (
	ethereumClientService: EthereumClientService,
	requestAbortController: AbortController | undefined,
	simulationState: ResolvedExecutionSimulationState,
	hash: bigint,
): Promise<EthTransactionReceiptResponse> => {
	if (simulationState.kind === 'passthrough') {
		return await ethereumClientService.getTransactionReceipt(
			hash,
			requestAbortController,
		)
	}
	...
	if (currentState.success === false)
		throw new JsonRpcResponseError(currentState.jsonRpcError)
	const getTransactionSpecificFields = (
		...
		switch (signedTransaction.type) {
			...
			case '1559':
			case '2930':
				return { type: signedTransaction.type }
			case '4844':
				return {
					type: signedTransaction.type,
					blobGasUsed:
						GAS_PER_BLOB * BigInt(signedTransaction.blobVersionedHashes.length),
			...
			default:
				assertNever(signedTransaction)
		}
	}

	const executionBlocks = await createPreparedSimulatedExecutionBlocks(
		ethereumClientService,
		requestAbortController,
		currentState,
	)
	for (const executionBlock of executionBlocks) {
		...
		for (const [
			transactionIndex,
			simulatedTransaction,
		] of executionBlock.simulatedTransactions.entries()) {
			cumulativeGasUsed += simulatedTransaction.ethSimulateV1CallResult.gasUsed
			if (
				hash !==
				simulatedTransaction.preSimulationTransaction.signedTransaction.hash
			) {
				currentLogIndex +=
					simulatedTransaction.ethSimulateV1CallResult.status === 'success'
						? simulatedTransaction.ethSimulateV1CallResult.logs.length
						: 0
				continue
			}
			return {
				...getTransactionSpecificFields(
					simulatedTransaction.preSimulationTransaction.signedTransaction,
				),
				...
				contractAddress:
					simulatedTransaction.preSimulationTransaction.signedTransaction.to !==
					null
						? null
				...
				effectiveGasPrice: calculateRealizedEffectiveGasPrice(
					simulatedTransaction.preSimulationTransaction.signedTransaction,
					executionBlock.baseFeePerGas || 0n,
				),
				from: simulatedTransaction.preSimulationTransaction.signedTransaction
					.from,
				to: simulatedTransaction.preSimulationTransaction.signedTransaction.to,
				logs:
					simulatedTransaction.ethSimulateV1CallResult.status === 'success'
						? simulatedTransaction.ethSimulateV1CallResult.logs.map(
								(x, logIndex) => ({
									removed: false,
									blockHash: executionBlock.blockHash,
									address: x.address,
...
export const getSimulatedBalance = async (
	...
	if (
		simulationState.kind === 'passthrough' ||
		(await canQueryNodeDirectly(simulationState.value, blockTag))
	)
		return await ethereumClientService.getBalance(
			address,
			blockTag,
			requestAbortController,
		)
	...
	if (currentState.success === false)
		throw new JsonRpcResponseError(currentState.jsonRpcError)
	for (const block of currentState.simulatedBlocks) {
		for (const [overrideAddress, override] of Object.entries(
			block.stateOverrides,
		)) {
			if (override?.balance !== undefined)
				ethBalances.set(
					EthereumQuantity.parse(overrideAddress),
					override.balance,
				)
		}
		for (const transaction of block.simulatedTransactions) {
			...
			for (const b of transaction.tokenBalancesAfter) {
				if (b.balance === undefined || b.token !== ETHEREUM_LOGS_LOGGER_ADDRESS)
					continue
...
export const getSimulatedCode = async (
	ethereumClientService: EthereumClientService,
	requestAbortController: AbortController | undefined,
	simulationState: ResolvedSimulationState,
	address: bigint,
	...
	if (
		...
		return {
			statusCode: 'success',
			getCodeReturn: await ethereumClientService.getCode(
				address,
				blockTag,
				requestAbortController,
			),
	...
	const input = stringToUint8Array(
		encodeFunctionCall(getCodeAbi, 'at', [addressString(address)]),
	)
	...
	try {
		const result = await simulatedCall(
			ethereumClientService,
			undefined,
			simulationState,
			getCodeTransaction,
		...
		if (error instanceof JsonRpcResponseError)
			return { statusCode: 'failure' } as const
		throw error
	}
}
...
const getNextBaseFeePerGas = (
	...
	if (parentGasUsed > parentGasTarget)
		return (
			...
			max(
				1n,
				(parentBaseFeePerGas * (parentGasUsed - parentGasTarget)) /
					parentGasTarget /
					ETHEREUM_EIP1559_BASEFEECHANGEDENOMINATOR,
			)
		)
	return max(
		0n,
		parentBaseFeePerGas -
			(parentBaseFeePerGas * (parentGasTarget - parentGasUsed)) /
				parentGasTarget /
				ETHEREUM_EIP1559_BASEFEECHANGEDENOMINATOR,
	)
}
...
const getSimulatedMockBlockFromPreparedContext = async (
	...
	return {
		...
		transactions: block.inputBlock.transactions.map(
...
const getSimulatedTransactionCountFromPreparedInputContext = async (
	...
	for (const block of getExecutionBlocksUpToTag(context, blockTag)) {
		for (const transaction of block.inputBlock.transactions) {
			if (transaction.signedTransaction.from === address)
				addedTransactions += 1n
		}
	}
	return (
...
export const getSimulatedTransactionCountFromInput = async (
	...
	return await getSimulatedTransactionCountFromPreparedInputContext(
		ethereumClientService,
		requestAbortController,
		context,
		address,
...
export async function getSimulatedBlockFromInput(
	ethereumClientService: EthereumClientService,
	requestAbortController: AbortController | undefined,
	simulationStateInput: ResolvedSimulationInput,
...
export async function getSimulatedBlockFromInput(
	...
	blockTag: EthereumBlockTag,
	fullObjects: false,
): Promise<EthereumBlockHeaderWithTransactionHashes>
export async function getSimulatedBlockFromInput(
	ethereumClientService: EthereumClientService,
	requestAbortController: AbortController | undefined,
	simulationStateInput: ResolvedSimulationInput,
	...
	if (
		...
		return await ethereumClientService.getBlock(
			requestAbortController,
			blockTag,
			fullObjects,
		)
	}
	const blockIndex = getExecutionBlockIndexForTag(
		context.parentBlock.number,
		context.executionBlocks.length,
		blockTag,
	)
	if (blockIndex === undefined) return null
	const block = await getSimulatedMockBlockFromPreparedContext(
		context,
		blockIndex,
	)
...
export const getSimulatedBlockByHashFromInput = async (
	...
	if (context === undefined)
		return await ethereumClientService.getBlockByHash(
			blockHash,
			requestAbortController,
			fullObjects,
		)
	const blockIndex = context.executionBlocks.findIndex(
...
export const getSimulatedTransactionByHashFromInput = async (
	...
	if (context === undefined)
		return await ethereumClientService.getTransactionByHash(
			hash,
			requestAbortController,
		)
...
const simulatedCallWithPreparedInputContext = async (
	...
	params: Pick<
		IUnsignedTransaction1559,
		'to' | 'maxFeePerGas' | 'maxPriorityFeePerGas' | 'input' | 'value'
	> &
		Partial<Pick<IUnsignedTransaction1559, 'from' | 'gasLimit'>>,
	blockTag: EthereumBlockTag = 'latest',
) => {
	if (blockTag === 'finalized') {
		try {
			return {
				result: await ethereumClientService.call(
					params,
					'finalized',
					requestAbortController,
				),
	...
	const transaction = {
		...
		type: '1559',
		from,
		nonce: await getSimulatedTransactionCountFromPreparedInputContext(
			ethereumClientService,
			requestAbortController,
			context,
			from,
	...
	try {
		const callResult = await simulateBlockCallWithPreparedInputContext(
			ethereumClientService,
			requestAbortController,
			context,
			transaction,
		...
		if (callResult === undefined)
			throw new Error('failed to get last call in eth simulate')
...
export const getSimulatedCodeFromInput = async (
	...
	try {
		const result = await simulatedCallWithPreparedInputContext(
			ethereumClientService,
			requestAbortController,
			context,
			getCodeTransaction,
...
export const getSimulatedBalanceFromInput = async (
	...
	if (executionBlocksToApply.length === 0)
		return await ethereumClientService.getBalance(
			address,
			blockTag,
			requestAbortController,
...
export const simulateEstimateGasFromInput = async (
	...
	const sendAddress = data.from !== undefined ? data.from : MOCK_ADDRESS
	const transactionCount = getSimulatedTransactionCountFromPreparedInputContext(
		ethereumClientService,
		requestAbortController,
		context,
		sendAddress,
	)
	const latestSimulatedBlock =
		context === undefined
			? undefined
			: await getSimulatedMockBlockFromPreparedContext(
					context,
					context.executionBlocks.length - 1,
				)
	const fallbackBlock =
		latestSimulatedBlock ??
		context?.parentBlock ??
		(await ethereumClientService.getBlock(requestAbortController))
	if (fallbackBlock === null) throw new Error('The latest block is null')
	const simulatedBlockIncrement =
		blockDelta === undefined
			? (context?.executionBlocks.length ?? 0)
			: blockDelta
	const maxGas = max(
		(fallbackBlock.gasLimit * 1023n) / 1024n -
			transactionQueueTotalGasLimitFromInput(
				context?.prepared.rpcBlocks[simulatedBlockIncrement],
			),
		0n,
	)
	const estimateGasTransaction = {
	...
	try {
		const lastResult = await simulateBlockCallWithPreparedInputContext(
			ethereumClientService,
			requestAbortController,
			context,
			estimateGasTransaction,
...
async function getSimulatedMockBlock(
	ethereumClientService: EthereumClientService,
	requestAbortController: AbortController | undefined,
	simulationState: SimulationState,
	blockDelta: number,
	...
	if (parentBlock === null) throw new Error('The latest block is null')
	if (parentBlock.baseFeePerGas === undefined)
		throw new Error(CANNOT_SIMULATE_OFF_LEGACY_BLOCK)
	if (simulationState.success === false)
		throw new JsonRpcResponseError(simulationState.jsonRpcError)
	const simulatedBlock = simulationState.simulatedBlocks[blockDelta]
	const blockHeaderTemplate = getSimulatedBlockHeaderTemplate(
		simulationState,
		blockDelta,
	)
	const parentHash =
		blockDelta === 0
			? (blockHeaderTemplate?.parentHash ?? parentBlock.hash)
			: getHashOfSimulatedBlock(simulationState, blockDelta - 1)
	return {
		...
		gasUsed:
			blockHeaderTemplate?.gasUsed ??
			transactionQueueTotalGasUsed(simulatedBlock?.simulatedTransactions || []),
		...
		timestamp:
			simulatedBlock?.blockTimestamp ||
			bigintSecondsToDate(
				dateToBigintSeconds(simulationState.blockTimestamp) +
					getBlockTimeManipulationSeconds(
						DEFAULT_BLOCK_MANIPULATION.deltaToAdd,
						DEFAULT_BLOCK_MANIPULATION.deltaUnit,
					),
			),
		size: blockHeaderTemplate?.size ?? parentBlock.size,
		totalDifficulty:
			blockHeaderTemplate?.totalDifficulty ??
			(parentBlock.totalDifficulty ?? 0n) + parentBlock.difficulty,
		uncles: blockHeaderTemplate?.uncles ?? [],
		baseFeePerGas:
			simulatedBlock?.blockBaseFeePerGas ??
			getNextBaseFeePerGas(
				parentBlock.gasUsed,
				parentBlock.gasLimit,
				parentBlock.baseFeePerGas,
		...
		transactionsRoot:
			blockHeaderTemplate?.transactionsRoot ?? parentBlock.transactionsRoot,
		transactions:
			simulatedBlock?.simulatedTransactions.map(
				(simulatedTransaction) =>
					simulatedTransaction.preSimulationTransaction.signedTransaction,
			) || [],
		withdrawals: blockHeaderTemplate?.withdrawals ?? [],
		...(blockHeaderTemplate?.blobGasUsed !== undefined
			? { blobGasUsed: blockHeaderTemplate.blobGasUsed }
			: {}),
		...(blockHeaderTemplate?.excessBlobGas !== undefined
			? { excessBlobGas: blockHeaderTemplate.excessBlobGas }
			: {}),
		...(blockHeaderTemplate?.parentBeaconBlockRoot !== undefined
			? { parentBeaconBlockRoot: blockHeaderTemplate.parentBeaconBlockRoot }
			: {}),
		...(blockHeaderTemplate?.withdrawalsRoot !== undefined
			? { withdrawalsRoot: blockHeaderTemplate.withdrawalsRoot }
			: {}),
	} as const
}

export async function getSimulatedBlockByHash(
	ethereumClientService: EthereumClientService,
	requestAbortController: AbortController | undefined,
	simulationState: ResolvedSimulationState,
	blockHash: EthereumBytes32,
	...
	if (simulationState.kind === 'simulated') {
		...
		if (currentState.success === false)
			throw new JsonRpcResponseError(currentState.jsonRpcError)
		const blockDelta = currentState.simulatedBlocks.findIndex(
			(_block, index) =>
				getHashOfSimulatedBlock(currentState, index) === blockHash,
		)
		if (blockDelta < 0)
			return await ethereumClientService.getBlockByHash(
				blockHash,
		...
		const block = await getSimulatedMockBlock(
			ethereumClientService,
			requestAbortController,
			currentState,
			blockDelta,
...
export async function getSimulatedBlock(
	ethereumClientService: EthereumClientService,
	requestAbortController: AbortController | undefined,
	simulationState: ResolvedSimulationState,
	blockTag?: EthereumBlockTag,
...
export async function getSimulatedBlock(
	...
	const blockDelta =
		blockTag === 'latest' || blockTag === 'pending'
			? currentState.simulatedBlocks.length - 1
			: Math.max(Number(blockTag - currentState.blockNumber), 0) - 1
	if (blockDelta < 0)
		return await ethereumClientService.getBlock(
			requestAbortController,
			blockTag,
...
const getLogsOfPreparedSimulatedExecutionBlock = (
	executionBlock: PreparedSimulatedExecutionBlock,
	logFilter: EthGetLogsRequest,
): EthGetLogsResponse => {
	const events: EthGetLogsResponse =
		executionBlock.simulatedTransactions.reduce(
			(acc, sim, transactionIndex) => {
				...
				return [
					...
					...sim.ethSimulateV1CallResult.logs.map((event, logIndex) => ({
						...
						transactionIndex: BigInt(transactionIndex),
						transactionHash:
							sim.preSimulationTransaction.signedTransaction.hash,
						blockHash: executionBlock.blockHash,
						blockNumber: executionBlock.blockNumber,
	...
	const includeLogByTopic = (
		logsTopics: readonly bigint[],
		filtersTopics: readonly (bigint | readonly bigint[] | null)[] | undefined,
	) => {
		...
		for (const [index, filter] of filtersTopics.entries()) {
			...
			if (Array.isArray(filter) && !filter.includes(logsTopics[index]))
				return false
		}
		return true
	}
		...
		(x) =>
			(logFilter.address === undefined ||
				...
				(Array.isArray(logFilter.address) &&
					logFilter.address.includes(x.address))) &&
			includeLogByTopic(x.topics, logFilter.topics),
	)
}

const resolveLogsBlockTag = (
	blockTag: EthereumBlockTag,
...
export const getSimulatedLogs = async (
	ethereumClientService: EthereumClientService,
	requestAbortController: AbortController | undefined,
	simulationState: ResolvedExecutionSimulationState,
	logFilter: EthGetLogsRequest,
): Promise<EthGetLogsResponse> => {
	if (simulationState.kind === 'passthrough')
		return await ethereumClientService.getLogs(
			logFilter,
			requestAbortController,
		)
	const currentState = simulationState.value
	...
	const toBlock =
		'toBlock' in logFilter && logFilter.toBlock !== undefined
			? logFilter.toBlock
			: 'latest'
	const fromBlock =
		'fromBlock' in logFilter && logFilter.fromBlock !== undefined
			? logFilter.fromBlock
			: 'latest'
	if (toBlock === 'pending' || fromBlock === 'pending')
		return await ethereumClientService.getLogs(
			logFilter,
			requestAbortController,
		)
	if (
		...
		(fromBlock !== 'latest' && toBlock !== 'latest' && fromBlock > toBlock)
	)
		throw new Error(
			`From block '${fromBlock}' is later than to block '${toBlock}' `,
		)

	if (toBlock === 'finalized' || fromBlock === 'finalized')
		return await ethereumClientService.getLogs(
			logFilter,
			requestAbortController,
		)
	const simulatedHead =
		currentState.blockNumber + BigInt(executionBlocks.length)
	if ('blockHash' in logFilter) {
		const executionBlock = executionBlocks.find(
			(block) => logFilter.blockHash === block.blockHash,
		)
		if (executionBlock !== undefined)
			return getLogsOfPreparedSimulatedExecutionBlock(executionBlock, logFilter)
		return await ethereumClientService.getLogs(
			logFilter,
			requestAbortController,
		)
	...
	if (typeof fromBlockNum !== 'bigint' || typeof toBlockNum !== 'bigint')
		return await ethereumClientService.getLogs(
			logFilter,
			requestAbortController,
		)
	...
	const nodeLogs =
		fromBlockNum <= currentState.blockNumber
	...
	const simulatedLogs = executionBlocks
		.filter(
			(block) =>
				block.blockNumber >= fromBlockNum && block.blockNumber <= toBlockNum,
		)
		.flatMap((block) =>
			getLogsOfPreparedSimulatedExecutionBlock(block, logFilter),
		)
	if (nodeLogs.length > 0 || simulatedLogs.length > 0)
		return [...nodeLogs, ...simulatedLogs]
	if (toBlockNum > currentState.blockNumber) return []
	return await ethereumClientService.getLogs(logFilter, requestAbortController)
}
export const getSimulatedBlockNumber = async (
	ethereumClientService: EthereumClientService,
	requestAbortController: AbortController | undefined,
	simulationState: ResolvedExecutionSimulationState,
) => {
	if (simulationState.kind === 'simulated')
		return await getSimulatedBlockNumberFromInput(
			ethereumClientService,
			requestAbortController,
			{ kind: 'simulated', value: simulationState.value.simulationStateInput },
		)
	return await ethereumClientService.getBlockNumber(requestAbortController)
}

function getSignedTransactionV(
	transaction: EthereumSendableSignedTransaction,
): bigint {
	if ('v' in transaction && transaction.v !== undefined) return transaction.v
	if (!('yParity' in transaction))
		throw new Error('Signed transaction is missing both v and yParity.')
	if (transaction.type === 'legacy')
		return (
			(transaction.yParity === 'even' ? 0n : 1n) +
			35n +
			2n * transaction.chainId
		)
	return transaction.yParity === 'even' ? 0n : 1n
}

export const getSimulatedTransactionByHash = async (
	ethereumClientService: EthereumClientService,
	requestAbortController: AbortController | undefined,
	simulationState: ResolvedSimulationState,
	hash: bigint,
	...
	for (const [blockDelta, block] of currentState.simulatedBlocks.entries()) {
		for (const [
			transactionIndex,
			simulatedTransaction,
		] of block.simulatedTransactions.entries()) {
			if (
				hash ===
				simulatedTransaction.preSimulationTransaction.signedTransaction.hash
			) {
				const v = getSignedTransactionV(
					simulatedTransaction.preSimulationTransaction.signedTransaction,
				)
				const additionalParams = {
					...
					transactionIndex: BigInt(transactionIndex),
					data: simulatedTransaction.preSimulationTransaction.signedTransaction
						.input,
					v,
				}
				if (
...
export const simulatedCall = async (
	ethereumClientService: EthereumClientService,
	requestAbortController: AbortController | undefined,
	simulationState: ResolvedSimulationState,
	params: Pick<
	...
	const transaction = {
		...
		type: '1559',
		from,
		nonce: await getSimulatedTransactionCount(
			ethereumClientService,
			requestAbortController,
			simulationState,
			from,
	...
	try {
		const callResult = await simulateBlockCallOnTopOfSimulationInput(
			ethereumClientService,
			requestAbortController,
			simulationState.kind === 'passthrough'
				? undefined
				: simulationState.value.simulationStateInput,
			transaction,
		)
		if (callResult === undefined)
			throw new Error('failed to get last call in eth simulate')
...
const getHashOfSimulatedBlock = (
	simulationState: SimulationState,
	blockDelta: number,
) =>
	getSimulatedBlockHeaderTemplate(simulationState, blockDelta)?.hash ??
	getHashOfSimulatedBlockFromInput(
		simulationState.simulationStateInput,
		blockDelta,
	)

const getSimulatedBlockHeaderTemplate = (
	simulationState: SimulationState,
	blockDelta: number,
) => {
	if (simulationState.success === false)
...
export const getMessageHashForPersonalSign = (params: PersonalSignParams) =>
	hashMessage({ raw: stringToUint8Array(params.params[0]) })

export const simulatePersonalSign = async (
	params: SignMessageParams,
	signingAddress: EthereumAddress,
) => {
	const account = privateKeyToAccount(
		bytes32String(
			signingAddress === ADDRESS_FOR_PRIVATE_KEY_ONE
				? MOCK_PUBLIC_PRIVATE_KEY
				: MOCK_SIMULATION_PRIVATE_KEY,
		),
	)
	switch (params.method) {
		case 'eth_signTypedData':
			throw new Error('No support for eth_signTypedData')
		...
		case 'personal_sign':
			return {
				signature: await account.signMessage({
					message: { raw: stringToUint8Array(params.params[0]) },
				}),
...
const getSimulatedTokenBalances = async (
	ethereumClientService: EthereumClientService,
	requestAbortController: AbortController | undefined,
	simulationStateInput: SimulationStateInputMinimalData,
	...
	const deduplicatedBalanceQueries = deduplicateByFunction(
		balanceQueries,
		(query: BalanceQuery) =>
			`${query.type}-${query.token}-${query.owner}${query.type === 'ERC1155' ? `${query.tokenId}` : ''}`,
	)
	const tokenAndEthBalancesInputData = stringToUint8Array(
		encodeFunctionCall(Multicall3ABI, 'aggregate3', [
			deduplicatedBalanceQueries.map((balanceQuery) => {
				if (
					balanceQuery.token === ETHEREUM_LOGS_LOGGER_ADDRESS &&
					balanceQuery.type === 'ERC20'
				) {
					return {
						target: addressString(MULTICALL3),
						allowFailure: true,
						callData: encodeFunctionCall(Multicall3ABI, 'getEthBalance', [
							addressString(balanceQuery.owner),
						]),
					}
				}
				if (balanceQuery.type === 'ERC20') {
					return {
						...
						allowFailure: true,
						callData: encodeFunctionCall(Erc20ABI, 'balanceOf', [
							addressString(balanceQuery.owner),
						]),
					}
				}
				return {
					target: addressString(balanceQuery.token),
					allowFailure: true,
					callData: encodeFunctionCall(Erc1155ABI, 'balanceOf', [
						addressString(balanceQuery.owner),
						balanceQuery.tokenId,
					]),
				}
			}),
		]),
	...
	const aggregate3CallResult = await simulateBlockCallOnTopOfSimulationInput(
		ethereumClientService,
		requestAbortController,
		simulationStateInput,
		callTransaction,
	...
	if (
		aggregate3CallResult === undefined ||
		aggregate3CallResult.status === 'failure'
	)
		throw Error('Failed aggregate3')
	const multicallReturnData = decodeFunctionOutput(
		Multicall3ABI,
		'aggregate3',
		dataStringWith0xStart(aggregate3CallResult.returnData),
	)
	if (multicallReturnData.length !== deduplicatedBalanceQueries.length)
		throw Error('Got wrong number of balances back')
	return multicallReturnData.map((singleCallResult, callIndex) => {
		...
		if (balanceQuery === undefined)
			throw new Error('aggregate3 failed to get eth balance')
		return {
			token: balanceQuery.token,
			tokenId: 'tokenId' in balanceQuery ? balanceQuery.tokenId : undefined,
			owner: balanceQuery.owner,
			balance: singleCallResult.success
				? EthereumQuantity.parse(singleCallResult.returnData)
				: undefined,
		}
	})
}
...
export const parseEventIfPossible = (abi: AbiLike, log: EthereumEvent) => {
	try {
		return decodeEventLoose(abi, {
			topics: log.topics.map((x) => bytes32String(x)),
			data: dataStringWith0xStart(log.data),
		})
	} catch (error) {
		return undefined
	}
...
export const parseTransactionInputIfPossible = (
	abi: AbiLike,
	data: EthereumData,
...
const getAddressesInteractedWithErc20s = (
	events: readonly EthereumEvent[],
): { token: bigint; owner: bigint; tokenId: undefined; type: 'ERC20' }[] => {
	const tokenOwners: {
		token: bigint
		owner: bigint
		tokenId: undefined
...
const getAddressesAndTokensIdsInteractedWithErc1155s = (
	events: readonly EthereumEvent[],
): { token: bigint; owner: bigint; tokenId: bigint; type: 'ERC1155' }[] => {
	const tokenOwners: {
		token: bigint
		owner: bigint
		tokenId: bigint
...
export const getTokenBalancesAfterForTransaction = async (
	...
	const erc20sAddresses = [
		{
			token: ETHEREUM_LOGS_LOGGER_ADDRESS,
			owner: sender,
			tokenId: undefined,
			type: 'ERC20' as const,
		}, // add original sender for eth always, as there's always gas payment
		...getAddressesInteractedWithErc20s(events),
	]
	const erc1155AddressIds =
		getAddressesAndTokensIdsInteractedWithErc1155s(events)
	return getSimulatedTokenBalances(
		ethereumClientService,
		requestAbortController,
		simulationStateInput,
		[...erc20sAddresses, ...erc1155AddressIds],
	)
}

export const sliceSimulationStateInput = (
	simulationStateInput: SimulationStateInput,
	blockIndex: number,
...
export const sliceSimulationState = (
	simulationState: SimulationState,
	blockIndex: number,
	transactionIndex: number,
): SimulationState => {
	...
	if (simulationState.success === false)
		return modifyObject(simulationState, {
			simulationStateInput: sliceSimulationStateInput(
				simulationState.simulationStateInput,
				blockIndex,
				transactionIndex,
			),
	...
	return modifyObject(simulationState, {
		...
		simulatedBlocks: [
			...
			{
				...slicedResultBlock,
				simulatedTransactions: slicedResultBlock.simulatedTransactions.slice(
					0,
					transactionIndex,
				),
			},
...
export const getTokenBalancesAfter = async (
	...
	const tokenBalancesAfterArray = await promiseAllMapAbortSafe(
		Array.from(simulationStateInput.entries()),
		async ([inputBlockIndex, inputBlock]) => {
			const groupedResultBlock = ethSimulateV1Result[inputBlockIndex]
			if (groupedResultBlock === undefined)
				throw new Error('singleResult block was undefined')
			return await promiseAllMapAbortSafe(
				Array.from(inputBlock.transactions.entries()),
				async ([inputTransactionIndex, inputTransaction]) => {
					const simulateResultTransaction =
						groupedResultBlock.calls[inputTransactionIndex]
					if (simulateResultTransaction === undefined)
						throw new Error('singleResult transaction was undefined')
					const sender = inputTransaction.signedTransaction.from
					const inputStateJustAfterTransaction = sliceSimulationStateInput(
						simulationStateInput,
						inputBlockIndex,
						inputTransactionIndex + 1,
					)
...
export const getSimulatedFeeHistory = async (
	ethereumClientService: EthereumClientService,
	requestAbortController: AbortController | undefined,
	request: FeeHistory,
): Promise<EthGetFeeHistoryResponse> => {
	...
	const currentRealBlockNumber = await ethereumClientService.getBlockNumber(
		requestAbortController,
	)
	const clampedBlockTag =
		typeof blockTag === 'bigint' && blockTag > currentRealBlockNumber
			? currentRealBlockNumber
			: blockTag
	const newestBlock = await ethereumClientService.getBlock(
		requestAbortController,
		clampedBlockTag,
		true,
	)
	...
	if (newestBlockBaseFeePerGas === undefined)
		throw new Error(`base fee per gas is missing for the block (it's too old)`)
	return {
		baseFeePerGas: [
			newestBlockBaseFeePerGas,
			getNextBaseFeePerGas(
				newestBlock.gasUsed,
				newestBlock.gasLimit,
				newestBlockBaseFeePerGas,
			),
		],
		...
		...(rewardPercentiles === undefined
			...
			: {
					reward: [
						rewardPercentiles.map((percentile) => {
								...
								if (
									...
									return {
										dataPoint: min(
											...
											tx.maxFeePerGas - (newestBlockBaseFeePerGas ?? 0n),
										),
										weight: tx.gas,
									}
								if ('gasPrice' in tx && 'gas' in tx)
									return {
										dataPoint: tx.gasPrice - (newestBlockBaseFeePerGas ?? 0n),
										weight: tx.gas,
									}
								return { dataPoint: 0n, weight: 0n }
							}

							const effectivePriorityAndGasWeights =
								newestBlock.transactions.map((tx) => getDataPoint(tx))
							...
							const zeroOutNegativeValues = effectivePriorityAndGasWeights.map(
								(point) =>
									modifyObject(point, { dataPoint: max(0n, point.dataPoint) }),
							)
							return calculateWeightedPercentile(
