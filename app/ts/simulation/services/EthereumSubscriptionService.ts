...
import {
	getEthereumSubscriptionsAndFilters,
	updateEthereumSubscriptionsAndFilters,
} from '../../background/storageVariables.js'
import type {
...
export async function removeEthereumSubscription(
	socket: WebsiteSocket,
	subscriptionOrFilterId: string,
) {
	const changes = await updateEthereumSubscriptionsAndFilters(
		(subscriptions: EthereumSubscriptionsAndFilters) => {
			return subscriptions.filter((subscription) => {
				const sameSubscription =
					subscription.subscriptionOrFilterId === subscriptionOrFilterId
				const sameSocket =
					subscription.subscriptionCreatorSocket.tabId === socket.tabId &&
					subscription.subscriptionCreatorSocket.connectionName ===
						socket.connectionName
				return !(sameSubscription && sameSocket) // only allow the same tab and connection to remove the subscription
			})
		},
	)
	if (
		changes.oldSubscriptions.find(
			(sub) =>
...
export async function sendSubscriptionMessagesForNewBlock(
	...
	const ethereumSubscriptionsAndFilters =
		await getEthereumSubscriptionsAndFilters()
	let liveBlockPromise:
		| Promise<Awaited<ReturnType<EthereumClientService['getBlock']>>>
		| undefined
	let simulationStatePromise:
		| Promise<ResolvedExecutionSimulationState>
		| undefined
	let simulatedBlocksPromise:
		| Promise<
				readonly NonNullable<
					Awaited<ReturnType<typeof getSimulatedBlockFromInput>>
				>[]
		  >
		| undefined
	const getLiveBlock = async () => {
		if (liveBlockPromise === undefined)
			liveBlockPromise = ethereumClientService.getBlock(
				undefined,
				blockNumber,
				false,
			)
	...
	const getCachedSimulationState = async () => {
		if (simulationStatePromise === undefined)
			simulationStatePromise = getSimulationState(ethereumClientService)
		return await simulationStatePromise
	}
	const getSimulatedBlocks = async () => {
		if (simulatedBlocksPromise !== undefined)
		...
		simulatedBlocksPromise = (async () => {
			...
			if (
				...
				simulationState.value.success !== true
			)
				return []
			...
			const simulatedHead = await getSimulatedBlockNumberFromInput(
			...
			for (
				let simulatedBlockNumber = blockNumber + 1n;
				simulatedBlockNumber <= simulatedHead;
				simulatedBlockNumber++
			) {
				const simulatedBlock = await getSimulatedBlockFromInput(
					ethereumClientService,
					undefined,
					simulationInput,
					simulatedBlockNumber,
	...
	for (const subscriptionOrFilter of ethereumSubscriptionsAndFilters) {
		if (
			websiteTabConnections.get(
				subscriptionOrFilter.subscriptionCreatorSocket.tabId,
			) === undefined
		) {
			// connection removed
			await removeEthereumSubscription(
				subscriptionOrFilter.subscriptionCreatorSocket,
				subscriptionOrFilter.subscriptionOrFilterId,
			)
			continue
		}
...
export async function createEthereumSubscription(
	...
	switch (params.params[0]) {
		case 'newHeads': {
			...
			await updateEthereumSubscriptionsAndFilters(
				(subscriptionsAndfilters: EthereumSubscriptionsAndFilters) => {
...
export async function createNewFilter(
	...
	const calledInlastBlock = await getSimulatedBlockNumberFromInput(
		ethereumClientService,
...
export async function getEthFilterChanges(
	filterId: string,
	ethereumClientService: EthereumClientService,
	requestAbortController: AbortController | undefined,
	...
	const filter = filtersAndSubscriptions.find(
		(subscriptionOrfilter) =>
			subscriptionOrfilter.subscriptionOrFilterId === filterId,
	)
	if (filter === undefined || filter.type !== 'eth_newFilter') return undefined
	if (filter.params.params[0].blockhash !== undefined)
		throw new Error('blockhash not supported for this method')
	const calledInlastBlock = await getSimulatedBlockNumber(
		ethereumClientService,
		requestAbortController,
		simulationState,
	)
	...
	await updateEthereumSubscriptionsAndFilters((subscriptionsAndfilters) => {
		return subscriptionsAndfilters.map((subscriptionOrfilter) =>
			subscriptionOrfilter.subscriptionOrFilterId === filterId
...
export async function getEthFilterLogs(
	filterId: string,
	ethereumClientService: EthereumClientService,
	requestAbortController: AbortController | undefined,
	simulationState: ResolvedExecutionSimulationState,
	...
	const filtersAndSubscriptions = await getEthereumSubscriptionsAndFilters()
	const filter = filtersAndSubscriptions.find(
		(filter) => filter.subscriptionOrFilterId === filterId,
	)
	if (filter === undefined || filter.type !== 'eth_newFilter') return undefined
	if (filter.params.params[0].blockhash !== undefined)
		throw new Error('blockhash not supported for this method')
	return await getSimulatedLogs(
		ethereumClientService,
		requestAbortController,
		simulationState,
		filter.params.params[0],
