...
export const getNextBlockTimeStampOverride = (
	...
	if (blockTimeManipulation.type === 'AddToTimestamp')
		return bigintSecondsToDate(
			prevTime +
				getBlockTimeManipulationSeconds(
					blockTimeManipulation.deltaToAdd,
					blockTimeManipulation.deltaUnit,
				),
		)
	return bigintSecondsToDate(
		max(prevTime + 1n, blockTimeManipulation.timeToSet),
	)
}

export type IEthereumClientService = Pick<
	EthereumClientService,
	keyof EthereumClientService
>
...
export class EthereumClientService {
	...
	private newBlockAttemptCallback: (
		blockHeader: EthereumBlockHeader,
		ethereumClientService: EthereumClientService,
		isNewBlock: boolean,
	) => Promise<void>
	private onErrorBlockCallback: (
		ethereumClientService: EthereumClientService,
		error: unknown,
	) => Promise<void>
	...
	constructor(
		...
		if (this.rpcEntry.httpsRpc !== requestHandler.rpcUrl)
			throw new Error(
				'The URL values for rpcEntry and requestHander must match',
			)
	}

	public readonly getRpcEntry = () => this.rpcEntry

	public readonly getNewBlockAttemptCallback = () =>
		this.newBlockAttemptCallback
	public readonly getOnErrorBlockCallback = () => this.onErrorBlockCallback

	public getCachedBlock = () => {
		if (this.cachedBlock === undefined || this.cachedBlock === null)
			return undefined
		// if the block is older than MAX_BLOCK_CACHE block intervals, invalidate cache
		if (
			Date.now() - this.cachedBlock.timestamp.getTime() >
			TIME_BETWEEN_BLOCKS * MAX_BLOCK_CACHE * 1000
		)
			return undefined
	...
	public readonly setBlockPolling = (enabled: boolean) => {
		if (enabled && this.cacheRefreshTimer === undefined) {
			...
			this.updateCache()
			const timeToTarget =
				Math.floor(now / 1000 / TIME_BETWEEN_BLOCKS) *
					1000 *
					TIME_BETWEEN_BLOCKS +
				7 * 1000 -
				now
			this.cacheRefreshTimer = setTimeout(
				() => {
					// wait until the clock is just right ( % 12 + 7 ), an then start querying every TIME_BETWEEN_BLOCKS secs
					this.updateCache()
				...
				timeToTarget > 0
					? timeToTarget
					: timeToTarget + TIME_BETWEEN_BLOCKS * 1000,
			)
			return
		}
		if (!enabled) {
	...
	private readonly updateCache = async () => {
		...
		try {
			...
			console.info(
				`${getCurrentTimestampString()} Current block number: ${newBlock.number} on ${this.getRpcEntry().name}`,
			)
	...
	public readonly estimateGas = async (
		data: PartialEthereumTransaction,
		requestAbortController: AbortController | undefined,
	) => {
		const response = await this.requestHandler.jsonRpcRequest(
			{ method: 'eth_estimateGas', params: [data] },
			requestAbortController,
		)
		return EthereumQuantity.parse(response)
	}

	public readonly getStorageAt = async (
		contract: bigint,
		slot: bigint,
		...
		const response = await this.requestHandler.jsonRpcRequest(
			{ method: 'eth_getStorageAt', params: [contract, slot, blockTag] },
			requestAbortController,
		)
		return EthGetStorageAtResponse.parse(response)
	}

	public readonly getTransactionCount = async (
		address: bigint,
		blockTag: EthereumBlockTag,
		requestAbortController: AbortController | undefined,
	) => {
	...
	public readonly getTransactionReceipt = async (
		hash: bigint,
		requestAbortController: AbortController | undefined,
	) => {
		const response = await this.requestHandler.jsonRpcRequest(
	...
	public async getBlock(
		...
		blockTag: EthereumBlockTag,
		fullObjects: boolean,
	): Promise<EthereumBlockHeaderWithTransactionHashes | EthereumBlockHeader>
	public async getBlock(
		requestAbortController: AbortController | undefined,
		blockTag: EthereumBlockTag,
		fullObjects: false,
	...
	public async getBlock(
		...
		if (cached && (blockTag === 'latest' || blockTag === cached.number)) {
			if (fullObjects === false)
				return {
					...cached,
					transactions: cached.transactions.map(
		...
		if (fullObjects === false)
			return EthereumBlockHeaderWithTransactionHashes.parse(
				await this.requestHandler.jsonRpcRequest({
					method: 'eth_getBlockByNumber',
					params: [blockTag, false],
				}),
			)
		return EthereumBlockHeader.parse(
			await this.requestHandler.jsonRpcRequest(
				{ method: 'eth_getBlockByNumber', params: [blockTag, fullObjects] },
				requestAbortController,
			),
		)
	}
	...
	public async getBlockByHash(
		...
		return EthereumBlockHeader.parse(
			await this.requestHandler.jsonRpcRequest(
				{ method: 'eth_getBlockByHash', params: [blockHash, fullObjects] },
				requestAbortController,
			),
		)
	}
	...
	public readonly getLogs = async (
		logFilter: EthGetLogsRequest,
		requestAbortController: AbortController | undefined,
	) => {
		const response = await this.requestHandler.jsonRpcRequest(
			{ method: 'eth_getLogs', params: [logFilter] },
			requestAbortController,
		)
	...
	public readonly getTransactionByHash = async (
		hash: bigint,
		requestAbortController: AbortController | undefined,
	) => {
		const response = await this.requestHandler.jsonRpcRequest(
	...
	public readonly call = async (
		...
		const response = await this.requestHandler.jsonRpcRequest(
			{ method: 'eth_call', params: [params, blockTag] },
			requestAbortController,
		)
		return EthereumData.parse(response)
	}

	public readonly ethSimulateV1 = async (
		blockStateCalls: readonly BlockCalls[],
		blockTag: EthereumBlockTag,
		requestAbortController: AbortController | undefined,
	) => {
		...
		return EthSimulateV1Result.parse(
			await this.requestHandler.jsonRpcRequest(call),
		)
	}

	public readonly prepareEthSimulateV1Input = async (
		...
		if (parentBlock === null)
			throw new Error(`The block ${blockNumber} is null`)
		...
		const gasLimitTransaction = (
			...
			...transaction,
			gas:
				transaction.gas > parentBlock.gasLimit
					? parentBlock.gasLimit
					: transaction.gas,
		})

		const splitTransactionsByGasLimit = (
			currentTransactions: EthereumSendableSignedTransaction[],
			gasLimit: bigint,
			...
			for (const transaction of currentTransactions) {
				if (transaction.gas > gasLimit)
					throw new Error(
						`Transaction gas ${transaction.gas.toString()} exceeds gas limit ${gasLimit.toString()}`,
					)
		...
		const gasLimitedInput: SimulationStateInputMinimalData =
			simulationStateInput.map((block) => ({
				...block,
				transactions: block.transactions.map((transaction) => ({
					signedTransaction: gasLimitTransaction(transaction.signedTransaction),
				})),
			}))
		const calculateTotalGasUsed = (currentTransactions: { gas: bigint }[]) =>
			currentTransactions.reduce(
				(totalGasUsed, transaction) => totalGasUsed + transaction.gas,
				0n,
			)
		...
		for (const inputBlock of gasLimitedInput) {
			const signedTransactions = inputBlock.transactions.map(
				(x) => x.signedTransaction,
			)
			if (calculateTotalGasUsed(signedTransactions) > parentBlock.gasLimit) {
				const splitted = splitTransactionsByGasLimit(
					signedTransactions,
					parentBlock.gasLimit,
				)
				for (const [index, newTransactions] of splitted.entries()) {
					...
					if (index === 0) {
						...
						rpcBlocks.push({
							...
							simulateWithZeroBaseFee: inputBlock.simulateWithZeroBaseFee,
						})
					}
				}
		...
		for (const inputBlock of rpcBlocks) {
			const newBlockOverride = {
				...
				time: getNextBlockTimeStampOverride(
					previousBlockOverride.time,
					inputBlock.blockTimeManipulation,
				),
			}
		...
		const getBlockStateCall = async (
			block: SimulationStateInputMinimalDataBlock,
			blockOverrides: BlockOverrides,
		) => {
			const transactionsWithRemoveZeroPricedOnes = block.transactions.map(
				(transaction) => {
					if (transaction.signedTransaction.type !== '1559')
						return transaction.signedTransaction
					const { maxFeePerGas, ...transactionWithoutMaxFee } =
			...
			const isHexSignature = (value: string): value is `0x${string}` =>
				value.startsWith('0x')
			const encodePackedHash = (
				messageHashAndSignature: MessageHashAndSignature,
			) => {
				if (!isHexSignature(messageHashAndSignature.signature))
					throw new Error('Signature must be hex encoded')
				...
				const packed = BigInt(
					keccak256(
						encodeAbiValues(
							...
							[
								messageHashAndSignature.messageHash,
			...
			const getMappingsMemorySlot = (hash: EthereumBytes32) =>
				keccak256(
					encodeAbiValues(['bytes32', 'uint256'], [bytes32String(hash), 0n]),
				)
			const signatureStructs = await Promise.all(
				block.signedMessages.map(async (sign) => {
					const messageHashAndSignature = await simulatePersonalSign(
						sign.originalRequestParameters,
						sign.fakeSignedFor,
					)
					return {
						key: getMappingsMemorySlot(
							encodePackedHash(messageHashAndSignature),
						),
						value: sign.fakeSignedFor,
					}
				}),
			)
			const stateSets = signatureStructs.reduce(
				(acc, current) => {
		...
		const blockStateCalls = await Promise.all(
			rpcBlocks.map(async (block, index) => {
				const blockOverrideForBlock = blockOverrides[index]
				if (blockOverrideForBlock === undefined)
					throw new Error('Block Overridex index overflow')
		...
		return {
			...
			request: {
				...
				params: [
					...
					blockNumber === parentBlock.number + 1n
						? blockNumber - 1n
						: blockNumber,
				] as const,
			},
	...
	public readonly ethSimulateV1Input = async (
		simulationStateInput: SimulationStateInputMinimalData,
		blockNumber: bigint,
		requestAbortController: AbortController | undefined,
	) => {
		return (
			await this.prepareEthSimulateV1Input(
	...
	public readonly simulatePrepared = async (
		simulationStateInput: SimulationStateInputMinimalData,
		blockNumber: bigint,
		requestAbortController: AbortController | undefined,
	) => {
		const prepared = await this.prepareEthSimulateV1Input(
			simulationStateInput,
			blockNumber,
			requestAbortController,
		)
		return {
			prepared,
			result: EthSimulateV1Result.parse(
				await this.requestHandler.jsonRpcRequest(prepared.request),
			),
		}
	}

	public readonly simulate = async (
		simulationStateInput: SimulationStateInputMinimalData,
		blockNumber: bigint,
		requestAbortController: AbortController | undefined,
	): Promise<EthSimulateV1Result> => {
		const input = await this.ethSimulateV1Input(
			simulationStateInput,
			blockNumber,
			requestAbortController,
		)
		return EthSimulateV1Result.parse(
			await this.requestHandler.jsonRpcRequest(input),
		)
	}
