...
export type Erc1155TokenBalanceChange = Erc1155Entry & {
	changeAmount: bigint
	tokenId: bigint
}
export type Erc721and1155OperatorChange = (Erc721Entry | Erc1155Entry) & {
...
export type SummaryOutcome = {
	...
	erc721TokenBalanceChanges: (Erc721Entry & {
		received: boolean
		tokenId: bigint
	})[]
...
const cloneNestedMap = <Key, NestedKey, Value>(
	input: ReadonlyMap<Key, ReadonlyMap<NestedKey, Value>>,
) => {
	return new Map(Array.from(input, ([key, value]) => [key, new Map(value)]))
}

const cloneBalanceChangeSummary = (
	...
	erc721TokenIdApprovalChanges: cloneNestedMap(
		summary.erc721TokenIdApprovalChanges,
	),
	erc1155TokenBalanceChanges: cloneNestedMap(
		summary.erc1155TokenBalanceChanges,
	),
})
...
const prepareSummaries = (
	...
	for (const address of new Set(addresses)) {
		...
		const clonedSummary =
			summary === undefined
				? createEmptyBalanceChangeSummary()
				: cloneBalanceChangeSummary(summary)
...
const cleanupErc20BalanceChange = (
	summary: BalanceChangeSummary,
	tokenAddress: string,
) => {
	if (summary.erc20TokenBalanceChanges.get(tokenAddress) === 0n)
		summary.erc20TokenBalanceChanges.delete(tokenAddress)
}

const cleanupErc1155BalanceChange = (
	summary: BalanceChangeSummary,
	tokenAddress: string,
	tokenId: string,
) => {
	if (
		summary.erc1155TokenBalanceChanges.get(tokenAddress)?.get(tokenId) === 0n
	) {
...
const buildFallbackAddressMetadata = (
	transactions: readonly (SimulatedAndVisualizedTransaction | undefined)[],
...
const getAddressMetadata = (
	address: string,
	addressMetaData: ReadonlyMap<string, AddressBookEntry>,
	fallbackAddressMetadata: FallbackAddressMetadata,
) => {
	return (
		addressMetaData.get(address) ??
		fallbackAddressMetadata.get(address) ??
		getFilledInContactEntry(BigInt(address))
	)
}

const applyErc721Change = (
	state: SummaryState,
	from: string,
	to: string,
	tokenAddress: string,
	change: TokenVisualizerResultWithMetadata,
): SummaryState => {
	if (change.type !== 'ERC721') return state

	const { nextState, preparedSummaries } = prepareSummaries(
		state,
		change.isApproval ? [from] : [from, to],
	)
	...
	if (change.isApproval) {
		...
		fromSummary.erc721TokenIdApprovalChanges
			.get(tokenAddress)
			?.set(change.tokenId.toString(), to)
		return nextState
	}

	const tokenId = change.tokenId.toString()
	for (const value of [
		{ address: from, received: false },
		{ address: to, received: true },
	]) {
		...
		const tokenBalanceChanges =
			addressSummary.erc721TokenBalanceChanges.get(tokenAddress)
		if (tokenBalanceChanges === undefined)
			throw new Error('token balance changes missing')
		...
		if (!value.received) {
			addressSummary.erc721TokenIdApprovalChanges
				.get(tokenAddress)
				?.delete(tokenId)
		}
	}
...
const applyNftAllApprovalChange = (
	state: SummaryState,
	from: string,
	to: string,
	tokenAddress: string,
	...
	const fromSummary = preparedSummaries.get(from)
	if (fromSummary === undefined) throw new Error('from summary missing')
	fromSummary.erc721and1155OperatorChanges.set(
		tokenAddress,
		change.allApprovalAdded ? to : undefined,
	)
	return nextState
}

const applyErc1155Change = (
	state: SummaryState,
	from: string,
	to: string,
	tokenAddress: string,
	...
	const fromTokenChanges =
		fromSummary.erc1155TokenBalanceChanges.get(tokenAddress) ??
	...
	const toTokenChanges =
		toSummary.erc1155TokenBalanceChanges.get(tokenAddress) ??
		new Map<string, bigint>()
...
const applyErc20Change = (
	...
	if (change.isApproval) {
		...
		if (tokenChanges === undefined) {
			fromSummary.erc20TokenApprovalChanges.set(
				tokenAddress,
				new Map([[to, change.amount]]),
			)
		} else {
	...
	fromSummary.erc20TokenBalanceChanges.set(
		tokenAddress,
		oldFromData === undefined ? -change.amount : oldFromData - change.amount,
	)
	const oldToData = toSummary.erc20TokenBalanceChanges.get(tokenAddress)
	toSummary.erc20TokenBalanceChanges.set(
		tokenAddress,
		oldToData === undefined ? change.amount : oldToData + change.amount,
	)
...
const applyTokenChange = (
	...
	switch (change.type) {
		case 'ERC1155':
			return applyErc1155Change(state, from, to, tokenAddress, change)
		case 'ERC721':
			return applyErc721Change(state, from, to, tokenAddress, change)
		case 'ERC20':
			return applyErc20Change(state, from, to, tokenAddress, change)
		case 'NFT All approval':
			return applyNftAllApprovalChange(state, from, to, tokenAddress, change)
	}
}

const applyGasFee = (
	state: SummaryState,
	result: SimulatedAndVisualizedTransaction,
): SummaryState => {
	const transactionSender = addressString(result.transaction.from.address)
	const { nextState, preparedSummaries } = prepareSummaries(state, [
		transactionSender,
	])
	...
	senderSummary.erc20TokenBalanceChanges.set(
		ethAddress,
		oldFromData === undefined ? -gasFee : oldFromData - gasFee,
	)
...
const buildSummaryState = (
	...
	return new Map(
		Array.from(summaryState.entries()).filter(
			([_address, summary]) => !isEmptyBalanceChangeSummary(summary),
		),
	)
}

const materializeSummaryForAddress = (
...
): Omit<SummaryOutcome, 'summaryFor'> | undefined => {
	const addressSummary = summaryState.get(address)
	if (addressSummary === undefined) return undefined

	const erc20TokenBalanceChanges: Erc20TokenBalanceChange[] = Array.from(
		addressSummary.erc20TokenBalanceChanges,
	).map(([tokenAddress, changeAmount]) => {
		const metadata = addressMetaData.get(tokenAddress)
		if (metadata === undefined || metadata.type !== 'ERC20')
			throw new Error(`Missing metadata for token: ${tokenAddress}`)
		const tokenPriceEstimate = tokenPriceEstimates.find(
			(tokenEstimate) =>
				addressString(tokenEstimate.token.address) === tokenAddress,
		)
		const quoteTokenAddress = tokenPriceEstimate?.quoteToken.address
		const quoteTokenEntry =
			quoteTokenAddress === undefined
				? undefined
				: addressMetaData.get(addressString(quoteTokenAddress))
		const tokenPriceEstimateQuoteToken =
			quoteTokenEntry?.type === 'ERC20' ? quoteTokenEntry : undefined
		return {
			...metadata,
			changeAmount,
			tokenPriceEstimate,
	...
	const erc20TokenApprovalChanges: ERC20TokenApprovalChange[] = Array.from(
		addressSummary.erc20TokenApprovalChanges,
	).map(([tokenAddress, approvals]) => {
		const metadata = addressMetaData.get(tokenAddress)
		if (metadata === undefined || metadata.type !== 'ERC20')
			throw new Error(`Missing metadata for token: ${tokenAddress}`)
		return {
	...
	const erc721TokenBalanceChanges: (Erc721Entry & {
		received: boolean
		tokenId: bigint
	})[] = Array.from(addressSummary.erc721TokenBalanceChanges).flatMap(
		([tokenAddress, tokenIds]) => {
			const metadata = addressMetaData.get(tokenAddress)
			if (metadata === undefined || metadata.type !== 'ERC721')
				throw new Error(`Missing metadata for token: ${tokenAddress}`)
	...
	const erc721TokenIdApprovalChanges: Erc721TokenApprovalChange[] = Array.from(
		addressSummary.erc721TokenIdApprovalChanges,
	).flatMap(([tokenAddress, approvals]) => {
		const metadata = addressMetaData.get(tokenAddress)
		if (metadata === undefined || metadata.type !== 'ERC721')
			throw new Error(`Missing metadata for token: ${tokenAddress}`)
		return Array.from(approvals).map(([tokenId, approvedAddress]) => ({
			tokenEntry: metadata,
			tokenId: BigInt(tokenId),
			approvedEntry: getAddressMetadata(
				approvedAddress,
				addressMetaData,
				fallbackAddressMetadata,
			),
	...
	const erc1155TokenBalanceChanges: Erc1155TokenBalanceChange[] = Array.from(
		addressSummary.erc1155TokenBalanceChanges,
	).flatMap(([tokenAddress, tokenIds]) => {
		const metadata = addressMetaData.get(tokenAddress)
		if (metadata === undefined || metadata.type !== 'ERC1155')
			throw new Error(`Missing metadata for token: ${tokenAddress}`)
		return Array.from(tokenIds).map(([tokenId, changeAmount]) => ({
			...metadata,
			tokenId: BigInt(tokenId),
			tokenIdnName: namedTokenIds.find(
				(namedTokenId) =>
					namedTokenId.tokenAddress === BigInt(tokenAddress) &&
					namedTokenId.tokenId === BigInt(tokenId),
			)?.tokenIdName,
			changeAmount,
		}))
	})

	const erc721and1155OperatorChanges: Erc721and1155OperatorChange[] =
		Array.from(addressSummary.erc721and1155OperatorChanges).map(
			([tokenAddress, operator]) => {
				const metadata = addressMetaData.get(tokenAddress)
				if (
					metadata === undefined ||
					(metadata.type !== 'ERC1155' && metadata.type !== 'ERC721')
				)
					throw new Error(`Missing metadata for token: ${tokenAddress}`)
...
export const summarizeLogsForAddress = (
	transactions: readonly (SimulatedAndVisualizedTransaction | undefined)[],
	address: string,
	addressMetaData: ReadonlyMap<string, AddressBookEntry>,
	tokenPriceEstimates: readonly TokenPriceEstimate[],
	...
	const { summaryState, fallbackAddressMetadata } =
		buildSummaryArtifacts(transactions)
	return materializeSummaryForAddress(
		summaryState,
		fallbackAddressMetadata,
		address,
		addressMetaData,
...
export const summarizeLogs = (
	...
	for (const [address] of summaryState.entries()) {
		const summary = materializeSummaryForAddress(
			summaryState,
			fallbackAddressMetadata,
			address,
			addressMetaData,
