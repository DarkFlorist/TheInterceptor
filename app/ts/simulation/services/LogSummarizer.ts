...
const isEmptyBalanceChangeSummary = (summary: BalanceChangeSummary) => {
	return summary.erc721TokenBalanceChanges.size === 0 && summary.erc721TokenIdApprovalChanges.size === 0 && summary.erc20TokenApprovalChanges.size === 0 && summary.erc20TokenBalanceChanges.size === 0 && summary.erc1155TokenBalanceChanges.size === 0 && summary.erc721and1155OperatorChanges.size === 0
}
...
const applyTransaction = (state: SummaryState, result: SimulatedAndVisualizedTransaction): SummaryState => {
	const stateAfterTokenChanges = extractTokenEvents(result.events).reduce((currentState, change) => applyTokenChange(currentState, change), state)
...
export const summarizeLogs = (transactions: readonly (SimulatedAndVisualizedTransaction | undefined)[], addressMetaData: ReadonlyMap<string, AddressBookEntry>, tokenPriceEstimates: readonly TokenPriceEstimate[], namedTokenIds: readonly NamedTokenId[]) => {
	...
	for (const [address] of summaryState.entries()) {
		...
		summaries.push({
			summaryFor: getAddressMetadata(address, addressMetaData, fallbackAddressMetadata),
			...summary,
		})
	}
	return summaries
