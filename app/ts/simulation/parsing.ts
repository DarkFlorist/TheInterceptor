...
import {
	...
	ENS_ADDRESS_CHANGED,
	ENS_ADDR_CHANGED,
	ENS_CONTENT_HASH_CHANGED,
	ENS_ETHEREUM_NAME_SERVICE,
	ENS_ETH_REGISTRAR_CONTROLLER,
	ENS_EXPIRY_EXTENDED,
	ENS_FUSES_SET,
	ENS_NAME_CHANGED,
	ENS_CONTROLLER_NAME_REGISTERED,
	ENS_BASE_REGISTRAR_NAME_RENEWED,
	ENS_NAME_UNWRAPPED,
	ENS_NEW_OWNER,
	ENS_NEW_RESOLVER,
	ENS_NEW_TTL,
	...
	ENS_TOKEN_WRAPPER,
	ENS_TRANSFER,
	ERC1155_TRANSFERBATCH_LOG,
	ERC1155_TRANSFERSINGLE_LOG,
	ERC721_APPROVAL_FOR_ALL_LOG,
	TRANSFER_LOG,
	WITHDRAWAL_LOG,
	ENS_BASE_REGISTRAR_NAME_REGISTERED,
	ENS_NAME_WRAPPED,
...
import {
	...
	handleEnsAddrChanged,
	handleEnsAddressChanged,
	handleEnsContentHashChanged,
	handleEnsExpiryExtended,
	handleEnsFusesSet,
	handleEnsNameChanged,
	handleEnsNameUnWrapped,
	handleEnsNewOwner,
	handleEnsNewResolver,
	handleEnsNewTtl,
	handleEnsControllerNameRenewed,
	handleEnsReverseClaimed,
	handleEnsTextChanged,
	handleEnsTextChangedKeyValue,
	handleEnsTransfer,
	handleErc721ApprovalForAllLog,
	handleControllerNameRegistered,
	handleBaseRegistrarNameRenewed,
	handleWithdrawalLog,
	handleBaseRegistrarNameRegistered,
	handleNameWrapped,
} from './logHandlers.js'
...
const getTokenEventHandler = (type: AddressBookEntryCategory, logSignature: string) => {
	...
	switch (type) {
		case 'ERC1155':
			return erc1155LogHanders.get(logSignature)
		case 'ERC20':
			return erc20LogHanders.get(logSignature)
		case 'ERC721':
			return erc721LogHanders.get(logSignature)
		case 'activeAddress':
		case 'contact':
		case 'contract':
			return undefined
		...
		if (parsedEvent.loggersAddressBookEntry.address === ENS_PUBLIC_RESOLVER || parsedEvent.loggersAddressBookEntry.address === ENS_PUBLIC_RESOLVER_2) {
			if (logSignature === ENS_ADDRESS_CHANGED)
				return {
					...
					type: 'ENS' as const,
					subType: 'ENSAddressChanged' as const,
				}
			if (logSignature === ENS_ADDR_CHANGED)
				return {
					logInformation: handleEnsAddrChanged(parsedEvent),
					type: 'ENS' as const,
					subType: 'ENSAddrChanged' as const,
				}
			if (logSignature === ENS_TEXT_CHANGED)
				return {
					logInformation: handleEnsTextChanged(parsedEvent),
					type: 'ENS' as const,
					subType: 'ENSTextChanged' as const,
				}
			if (logSignature === ENS_TEXT_CHANGED_KEY_VALUE)
				return {
					logInformation: handleEnsTextChangedKeyValue(parsedEvent),
					type: 'ENS' as const,
					subType: 'ENSTextChangedKeyValue' as const,
				}
			if (logSignature === ENS_CONTENT_HASH_CHANGED)
				return {
					logInformation: handleEnsContentHashChanged(parsedEvent),
					type: 'ENS' as const,
					subType: 'ENSContentHashChanged' as const,
				}
			if (logSignature === ENS_NAME_CHANGED)
				return {
					logInformation: handleEnsNameChanged(parsedEvent),
					type: 'ENS' as const,
					subType: 'ENSNameChanged' as const,
				}
		}
		if (parsedEvent.loggersAddressBookEntry.address === ENS_TOKEN_WRAPPER) {
			if (logSignature === ENS_FUSES_SET)
				return {
					logInformation: handleEnsFusesSet(parsedEvent),
					type: 'ENS' as const,
					subType: 'ENSFusesSet' as const,
				}
			if (logSignature === ENS_NAME_UNWRAPPED)
				return {
					logInformation: handleEnsNameUnWrapped(parsedEvent),
					type: 'ENS' as const,
					subType: 'ENSNameUnwrapped' as const,
				}
			if (logSignature === ENS_NAME_WRAPPED)
				return {
					logInformation: handleNameWrapped(parsedEvent),
					type: 'ENS' as const,
					subType: 'ENSNameWrapped' as const,
				}
			if (logSignature === ENS_EXPIRY_EXTENDED)
				return {
					logInformation: handleEnsExpiryExtended(parsedEvent),
					type: 'ENS' as const,
			...
			if (logSignature === ENS_CONTROLLER_NAME_REGISTERED)
				return {
					...
					type: 'ENS' as const,
					subType: 'ENSControllerNameRegistered' as const,
				}
			if (logSignature === ENS_CONTROLLER_NAME_RENEWED)
				return {
					logInformation: handleEnsControllerNameRenewed(parsedEvent),
					type: 'ENS' as const,
			...
			if (logSignature === ENS_BASE_REGISTRAR_NAME_RENEWED)
				return {
					...
					type: 'ENS' as const,
					subType: 'ENSBaseRegistrarNameRenewed' as const,
				}
			if (logSignature === ENS_BASE_REGISTRAR_NAME_REGISTERED)
				return {
					logInformation: handleBaseRegistrarNameRegistered(parsedEvent),
					type: 'ENS' as const,
			...
			if (logSignature === ENS_TRANSFER)
				return {
					...
					type: 'ENS' as const,
					subType: 'ENSTransfer' as const,
				}
			if (logSignature === ENS_NEW_OWNER)
				return {
					logInformation: handleEnsNewOwner(parsedEvent),
					type: 'ENS' as const,
					subType: 'ENSNewOwner' as const,
				}
			if (logSignature === ENS_NEW_RESOLVER)
				return {
					logInformation: handleEnsNewResolver(parsedEvent),
					type: 'ENS' as const,
					subType: 'ENSNewResolver' as const,
				}
			if (logSignature === ENS_NEW_TTL)
				return {
					logInformation: handleEnsNewTtl(parsedEvent),
					type: 'ENS' as const,
					subType: 'ENSNewTTL' as const,
				}
			if (logSignature === ENS_EXPIRY_EXTENDED)
				return {
					logInformation: handleEnsExpiryExtended(parsedEvent),
			...
			if (logSignature === ENS_REVERSE_CLAIMED)
				return {
					...
					type: 'ENS' as const,
					subType: 'ENSReverseClaimed' as const,
				}
		}
	}
	return undefined
...
export const parseInputData = async (
	transaction: {
		to: EthereumAddress | undefined | null
		...
		const valuesWithTypes = parsed.args.map((value, index) => {
			...
			if (typeof value === 'object' && value !== null && 'hash' in value) {
				return {
					paramName,
					typeValue: {
						type: 'fixedBytes' as const,
						value: EthereumData.parse(value.hash),
					},
				}
			}
			return {
