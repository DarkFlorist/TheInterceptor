	...
	return <main class = 'signing-page'>
		...
		{ transaction === undefined ? <section class = 'signing-panel'><h2>{ record.input.method === 'personal_sign' ? 'Personal message to sign' : 'Typed data to sign (EIP-712)' }</h2>{ typedData === undefined ? <pre>{ messageText }</pre> : <><h3>Domain</h3><TypedDataFields fields = { typedData.domain }/><h3>{ typedData.primaryType }</h3><TypedDataFields fields = { typedData.message }/><details><summary>Full typed data and type definitions</summary><pre>{ JSON.stringify(JSON.parse(record.input.data), undefined, 2) }</pre></details></> }{ messageBytes === undefined ? undefined : <details><summary>Exact message bytes · { messageBytes.length } bytes</summary><p class = 'signing-muted'>Text is a UTF-8 preview. Binary bytes and invisible characters may not display as text; the exact bytes below are signed.</p><pre>{ record.input.data }</pre></details> }</section> : <section class = 'signing-panel'>
			<dl class = 'signing-grid'>
				...
				<div><dt>Recipient</dt><dd class = 'signing-address'>{ transaction.to ?? 'Contract creation' }</dd></div>
		...
		{ record.phase === 'confirmed' ? <section class = { `signing-panel ${ record.executionSucceeded ? 'signing-success' : 'signing-error' }` } role = 'status'><h2>{ record.executionSucceeded ? 'Transaction confirmed' : 'Transaction reverted' }</h2><p>{ record.executionSucceeded ? 'The network confirmed your transaction. You can return to the application.' : 'The network included your transaction, but execution failed. Network fees may still apply.' }</p><p class = 'signing-address'>{ record.transactionHash }</p></section> : undefined }
		...
		{ status === '' || qr !== undefined || complete ? undefined : <p class = 'signing-muted' role = 'status'>{ status }</p> }
		...
		{ qr === undefined || record.phase !== 'approved' ? undefined : <section class = 'signing-panel'>
			...
			{ scanning ? <><SigningQrScanner onFrame = { receiveSignedResponse } onError = { () => setStatus('Scan stopped.') } onStart = { () => { decoder.current = createAirGapUrDecoder('eth-signature'); setStatus('Waiting for the response QR from Vault.') } }/><p class = 'signing-muted' role = 'status'>{ status }</p></> : undefined }
		</section> }
		...
		{ record.transactionHash === undefined || complete ? undefined : <details><summary>Transaction hash</summary><p class = 'signing-address'>{ record.transactionHash }</p></details> }
	</main>
}
