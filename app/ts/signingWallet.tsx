...
function SigningWalletPage() {
	...
	return <main style = 'max-width: 760px; margin: 30px auto; padding: 24px; color: var(--text-color); display: flex; flex-direction: column; gap: 16px;'>
		...
		{ kind === 'airgap' ? <section>
			...
			<button class = 'button' onClick = { () => { setDecoder(createAirGapUrDecoder(importType)); setScan(!scan) } }>{ scan ? 'Cancel scan' : 'Scan public account' }</button>
		...
		{ busy ? <button class = 'button' onClick = { () => controller.current.abort(new Error('Wallet setup cancelled')) }>Cancel device operation</button> : undefined }
		...
		{ binding === undefined ? undefined : <button class = 'button' disabled = { busy } onClick = { () => run(async () => { await sendSigningPageRequest({ method: 'signing_saveWallet', address: binding.wallet.address, wallet: undefined, revision: binding.revision, name: undefined }); setBinding(undefined); setStatus('Wallet removed. The address remains saved.') }) }>Remove signing wallet</button> }
	</main>
}
