...
function validateEIP712TypesSubset(
	...
	currentType: string,
	types: {
		[x: string]: readonly { readonly name: string; readonly type: string }[] | undefined
	},
): boolean {
	...
	const pairArray: [string, EnrichedEIP712MessageRecord][] = await promiseAllMapAbortSafe(Array.from(messageEntries), async ([key, messageEntry]): Promise<[string, EnrichedEIP712MessageRecord]> => {
		...
		if (arraylessType.isArray) {
			...
			return [
				...
				{
					type: 'record[]',
					value: await promiseAllMapAbortSafe(jsonEncodeableArray.value, (subSubMessage) => {
		...
		return [
			...
			{
				type: 'record',
				value: await extractEIP712MessageSubset(ethereumClientService, requestAbortController, depth + 1, messageEntry, fullType, types, useLocalStorage),
			},
		]
	})
