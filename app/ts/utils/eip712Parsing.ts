...
import {
	JSONEncodeableObject,
	JSONEncodeableObjectArray,
} from '../utils/json.js'
import type {
	EIP712Message,
	EnrichedEIP712,
...
function separateArraySuffix(typeWithMaybeArraySuffix: string) {
	...
	if (splitted.length === 1)
		return { arraylessType: typeWithMaybeArraySuffix, isArray: false }
	return { arraylessType: splitted[0], isArray: true }
}

function validateEIP712TypesSubset(
	depth: number,
	...
	for (const key of keys) {
		...
		if (jsonEncodeableArray.success) {
			if (
				jsonEncodeableArray.value
					.map((arrayElement) =>
						JSONEncodeableObject.test(arrayElement)
							? validateEIP712TypesSubset(
									depth,
									arrayElement,
									currentType,
									types,
			...
			if (
				validateEIP712TypesSubset(depth + 1, subMessage, currentType, types) ===
				false
			)
				return false
		}
...
export function validateEIP712Types(message: EIP712Message) {
	return (
		...
		validateEIP712TypesSubset(0, message.domain, 'EIP712Domain', message.types)
	)
}

async function extractEIP712MessageSubset(
	...
	if (currentTypes === undefined)
		throw new Error(`Types not found: ${currentType}`)
	const messageEntries = Object.entries(message)
	const pairArray: [string, EnrichedEIP712MessageRecord][] =
		await promiseAllMapAbortSafe(
			Array.from(messageEntries),
			async ([key, messageEntry]): Promise<
				[string, EnrichedEIP712MessageRecord]
			> => {
				if (messageEntry === undefined)
					throw new Error(`Subtype not found: ${key}`)
				const fullType = findType(key, currentTypes)
				if (fullType === undefined)
					throw new Error(`Type not found for key: ${key}`)
				const arraylessType = separateArraySuffix(fullType)
				if (SolidityType.test(arraylessType.arraylessType)) {
					return [
						key,
						await parseSolidityValueByTypeEnriched(
							ethereumClientService,
							requestAbortController,
							SolidityType.parse(arraylessType.arraylessType),
							messageEntry,
							arraylessType.isArray,
							useLocalStorage,
						),
					]
				}
				if (arraylessType.isArray) {
					const jsonEncodeableArray =
						JSONEncodeableObjectArray.safeParse(messageEntry)
					if (!jsonEncodeableArray.success)
						throw new Error(
							`Type was defined to be an array but it was not: ${messageEntry}`,
						)
					const currentType = arraylessType.arraylessType
					if (currentType === undefined)
						throw new Error(`array's type is missing`)
					return [
						key,
						{
							type: 'record[]',
							value: await promiseAllMapAbortSafe(
								jsonEncodeableArray.value,
								(subSubMessage) => {
									if (JSONEncodeableObject.test(subSubMessage))
										return extractEIP712MessageSubset(
											ethereumClientService,
											requestAbortController,
											depth + 1,
											subSubMessage,
				...
				if (!JSONEncodeableObject.test(messageEntry))
					throw new Error(`Not a JSON type: ${messageEntry}`)
				return [
					key,
					{
						type: 'record',
						value: await extractEIP712MessageSubset(
							ethereumClientService,
							requestAbortController,
							depth + 1,
							messageEntry,
	...
	return pairArray.reduce(
		(accumulator, [key, value]) => ({ ...accumulator, [key]: value }),
		{} as Promise<EnrichedEIP712Message>,
	)
}

export async function extractEIP712Message(
	ethereumClientService: EthereumClientService,
	requestAbortController: AbortController | undefined,
	message: EIP712Message,
	useLocalStorage = true,
): Promise<EnrichedEIP712> {
	return {
		primaryType: message.primaryType,
		message: await extractEIP712MessageSubset(
			ethereumClientService,
			requestAbortController,
			0,
			message.message,
		...
		domain: await extractEIP712MessageSubset(
			ethereumClientService,
			requestAbortController,
			0,
			message.domain,
