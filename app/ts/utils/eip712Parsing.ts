...
async function extractEIP712MessageSubset(ethereumClientService: EthereumClientService, depth: number, message: JSONEncodeableObject, currentType: string, types: { [x: string]: readonly { readonly name: string, readonly type: string}[] | undefined }, useLocalStorage: boolean = true): Promise<EnrichedEIP712Message> {
...
export async function extractEIP712Message(ethereumClientService: EthereumClientService, message: EIP712Message, useLocalStorage: boolean = true): Promise<EnrichedEIP712> {
	return {
		primaryType: message.primaryType,
		message: await extractEIP712MessageSubset(ethereumClientService, 0, message.message, message.primaryType, message.types, useLocalStorage),
		domain: await extractEIP712MessageSubset(ethereumClientService, 0, message.domain, 'EIP712Domain', message.types, useLocalStorage),
	}
}
