// Local data-driven ENS normalizer derived from @adraffy/ens-normalize 1.11.1.
...
// The package dependency is intentionally not used at runtime; this file keeps the generated data local.
// biome-ignore-all lint: generated normalization data mirrors the upstream ENS table decoder.
...
function decode_arithmetic(bytes: any) {
	...
	return symbols.map((x: any) => { // index into payload
		switch (x - offset) {
...
function read_compressed_payload(s: any) {
...
function unsafe_atob(s: any) {
	let lookup: any[] = [];
	[...'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'].forEach((c: any, i: any) => lookup[c.charCodeAt(0)] = i);
	let n = s.length;
...
function read_deltas(n: any, next: any) {
	let v = Array(n);
...
// [123][5] => [0 3] [1 1] [0 0]
function read_sorted(next: any, prev: any = 0) {
	let ret = [];
	while (true) {
		let x = next();
		let n = next();
...
function read_array_while(next: any) {
	let v: any[] = [];
	while (true) {
		let x = next(v.length);
		if (!x) break;
...
function read_transposed(n: any, w: any, next: any) {
	let m: any[][] = Array(n).fill(undefined).map(() => []);
	for (let i = 0; i < w; i++) {
		read_deltas(n, next).forEach((x: any, j: any) => m[j]!.push(x));
	}
	return m;
}
...
function read_linear_table(w: any, next: any) {
	...
	return m.flatMap((v: any, i: any) => {
		let [x, ...ys] = v;
			return Array(vN[i]).fill(undefined).map((_: any, j: any) => {
			let j_dy = j * dy;
			return [x + j * dx, ys.map((y: any) => y + j_dy)];
		});
	});
}
...
function read_trie(next: any) {
	...
	function decode(Q: any) { // characters that lead into this node
		...
		let B = read_array_while(() => { // buckets leading to new nodes
			let cps = read_sorted(next).map((i: any) => sorted[i]);
...
function hex_cp(cp: any) {
	return cp.toString(16).toUpperCase().padStart(2, '0');
}

function quote_cp(cp: any) {
...
*/
function explode_cp(s: any) { // this is about 2x faster
	let cps = [];
...
function compare_arrays(a: any, b: any) {
	let n = a.length;
	let c = n - b.length;
...
function unpack_cc(packed: any) {
	return (packed >> 24) & 0xFF;
}
function unpack_cp(packed: any) {
	return packed & 0xFFFFFF;
}

let SHIFTED_RANK: any, EXCLUSIONS: any, DECOMP: any, RECOMP: any;
...
function init$1() {
	...
	SHIFTED_RANK = new Map(read_sorted_arrays(r).flatMap((v: any, i: any) => v.map((x: any) => [x, (i+1) << 24]))); // pre-shifted
...
function is_hangul(cp: any) {
	return cp >= S0 && cp < S1;
}

function compose_pair(a: any, b: any) {
...
function composed_from_decomposed(v: any) {
	let ret = [];
	let stack = [];
	let prev_cp = -1;
	let prev_cc = 0;
...
function nfd(cps: any) {
	return decomposed(cps).map(unpack_cp);
}
function nfc(cps: any) {
...
const Array_from = (x: any): any[] => Array.from(x); // Array.from.bind(Array);

function group_has_cp(g: any, cp: any) {
...
let MAPPED: any, IGNORED: any, CM: any, NSM: any, ESCAPE: any, GROUPS: any, WHOLE_VALID: any, WHOLE_MAP: any, VALID: any, EMOJI_LIST: any, EMOJI_ROOT: any;
...
function init() {
	...
	*/
	...
	const read_chunked = () => {
		...
		let N: any = read_array_while(r).map((x: any) => x+0x60);
		if (N.length) {
	...
	for (let {V, M} of new Set<any>(WHOLE_MAP.values())) {
		// connect all groups that have each whole character
		let recs = [];
		for (let cp of V) {
			let gs = GROUPS.filter((g: any) => group_has_cp(g, cp));
			let rec: any = recs.find(({G}: any) => gs.some((g: any) => G.has(g)));
			if (!rec) {
				rec = {G: new Set(), V: []};
				recs.push(rec);
			}
		...
		let union = recs.flatMap((x: any) => Array_from(x.G)); // all of the groups used by this whole
		for (let {G, V} of recs) {
			let complement = new Set(union.filter((g: any) => !G.has(g))); // groups not covered by the extent
			for (let cp of V) {
	...
	const add_to_union = (cp: any) => VALID.has(cp) ? multi.add(cp) : VALID.add(cp);
	for (let g of GROUPS) {
...
function check_label_extension(cps: any) {
	if (cps.length >= 4 && cps[2] == HYPHEN && cps[3] == HYPHEN) {
		throw new Error(`invalid label extension: "${str_from_cps(cps.slice(0, 4))}"`); // this can only be ascii so cant be bidi
	}
}
function check_leading_underscore(cps: any) {
...
function safe_str_from_cps(cps: any, max: any = Infinity, quoter: any = quote_cp) {
...
function is_combining_mark(cp: any, only_nsm?: any) { // 20240127: add extra argument
	init();
	return only_nsm ? NSM.has(cp) : CM.has(cp);
}
function should_escape(cp: any) {
	init();
...
function split(name: any, nf: any, ef: any) {
	...
	return name.split(STOP_CH).map((label: any) => {
		...
		try {
			...
			if (!emoji && norm.every((cp: any) => cp < 0x80)) { // special case for ascii
				...
				let chars = tokens.flatMap((x: any) => x.is_emoji ? [] : x); // all of the nfc tokens concat together
...
function check_whole(group: any, unique: any) {
	let maker;
	let shared = [];
	...
	if (maker) {
		...
		for (let g of maker) {
			if (shared.every((cp: any) => group_has_cp(g, cp))) {
...
function determine_group(unique: any) {
	let groups = GROUPS;
	for (let cp of unique) {
...
function flatten(split: any) {
	return split.map(({input, error, output}: any) => {
		if (error) {
...
function error_group_member(g: any, cp: any) {
	...
	let gg = GROUPS.find((g: any) => g.P.has(cp)); // only check primary
	if (gg) {
		quoted = `${gg.N} ${quoted}`;
	}
	return new Error(`illegal mixture: ${g.N} + ${quoted}`);
}
function error_placement(where: any) {
...
function check_group(g: any, cps: any) {
	for (let cp of cps) {
...
function tokens_from_str(input: any, nf: any, ef: any) {
	let ret: any[] = [];
	let chars: any[] = [];
...
function filter_fe0f(cps: any) {
	return cps.filter((cp: any) => cp != FE0F);
}
...
function consume_emoji_reversed(cps: any, eaten?: any) {
	let node = EMOJI_ROOT;
	let emoji;
	let pos = cps.length;
	while (pos) {
