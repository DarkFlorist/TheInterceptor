...
async function readBlobWithSizeLimit(response: Response, maxSizeInBytes: number): Promise<Blob | ImageToUriResult> {
	const contentLength = parseContentLength(response.headers.get('content-length'))
	if (contentLength !== undefined && contentLength > maxSizeInBytes) return imageTooLarge(maxSizeInBytes)
