...
export async function itentifyAddressViaOnChainInformation(ethereumClientService: IEthereumClientService, requestAbortController: AbortController | undefined, address: EthereumAddress): Promise<IdentifiedAddress> {
	...
	const calls = [
		{
			targetAddress,
			callData: stringToUint8Array(encodeFunctionCall(Erc721ABI, 'supportsInterface', ['0x80ac58cd'])),
		}, // Is Erc721Definition
		{
			targetAddress,
			callData: stringToUint8Array(encodeFunctionCall(Erc721ABI, 'supportsInterface', ['0x5b5e139f'])),
		}, // Is Erc721Metadata
		{
			targetAddress,
			callData: stringToUint8Array(encodeFunctionCall(Erc721ABI, 'supportsInterface', ['0xd9b67a26'])),
		...
		{
			targetAddress,
			callData: stringToUint8Array(encodeFunctionCall(Erc20ABI, 'decimals', [])),
		},
		{
			targetAddress,
			callData: stringToUint8Array(encodeFunctionCall(Erc20ABI, 'totalSupply', [])),
		},
	]
