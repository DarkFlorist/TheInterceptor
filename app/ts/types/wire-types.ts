...
const TimestampParser: funtypes.ParsedValue<funtypes.String, Date>['config'] = {
	parse: (value) => {
		...
		return {
			success: true,
			value: new Date(Number.parseInt(value, 16) * 1000),
		}
	},
	serialize: (value) => {
...
const EthereumSignatureParityParser: funtypes.ParsedValue<funtypes.String, 'even' | 'odd'>['config'] = {
	parse: (value) => {
		switch (value) {
			case '0x0':
				return { success: true, value: 'even' }
			case '0x1':
				return { success: true, value: 'odd' }
			default:
				return {
					success: false,
	...
	serialize: (value) => {
		switch (value) {
			case 'even':
				return { success: true, value: '0x0' }
			case 'odd':
				return { success: true, value: '0x1' }
			default:
				return {
					success: false,
					message: `${ value } is not a supported signature parity.`,
...
const EthereumTypedTransactionVParser: funtypes.ParsedValue<funtypes.String, bigint>['config'] = {
	parse: (value) => {
		switch (value) {
			case '0x0':
				return { success: true, value: 0n }
			case '0x1':
				return { success: true, value: 1n }
			default:
				return {
					success: false,
...
export const EthereumBlockHeader = funtypes.Union(
	...
	funtypes.Intersect(
		EthereumBlockHeaderWithoutTransactions,
		funtypes.ReadonlyObject({
			transactions: funtypes.ReadonlyArray(EthereumBlockHeaderTransaction),
		}),
	),
)
