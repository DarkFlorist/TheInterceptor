...
export const EIP712Types = funtypes.Record(
	funtypes.String,
...
const EIP712MessageParser: funtypes.ParsedValue<
	funtypes.String,
	EIP712MessageUnderlying
>['config'] = {
	parse: (value) => {
		if (
			!isJSON(value) ||
			!EIP712MessageUnderlying.test(
				jsonParserWithNumbersAsStringsConverter(value),
			)
		)
			return { success: false, message: `${value} is not EIP712 message` }
		return {
			success: true,
			value: EIP712MessageUnderlying.parse(
				jsonParserWithNumbersAsStringsConverter(value),
			),
		}
	},
...
export type TypeEnrichedEIP712MessageRecord =
	| EnrichedGroupedSolidityType
	| {
			type: 'record'
			value: { [x: string]: TypeEnrichedEIP712MessageRecord | undefined }
	  }
	| {
			type: 'record[]'
			value: ReadonlyArray<{
				[x: string]: TypeEnrichedEIP712MessageRecord | undefined
			}>
	  }
export type EnrichedEIP712MessageRecord = funtypes.Static<
	typeof EnrichedEIP712MessageRecord
>
export const EnrichedEIP712MessageRecord: funtypes.Runtype<TypeEnrichedEIP712MessageRecord> =
	funtypes.Lazy(() =>
		funtypes.Union(
			EnrichedGroupedSolidityType,
			funtypes.ReadonlyObject({
				type: funtypes.Literal('record'),
				value: funtypes.ReadonlyRecord(
					funtypes.String,
			...
			funtypes.ReadonlyObject({
				type: funtypes.Literal('record[]'),
				value: funtypes.ReadonlyArray(
					funtypes.ReadonlyRecord(funtypes.String, EnrichedEIP712MessageRecord),
				),
			}),
		),
	)

export type EnrichedEIP712Message = funtypes.Static<
	typeof EnrichedEIP712Message
>
...
const numberAsBigIntParser: funtypes.ParsedValue<
	funtypes.Number,
	bigint
>['config'] = {
	parse: (value) => {
...
export const Eip712Number = funtypes.Union(
	EthereumQuantity,
	NonHexBigInt,
	NumberAsBigInt,
)
