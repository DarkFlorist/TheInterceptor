import { popupRpcMessageHandler, popupSnapshotMessageHandler, popupMessageHandler, type PopupMessageHandlerMap } from '../popupMessageHandlerRegistry.js'
...
export const addressBookPopupMessageHandlers = {
	popup_addOrModifyAddressBookEntry: popupRpcMessageHandler(
		'popup_addOrModifyAddressBookEntry',
		async (context, request) => await addOrModifyAddressBookEntry(context.simulationServicesOwner, context.websiteTabConnections, request),
		() => ({ type: 'AddOrModifyAddressBookEntryReply', ok: false, message: 'RPC configuration is unavailable. Restore it before changing address-book entries.' }),
