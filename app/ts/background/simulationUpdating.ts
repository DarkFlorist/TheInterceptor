...
import {
	...
	getAddressToMakeRich,
	getBaseFeeAdjustedTransactions,
	getNonceFixedSimulationStateInput,
	getSimulatedCode,
	getTokenBalancesAfterForTransaction,
	getWebsiteCreatedEthereumUnsignedTransactions,
	mockSignTransaction,
	simulateEstimateGasFromInput,
	sliceSimulationState,
...
import {
	...
	type ExecutionSimulationState,
	type NonSimulatedAndVisualizedTransaction,
	type PreSimulationTransaction,
	type SignedMessageTransaction,
	type SimulationState,
	type SimulationStateInput,
	type SimulationStateInputBlock,
	type VisualizedSimulatorState,
...
import {
	getAddressBookEntriesForVisualiserFromTransactions,
	identifyAddress,
	nameTokenIds,
	retrieveEnsNodeAndLabelHashes,
...
import {
	...
	updatePopupVisualisationWithCallBack,
...
import {
	formSimulatedAndVisualizedTransactions,
	getFromAndToMetadata,
} from '../components/formVisualizerResults.js'
import {
	promiseAllMapAbortSafe,
...
const getMakeCurrentAddressRichStateOverride = (
	addressesToMakeRich: bigint[],
) => {
...
export const getAddressesbeingMadeRich = async () => {
	...
	return [
		...makeRichAddressList.filter((x) => x.makingRich).map((x) => x.address),
		...(currentAddressBeingRich === undefined ? [] : [currentAddressBeingRich]),
	]
}

export const getCurrentSimulationInput =
	async (): Promise<SimulationStateInput> => {
		const [settings, preSimulationBlockTimeManipulation] = await Promise.all([
			getSettings(),
			getPreSimulationBlockTimeManipulation(),
		])
		const richListPromise = silenceChromeUnCaughtPromise(
			getAddressesbeingMadeRich(),
		)
		...
		let previousBlockTimeManipulation = settings.simulationMode
...
export async function getMetadataForSimulation(
	...
	const addressBookEntryPromises = silenceChromeUnCaughtPromise(
		getAddressBookEntriesForVisualiserFromTransactions(
			ethereum,
	...
	const namedTokenIdPromises = silenceChromeUnCaughtPromise(
		nameTokenIds(ethereum, allEvents),
	)
	const addressBookEntries = await addressBookEntryPromises
	const ensPromise = silenceChromeUnCaughtPromise(
		retrieveEnsNodeAndLabelHashes(ethereum, allEvents, addressBookEntries),
	)
	...
	return [
		...simulationInput,
		{
			...
			blockTimeManipulation: {
				type: 'SetTimetamp',
				timeToSet: dateToBigintSeconds(executionTimestamp),
			},
			simulateWithZeroBaseFee: false,
		},
	]
...
export const simulateGovernanceContractExecution = async (
	pendingTransaction: PendingTransaction,
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
): Promise<
	DistributiveOmit<SimulateExecutionReplyData, 'transactionOrMessageIdentifier'>
> => {
	...
	try {
		...
		if (pendingTransaction.transactionOrMessageCreationStatus !== 'Simulated')
			return returnError('Still simulating the voting transaction')
		const fourByte = get4Byte(
			pendingTransaction.transactionToSimulate.transaction.input,
		)
		const fourByteString = get4ByteString(
			pendingTransaction.transactionToSimulate.transaction.input,
		)
		if (fourByte === undefined || fourByteString === undefined)
			return returnError('Could not identify the 4byte signature')
		const explanation = FourByteExplanations[fourByte]
		if (
			explanation !== 'Cast Vote' &&
		...
		if (pendingTransaction.transactionToSimulate.transaction.to === null)
			return returnError(
				'The transaction creates a contract instead of casting a vote',
			)
		const params = decodeCallDataLoose(
		...
		if (params === undefined)
			return returnError('Could not find the voting function')
		const { proposalId: rawProposalId } = params.namedArgs
		const proposalId = funtypes.BigInt.parse(rawProposalId)
		const addr = await identifyAddress(
			ethereum,
			undefined,
			pendingTransaction.transactionToSimulate.transaction.to,
		)
		if (!('abi' in addr) || addr.abi === undefined)
			return {
				success: false as const,
				errorType: 'MissingAbi' as const,
				errorMessage: 'ABi for the governance contract is missing',
				errorAddressBookEntry: addr,
			}
		const contractExecutionResult = await simulateCompoundGovernanceExecution(
			ethereum,
			addr,
			proposalId,
		)
		if (contractExecutionResult === undefined)
			return returnError('Failed to simulate governance execution')
		const parentBlock = await ethereum.getBlock(undefined)
		if (parentBlock === null) throw new Error('The latest block is null')
		if (parentBlock.baseFeePerGas === undefined)
			return returnError('cannot build simulation from legacy block')
		const simulationInput = await getCurrentSimulationInput()
		const signedExecutionTransaction = mockSignTransaction({
			...contractExecutionResult.executingTransaction,
			gas: contractExecutionResult.ethSimulateV1CallResult.gasUsed,
		})
		...
		const governanceExecutionSimulationInput =
			getGovernanceExecutionSimulationInput(
				simulationInput,
				executionTransaction,
		...
		return {
			...
			result: await visualizeSimulatorState(
				governanceContractSimulationState,
				ethereum,
				tokenPriceService,
				undefined,
			),
...
export const simulateGnosisSafeMetaTransaction = async (
...
> => {
	...
	try {
		...
		*/
			...
			...(isDelegateCall
				? {
						...
						input: stringToUint8Array(
							encodeFunctionCall(
								delegateCallExecuteAbi,
								'delegateCallExecute',
								[
									addressString(gnosisSafeMessage.to.address),
									dataStringWith0xStart(
										gnosisSafeMessage.parsedMessageData.input,
									),
								],
							),
						),
		...
		const simulationState = await getUpdatedSimulationState(
			ethereumClientService,
		)
		if (simulationState.kind === 'passthrough')
			throw new Error(
				'Failed to fetch simulation state for Gnosis Safe transaction.',
			)
		if (simulationState.value.success === false)
			throw new JsonRpcResponseError(simulationState.value.jsonRpcError)
		const resolvedSimulationState = simulationState.value
		const gasLimit =
			gnosisSafeMessage.message.message.baseGas !== 0n
				? {
				...
				: await (async () => {
						const estimateGas = await simulateEstimateGasFromInput(
							ethereumClientService,
							undefined,
							toResolvedSimulationInput(simulationInput),
							transactionWithoutGas,
						)
						if ('error' in estimateGas)
							throw new Error(estimateGas.error.message)
		...
		const getTemporaryAccountOverrides = async () => {
			...
			const gnosisSafeCode = await getSimulatedCode(
				ethereumClientService,
				undefined,
				{ kind: 'simulated', value: resolvedSimulationState },
				gnosisSafeMessage.verifyingContract.address,
			)
			if (gnosisSafeCode?.getCodeReturn === undefined)
				throw new Error(
					'Failed to simulate gnosis safe transaction. Could not retrieve gnosis safe code.',
				)
			return {
				[addressString(gnosisSafeMessage.verifyingContract.address)]: {
					code: getGnosisSafeProxyProxy(),
				},
				[addressString(ORIGINAL_GNOSIS_SAFE)]: {
					code: gnosisSafeCode.getCodeReturn,
				},
			}
		}
		const temporaryAccountOverrides = await getTemporaryAccountOverrides()
		const simulationStateAfterGnosisSafeMetaTransaction =
			await appendTransactionToInputAndSimulate(
				ethereumClientService,
				undefined,
				simulationInput,
				[metaTransaction],
				undefined,
				temporaryAccountOverrides,
			)
		return {
			success: true as const,
			result: await visualizeSimulatorState(
				simulationStateAfterGnosisSafeMetaTransaction,
				ethereumClientService,
				tokenPriceService,
				undefined,
			),
...
export const updateSimulationMetadata = async (
	...
	return await updatePopupVisualisationWithCallBack(async (prevState) => {
		...
		try {
			const eventsForEachBlockAndTransactionPromise =
				silenceChromeUnCaughtPromise(
					promiseAllMapAbortSafe(
						prevState.simulationState.value.simulatedBlocks,
						async (block) =>
							promiseAllMapAbortSafe(
								block.simulatedTransactions,
								async (simulatedTransaction) =>
									simulatedTransaction.ethSimulateV1CallResult.status ===
									'failure'
										? []
										: await parseEvents(
			...
			const metadata = await getMetadataForSimulation(
				prevState.simulationState.value,
				ethereum,
				requestAbortController,
				events,
				inputData,
...
export const prepareSimulationInputForRpc = async (
	simulationInput: SimulationStateInput,
	ethereum: EthereumClientService,
) => {
	const parentBlock = await ethereum.getBlock(undefined)
	const baseFeeFixedInputStateBlocks =
		parentBlock === undefined
			? simulationInput
			: simulationInput.map((block) =>
					modifyObject(block, {
						transactions: getBaseFeeAdjustedTransactions(
							parentBlock,
							block.transactions,
						),
					}),
				)
	const nonceFixed = await getNonceFixedSimulationStateInput(
		ethereum,
		undefined,
		baseFeeFixedInputStateBlocks,
	)
	return nonceFixed.nonceFixed
		? nonceFixed.simulationStateInput
		: baseFeeFixedInputStateBlocks
}

export const buildSimulationStateFromPreparedInput = async (
	preparedSimulationInput: SimulationStateInput,
	ethereum: EthereumClientService,
) => {
...
export const buildExecutionSimulationStateFromPreparedInput = async (
	preparedSimulationInput: SimulationStateInput,
	ethereum: EthereumClientService,
): Promise<ExecutionSimulationState> => {
	return await createExecutionSimulationState(
		ethereum,
		undefined,
		preparedSimulationInput,
	)
...
export const createSimulationStateWithNonceAndBaseFeeFixing = async (
	simulationInput: SimulationStateInput,
	ethereum: EthereumClientService,
) => {
	return await buildSimulationStateFromPreparedInput(
		await prepareSimulationInputForRpc(simulationInput, ethereum),
		ethereum,
	)
}

export async function visualizeSimulatorState(
	simulationState: SimulationState,
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	requestAbortController: AbortController | undefined,
): Promise<VisualizedSimulatorState> {
	const getWeth = async (): Promise<Erc20TokenEntry | undefined> => {
		...
		const entry = await identifyAddress(
			ethereum,
			requestAbortController,
			wethAddr,
		)
				...
				return await promiseAllMapAbortSafe(
					...
					async (transaction) =>
						await parseInputData(
							{
								to: transaction.signedTransaction.to,
								input: transaction.signedTransaction.input,
								value: transaction.signedTransaction.value,
							},
							ethereum,
							requestAbortController,
						),
	...
	if (simulationState.success === false) {
		const parsedInputDataForEachBlockAndTransaction =
			await parsedInputDataForEachBlockAndTransactionPromise
		const updatedMetadata = await getMetadataForSimulation(
			simulationState,
			ethereum,
			requestAbortController,
			[],
			parsedInputDataForEachBlockAndTransaction.flat(),
		)
		const refreshedSimulationState = modifyObject(simulationState, {
			simulationConductedTimestamp: new Date(),
		})

		const visualizedBlocks = await promiseAllMapAbortSafe(
			simulationState.simulationStateInput,
			async (block, blockIndex) => {
				const parsedInputDataForBlock =
					parsedInputDataForEachBlockAndTransaction[blockIndex]
				if (parsedInputDataForBlock === undefined)
					throw new Error('Block index overflow')

				return {
					visualizedPersonalSignRequests: await promiseAllMapAbortSafe(
						block.signedMessages,
						(signedMessage) =>
							silenceChromeUnCaughtPromise(
					...
					simulatedAndVisualizedTransactions: block.transactions.map(
						(transaction, index): NonSimulatedAndVisualizedTransaction => {
							...
							if (parsedInputData === undefined)
								throw new Error('Transaction index overflow')
							return {
								...transaction,
								...
								error: {
									...
									decodedErrorMessage: 'Could not simulate transaction',
								},
								parsedInputData,
								transaction: {
									...getFromAndToMetadata(
										transaction.signedTransaction,
										updatedMetadata.addressBookEntries,
									),
		...
		return {
			...
			visualizedSimulationState: {
				success: false,
				jsonRpcError: simulationState.jsonRpcError,
				visualizedBlocks,
			},
		}
	}

	const metadataRestructure = (
		metadata: AddressBookEntry & { type: 'ERC20'; decimals: bigint },
	) => ({ address: metadata.address, decimals: metadata.decimals })
	function onlyTokensAndTokensWithKnownDecimals(
		metadata: AddressBookEntry,
	): metadata is AddressBookEntry & { type: 'ERC20'; decimals: `0x${string}` } {
		return (
			metadata.type === 'ERC20' &&
			metadata.decimals !== undefined &&
			metadata.address !== ETHEREUM_LOGS_LOGGER_ADDRESS
		)
	}
		...
		async (block, blockIndex) => {
			...
			return await promiseAllMapAbortSafe(
				...
				async (transaction, transactionIndex) => {
					const slicedSimulationState = sliceSimulationState(
						simulationState,
						blockIndex,
						transactionIndex,
					)
					return await runProtectorsForTransaction(
						slicedSimulationState,
						transaction,
						ethereum,
						requestAbortController,
					)
	...
	const eventsForEachBlockAndTransaction =
		await eventsForEachBlockAndTransactionPromise
	const parsedInputDataForEachBlockAndTransaction =
		await parsedInputDataForEachBlockAndTransactionPromise
	const updatedMetadata = await getMetadataForSimulation(
		simulationState,
		ethereum,
		requestAbortController,
		eventsForEachBlockAndTransaction.flat(),
		parsedInputDataForEachBlockAndTransaction.flat(),
	)

	const tokenPriceEstimatesPromise =
		weth === undefined
			? []
			: silenceChromeUnCaughtPromise(
					tokenPriceService.estimateEthereumPricesForTokens(
						requestAbortController,
						weth,
						updatedMetadata.addressBookEntries
							.filter(onlyTokensAndTokensWithKnownDecimals)
							.map(metadataRestructure),
					),
				)

	const protectorsForEachBlockAndTransaction =
		await protectorsForEachBlockAndTransactionPromise
	...
	const visualizedBlocks = await promiseAllMapAbortSafe(
		simulationState.simulatedBlocks,
		async (block, blockIndex) => {
			const eventsForEachTransaction =
				eventsForEachBlockAndTransaction[blockIndex]
			const parsedInputDataForBlock =
				parsedInputDataForEachBlockAndTransaction[blockIndex]
			const protectorsForBlock =
				protectorsForEachBlockAndTransaction[blockIndex]
			if (
				eventsForEachTransaction === undefined ||
				parsedInputDataForBlock === undefined ||
				protectorsForBlock === undefined
			)
				throw new Error('Block index overflow')
			return {
				visualizedPersonalSignRequests: await promiseAllMapAbortSafe(
				...
				simulatedAndVisualizedTransactions:
					formSimulatedAndVisualizedTransactions(
						...
						simulationState.rpcNetwork,
						parsedInputDataForBlock,
						protectorsForBlock,
						updatedMetadata.addressBookEntries,
						updatedMetadata.namedTokenIds,
						updatedMetadata.ens,
						tokenPriceEstimates,
						weth,
					),
