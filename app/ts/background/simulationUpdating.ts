...
export const getCurrentSimulationInput = async (): Promise<SimulationStateInput> => {
	const [settings, preSimulationBlockTimeManipulation] = await Promise.all([getSettings(), getPreSimulationBlockTimeManipulation()])
...
export async function getMetadataForSimulation(simulationState: SimulationState, ethereum: EthereumClientService, requestAbortController: AbortController | undefined, eventsForEachTransaction: readonly EnrichedEthereumEvents[], inputData: readonly EnrichedEthereumInputData[]) {
...
export const getGovernanceExecutionSimulationInput = (simulationInput: SimulationStateInput, executionTransaction: PreSimulationTransaction, executionTimestamp: Date, executionStateOverrides: StateOverrides): SimulationStateInput => {
	return [
		...simulationInput,
		{
...
export const getGovernanceExecutionTokenBalancesAfter = async (
	...
	const simulationInputAfterExecution = getGovernanceExecutionSimulationInput(simulationInput, executionTransaction, executionTimestamp, executionStateOverrides)
	return await getTokenBalancesAfterForTransaction(ethereum, undefined, simulationInputAfterExecution, callResult, executionTransaction.signedTransaction.from)
}
...
export const simulateGovernanceContractExecution = async (pendingTransaction: PendingTransaction, ethereum: EthereumClientService, tokenPriceService: TokenPriceService): Promise<DistributiveOmit<SimulateExecutionReplyData, 'transactionOrMessageIdentifier'>> => {
	...
	try {
		...
		if (explanation !== 'Cast Vote' && explanation !== 'Submit Vote' && explanation !== 'Cast Vote by Signature' && explanation !== 'Cast Vote with Reason' && explanation !== 'Cast Vote with Reason and Additional Info' && explanation !== 'Cast Vote with Reason And Additional Info by Signature')
		...
		const governanceExecutionSimulationInput = getGovernanceExecutionSimulationInput(simulationInput, executionTransaction, contractExecutionResult.executionTimestamp, contractExecutionResult.executionStateOverrides)
		const tokenBalancesAfter = await getGovernanceExecutionTokenBalancesAfter(ethereum, simulationInput, executionTransaction, contractExecutionResult.executionTimestamp, contractExecutionResult.executionStateOverrides, contractExecutionResult.ethSimulateV1CallResult)
			...
			simulatedBlocks: [
				{
					...
					blockTimeManipulation: {
						type: 'SetTimetamp',
						timeToSet: dateToBigintSeconds(contractExecutionResult.executionTimestamp),
					},
		...
		return {
			...
			result: await visualizeSimulatorState(governanceContractSimulationState, ethereum, tokenPriceService, undefined),
		}
	} catch (error) {
		console.warn(error)
...
export const simulateGnosisSafeMetaTransaction = async (
...
): Promise<DistributiveOmit<SimulateExecutionReplyData, 'transactionOrMessageIdentifier'>> => {
	const returnError = (errorMessage: string) => ({
		success: false as const,
		errorType: 'Other' as const,
		errorMessage,
	...
	try {
		...
		*/
		...
		return {
			success: true as const,
			result: await visualizeSimulatorState(simulationStateAfterGnosisSafeMetaTransaction, ethereumClientService, tokenPriceService, undefined),
		}
	} catch (error) {
		console.warn(error)
		if (error instanceof Error) return returnError(error.message)
	...
	const baseFeeFixedInputStateBlocks =
		parentBlock === undefined
			...
			: simulationInput.map((block) =>
					modifyObject(block, {
						transactions: getBaseFeeAdjustedTransactions(parentBlock, block.transactions),
					}),
				)
...
export async function visualizeSimulatorState(simulationState: SimulationState, ethereum: EthereumClientService, tokenPriceService: TokenPriceService, requestAbortController: AbortController | undefined): Promise<VisualizedSimulatorState> {
	...
	const parsedInputDataForEachBlockAndTransactionPromise = promiseAllMapAbortSafe(simulationState.simulationStateInput, async (block) => {
	...
	const metadataRestructure = (metadata: AddressBookEntry & { type: 'ERC20'; decimals: bigint }) => ({
	...
	const visualizedBlocks = await promiseAllMapAbortSafe(simulationState.simulatedBlocks, async (block, blockIndex) => {
		...
		return {
			visualizedPersonalSignRequests: await promiseAllMapAbortSafe(block.signedMessages, (signedMessage) => silenceChromeUnCaughtPromise(craftPersonalSignPopupMessage(ethereum, requestAbortController, signedMessage, settings.activeRpcNetwork))),
			simulatedAndVisualizedTransactions: formSimulatedAndVisualizedTransactions(
