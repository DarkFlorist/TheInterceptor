...
import { assertSafeMessageSignatureValid, createSafeMessageTypedData } from '../safe/safeMessage.js'
...
export type SafeAppsMessageServices = {
	readonly submit: (message: string, signature: string) => Promise<{ messageHash: string }>
...
async function requestMessageService(path: string, body?: { message?: string, signature: string }, allowMissing = false) {
...
export function createSafeAppsMessageServices(ethereum: EthereumClientService, safeAddress: bigint, chainId: bigint): SafeAppsMessageServices {
	...
	const hashMessage = (message: string) => hashTypedData(createSafeMessageTypedData(chainId, safeAddress, message))
	return {
		async submit(message, signature) {
			...
			if (existing !== undefined && (existing.messageHash !== messageHash || existing.message !== message)) throw safeAppsServiceError('The Safe message service returned a different message.')
			if (existing === undefined) await requestMessageService(`chains/${ chainId.toString() }/safes/${ addressString(safeAddress) }/messages`, { message, signature: owner.signature })
		...
		async getSignature(messageHash) {
			...
			await assertSafeMessageSignatureValid(ethereum, safeAddress, stored.message, signature, snapshot.blockNumber)
			return signature
		},
	}
}
