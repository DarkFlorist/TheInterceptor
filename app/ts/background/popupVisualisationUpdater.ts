...
function buildPassthroughVisualizedState(simulationId: number, numberOfAddressesMadeRich: number, simulationResultState: 'done' | 'corrupted' = 'done'): CompleteVisualizedSimulation {
	return createPassthroughCompleteVisualizedSimulation(simulationId, simulationResultState, numberOfAddressesMadeRich)
}

function buildDefinedEmptyVisualizedState(simulationState: Extract<SimulationState, { success: true }>, simulationId: number, numberOfAddressesMadeRich: number): CompleteVisualizedSimulation {
	return {
...
const hasSimulationInputOperations = (simulationState: SimulationState) => simulationState.simulationStateInput.some((block) => block.transactions.length > 0 || block.signedMessages.length > 0)
...
export const updatePopupVisualisationIfNeeded = async (ethereum: EthereumClientService, tokenPriceService: TokenPriceService, invalidateOldState = false, onlyIfNotAlreadyUpdating = false, skipIfUnchanged = false) => {
	try {
		...
		if (skipIfUnchanged && popupVisualisation.simulationState.kind === 'simulated') {
			...
			const cachedFingerprint = getPopupVisualisationFingerprint(popupVisualisation.simulationState.value.simulationStateInput, popupVisualisation.simulationState.value.rpcNetwork, popupVisualisation.simulationState.value.blockNumber)
	...
	return await updateSimulationVisualisationSemaphore
		.execute(async () => {
			...
			if (!hasSimulationInputOperations(simulationState.value)) {
				const newState = simulationState.value.success ? buildDefinedEmptyVisualizedState(simulationState.value, simulationId, numberOfAddressesMadeRich) : buildPassthroughVisualizedState(simulationId, numberOfAddressesMadeRich)
				await setPopupVisualisationState(newState)
				await sendPopupMessageToOpenWindows({
					method: 'popup_simulation_state_changed',
					data: { visualizedSimulatorState: newState },
				...
				const getUpdatedState = async () => {
					if (ethereum.getChainId() === simulationState.value.rpcNetwork.chainId) {
						...
						return await setPopupVisualisationState({
							...refreshed,
							...doneState,
							simulationState: toResolvedSimulationState(refreshed.simulationState),
							numberOfAddressesMadeRich,
						})
					}
