...
function buildPassthroughVisualizedState(
	...
	return createPassthroughCompleteVisualizedSimulation(
		simulationId,
...
const hasSimulationInputOperations = (simulationState: SimulationState) =>
	simulationState.simulationStateInput.some(
		(block) => block.transactions.length > 0 || block.signedMessages.length > 0,
	)

export const updatePopupVisualisationIfNeeded = async (
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	invalidateOldState = false,
	...
	try {
		...
		if (
			...
			updateSimulationVisualisationSemaphore.getPermits() === 0
		)
			return popupVisualisation
		if (
			onlyIfNotAlreadyUpdating &&
			popupVisualisation.simulationState.kind === 'simulated'
		) {
			const ageSeconds =
				(Date.now() -
		...
		if (!(isMainPopupWindowOpenReply?.data.isOpen === true))
			return popupVisualisation
		if (
			skipIfUnchanged &&
			popupVisualisation.simulationState.kind === 'simulated'
		) {
			const currentSimulationInput =
				await getCurrentSimulationStateInput(ethereum)
			const currentFingerprint = getPopupVisualisationFingerprint(
				currentSimulationInput.simulationStateInput,
				currentSimulationInput.rpcNetwork,
				currentSimulationInput.blockNumber,
			)
		...
		if (invalidateOldState) {
			...
			const visualizedSimulatorState = await setPopupVisualisationState(
				modifyObject(popupVisualisation, {
					simulationId,
...
export async function updatePopupVisualisationState(
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	abortController: AbortController | undefined,
	throwOnUnexpectedError = false,
) {
	return await updateSimulationVisualisationSemaphore
		.execute(async () => {
			...
			const numberOfAddressesMadeRich = (await getAddressesbeingMadeRich())
				.length
			if (simulationState.kind === 'passthrough') {
				const newState = buildPassthroughVisualizedState(
					simulationId,
					numberOfAddressesMadeRich,
				)
			...
			const changedMessagePromise = silenceChromeUnCaughtPromise(
				sendPopupMessageToOpenWindows({
					method: 'popup_simulation_state_changed',
					data: { visualizedSimulatorState },
				}),
			...
			try {
				const getUpdatedState = async () => {
					if (
						ethereum.getChainId() === simulationState.value.rpcNetwork.chainId
					) {
						const refreshed = await visualizeSimulatorState(
							simulationState.value,
							ethereum,
							tokenPriceService,
							abortController,
						...
						return await setPopupVisualisationState({
							...refreshed,
							...doneState,
							simulationState: toResolvedSimulationState(
								refreshed.simulationState,
							),
							numberOfAddressesMadeRich,
						})
					}
				...
				if (error instanceof Error && isFailedToFetchError(error)) {
					...
					await sendPopupMessageToOpenWindows({
						method: 'popup_simulation_state_changed',
						data: { visualizedSimulatorState: state },
					})
					return
				}
