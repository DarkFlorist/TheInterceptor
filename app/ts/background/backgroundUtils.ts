...
function isIgnorableClosedMessageChannelError(error: Error) {
	return error.message?.includes('A listener indicated an asynchronous response by returning true, but the message channel closed before a response was received') || error.message?.includes('The message port closed before a response was received')
}
...
export async function getActiveAddressesForAllTabs(settings: Settings) {
	...
	return Promise.all(
		tabStates.map(async (state) => ({
			tabId: state.tabId,
			activeAddress: state.activeSigningAddress === undefined ? undefined : await getActiveAddressEntry(state.activeSigningAddress),
		})),
	)
}
...
export async function setExtensionIcon(details: browser.action._SetIconDetails) {
	...
	if (manifest.manifest_version === 2) {
		...
		await (browser.action.setIcon as unknown as (details: browser.action._SetIconDetails, callback: () => void) => Promise<void>)(details, () => {
...
export async function setExtensionBadgeText(details: browser.browserAction._SetBadgeTextDetails) {
	try {
		...
		if (manifest.manifest_version === 2) {
			...
			// see https://issues.chromium.org/issues/337214677
			await (browser.action.setBadgeText as unknown as (details: browser.browserAction._SetBadgeTextDetails, callback: () => void) => Promise<void>)(details, () => {
				browser.runtime.lastError
			})
		}
		checkAndThrowRuntimeLastError()
...
export async function setExtensionBadgeBackgroundColor(details: browser.action._SetBadgeBackgroundColorDetails) {
	try {
		...
		if (manifest.manifest_version === 2) {
			...
			// see https://issues.chromium.org/issues/337214677
			await (browser.action.setBadgeBackgroundColor as unknown as (details: browser.action._SetBadgeBackgroundColorDetails, callback: () => void) => Promise<void>)(details, () => {
				browser.runtime.lastError
			})
		}
		checkAndThrowRuntimeLastError()
