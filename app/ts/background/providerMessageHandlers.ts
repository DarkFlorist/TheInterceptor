...
export async function ethAccountsReply(
	...
	const returnValue = {
		...
		method: 'eth_accounts_reply' as const,
		result: '0x' as const,
	}
	...
	sendPopupMessageToOpenWindows({
		...
		data: { tabId: port.sender.tab.id, activeSigningAddress },
	})
	if (socket)
		sendInternalWindowMessage({
			method: 'window_signer_accounts_changed',
...
export async function walletSwitchEthereumChainReply(
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	resetSimulationServices: ResetSimulationServices,
	websiteTabConnections: WebsiteTabConnections,
...
export async function connectedToSigner(
	...
	if (!settings.simulationMode || settings.useSignersAddressAsActiveAddress) {
		sendSubscriptionReplyOrCallBackToPort(port, {
			type: 'result',
	...
	switch (params.forwardRequest.method) {
		...
		case 'eth_sendTransaction': {
			const uniqueRequestIdentifier = {
				requestId: params.forwardRequest.requestId,
				requestSocket: request.uniqueRequestIdentifier.requestSocket,
			}
			if (params.success) {
				try {
