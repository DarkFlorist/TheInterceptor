...
async function addMetadataToOpenSeaOrder(ethereumClientService: EthereumClientService, requestAbortController: AbortController | undefined, openSeaOrder: OpenSeaOrderMessage) {
	return {
		...
		offer: await promiseAllMapAbortSafe(openSeaOrder.offer, async (offer) => ({
			...offer,
			token: await identifyAddress(ethereumClientService, requestAbortController, offer.token),
		})),
		consideration: await promiseAllMapAbortSafe(openSeaOrder.consideration, async (offer) => ({
			...offer,
			token: await identifyAddress(ethereumClientService, requestAbortController, offer.token),
			recipient: await identifyAddress(ethereumClientService, requestAbortController, offer.recipient),
		})),
	}
}
