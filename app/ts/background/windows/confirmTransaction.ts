...
import {
	...
	ERROR_INTERCEPTOR_NO_ACTIVE_ADDRESS,
	METAMASK_ERROR_BLANKET_ERROR,
	METAMASK_ERROR_FAILED_TO_PARSE_REQUEST,
	METAMASK_ERROR_USER_REJECTED_REQUEST,
...
import {
	type InterceptorTransactionStack,
	PASSTHROUGH_STATE,
	type WebsiteCreatedEthereumUnsignedTransaction,
...
import {
	...
	getUniqueRequestIdentifierString,
...
const getSimulationStartedTimestamp = (
	popupVisualisation: TimestampedPopupVisualisation,
) => popupVisualisation.data.simulationStartedTimestamp
...
const shouldReplacePopupVisualisation = (
	...
	const currentTimestamp =
		currentPopupVisualisation === undefined
			? undefined
			: getSimulationStartedTimestamp(currentPopupVisualisation)
...
export function toPopupPendingTransactionOrSignableMessage(
	...
	switch (pending.type) {
		...
		case 'SignableMessage': {
			...
			const transactionOrMessageCreationStatus =
				pending.transactionOrMessageCreationStatus
				...
				case 'Simulated':
					return {
						...
						visualizedPersonalSignRequest:
							pending.visualizedPersonalSignRequest,
					}
				case 'Crafting':
				case 'Simulating':
					return {
...
export async function updateConfirmTransactionView(
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	onlyIfNotAlreadyUpdating = false,
) {
	try {
		...
		const message: UpdateConfirmTransactionDialog = {
			...
			data: {
				...
				visualizedSimulatorState: (await settings).simulationMode
					? await visualizedSimulatorStatePromise
					: createPassthroughCompleteVisualizedSimulation(),
			},
		}
		const messagePendingTransactions: UpdateConfirmTransactionDialogPendingTransactions =
			{
				...
				data: {
					pendingTransactionAndSignableMessages:
						pendingTransactionAndSignableMessages.map(
...
const getPendingTransactionOrMessageByidentifier = async (
	uniqueRequestIdentifier: UniqueRequestIdentifier,
) => {
	return (await getPendingTransactionsAndMessages()).find((tx) =>
		doesUniqueRequestIdentifiersMatch(
			tx.uniqueRequestIdentifier,
			uniqueRequestIdentifier,
		),
	)
}

export const setGasLimitForTransaction = async (
	transactionIdentifier: BigInt,
	gasLimit: bigint,
) => {
	const pendingTransaction = (await getPendingTransactionsAndMessages()).find(
		(tx) =>
			tx.type === 'Transaction' &&
			tx.transactionIdentifier === transactionIdentifier,
	)
	if (pendingTransaction === undefined) {
		const theTransactionIsAlreadyInStack = (
			await getInterceptorTransactionStack()
		).operations.some(
			(transaction) =>
		...
		await updateInterceptorTransactionStack(
			(prevStack: InterceptorTransactionStack) => {
				return {
					operations: prevStack.operations.map((operation) => {
						...
						if (
							...
							return operation
						if (
							operation.preSimulationTransaction.originalRequestParameters
								.method !== 'eth_sendTransaction'
						)
							return operation
						const originalParams =
							operation.preSimulationTransaction.originalRequestParameters
						const originalRequestParameters = modifyObject(originalParams, {
							params: [
								modifyObject(originalParams.params[0], { gas: gasLimit }),
							],
						})
							...
							preSimulationTransaction: modifyObject(
								operation.preSimulationTransaction,
								{
									...
									signedTransaction: modifyObject(
										operation.preSimulationTransaction.signedTransaction,
										{ gas: gasLimit },
									),
								},
							),
	...
	await updatePendingTransactionOrMessage(
		pendingTransaction.uniqueRequestIdentifier,
		async (transaction) => {
			if (
				transaction.originalRequestParameters.method === 'eth_sendTransaction'
			) {
				const originalRequestParameters = modifyObject(
					transaction.originalRequestParameters,
					{
						params: [
							modifyObject(transaction.originalRequestParameters.params[0], {
								gas: gasLimit,
							}),
						],
					},
...
export async function resolvePendingTransactionOrMessage(
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	websiteTabConnections: WebsiteTabConnections,
	confirmation: TransactionConfirmation,
) {
	const pendingTransactionOrMessage =
		await getPendingTransactionOrMessageByidentifier(
	...
	const reply = (
		message:
			...
			| { type: 'result'; error: { code: number; message: string } }
			| { type: 'result'; result: unknown },
	) => {
		if (message.type === 'result' && !('error' in message)) {
			if (
				pendingTransactionOrMessage.originalRequestParameters.method ===
				...
				return replyToInterceptedRequest(websiteTabConnections, {
					...
					...message,
					result: EthereumBytes32.parse(message.result),
			...
			return replyToInterceptedRequest(websiteTabConnections, {
				...pendingTransactionOrMessage.originalRequestParameters,
				...message,
				result: funtypes.String.parse(message.result),
				uniqueRequestIdentifier: confirmation.data.uniqueRequestIdentifier,
			})
		}
		return replyToInterceptedRequest(websiteTabConnections, {
	...
	if (
		...
		pendingTransactionOrMessage.simulationMode === false
	) {
		await updatePendingTransactionOrMessage(
			confirmation.data.uniqueRequestIdentifier,
			async (transaction) =>
	...
	await removePendingTransactionOrMessage(
		confirmation.data.uniqueRequestIdentifier,
	)
	if ((await getPendingTransactionsAndMessages()).length === 0)
		await tryFocusingTabOrWindow({
			type: 'tab',
			id: pendingTransactionOrMessage.uniqueRequestIdentifier.requestSocket
				.tabId,
		})
	if (!(await updateConfirmTransactionView(ethereum, tokenPriceService)))
		await closePopupOrTabById(pendingTransactionOrMessage.popupOrTabId)
	...
	if (
		pendingTransactionOrMessage === undefined ||
		pendingTransactionOrMessage.transactionOrMessageCreationStatus !==
			'Simulated'
	)
		return reply(
			formRejectMessage(
				METAMASK_ERROR_BLANKET_ERROR,
				'The Interceptor failed to process the transaction',
			),
		)
	if (confirmation.data.action === 'reject')
		return reply(
			formRejectMessage(
				METAMASK_ERROR_USER_REJECTED_REQUEST,
				'User denied transaction signature',
	...
	if (confirmation.data.action === 'signerIncluded')
		throw new Error('Signer included transaction that was in simulation')
	...
	switch (pendingTransactionOrMessage.type) {
					...
					operations: [
						...prevStack.operations,
						{
							...
							signedMessageTransaction:
								pendingTransactionOrMessage.signedMessageTransaction,
						},
					],
				}),
			)
			...
			return reply({
				...
				result: (
					await simulatePersonalSign(
						pendingTransactionOrMessage.originalRequestParameters,
						pendingTransactionOrMessage.signedMessageTransaction.fakeSignedFor,
					)
				).signature,
			})
		}
		case 'Transaction': {
			const signedTransaction = mockSignTransaction(
				pendingTransactionOrMessage.transactionToSimulate.transaction,
			)
			const transaction = {
				...pendingTransactionOrMessage.transactionToSimulate,
				signedTransaction,
			}
			await updateInterceptorTransactionStack(
				(prevStack: InterceptorTransactionStack) => ({
					operations: [
						...prevStack.operations,
						{
							...
							preSimulationTransaction: transaction,
						},
					],
				}),
			)
			await updatePopupVisualisationIfNeeded(ethereum, tokenPriceService, false)
			markPerformance(
				POPUP_PERFORMANCE_MARKS.backgroundTransactionStackAppended,
			)
			return reply({
				type: 'result',
				result: EthereumBytes32.serialize(signedTransaction.hash),
			})
		}
		default:
			assertNever(pendingTransactionOrMessage)
	}
}

export const onCloseWindowOrTab = async (
	popupOrTabs: PopupOrTabId,
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	websiteTabConnections: WebsiteTabConnections,
) => {
	// check if user has closed the window on their own, if so, reject all signatures
	const transactions = await getPendingTransactionsAndMessages()
	const [firstTransaction] = transactions
	if (
		firstTransaction === undefined ||
		firstTransaction?.popupOrTabId.type !== popupOrTabs.type ||
		firstTransaction.popupOrTabId.id !== popupOrTabs.id
	)
		return
	await resolveAllPendingTransactionsAndMessageAsNoResponse(
		transactions,
		ethereum,
		tokenPriceService,
...
const resolveAllPendingTransactionsAndMessageAsNoResponse = async (
	transactions: readonly PendingTransactionOrSignableMessage[],
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	websiteTabConnections: WebsiteTabConnections,
) => {
...
const isSerializedEip1559Transaction = (
	transaction: `0x${string}`,
): transaction is `0x02${string}` => transaction.startsWith('0x02')
const recoverSerializedEip1559TransactionAddress = async (
	serializedTransaction: `0x02${string}`,
) => {
	const parsedTransaction = parseSerializedTransaction(serializedTransaction)
	if (parsedTransaction.type !== 'eip1559')
		throw new Error('Expected EIP-1559 transaction')
	if (
		parsedTransaction.chainId === undefined ||
		parsedTransaction.gas === undefined ||
		parsedTransaction.maxFeePerGas === undefined ||
		parsedTransaction.maxPriorityFeePerGas === undefined ||
		parsedTransaction.nonce === undefined ||
		parsedTransaction.r === undefined ||
		parsedTransaction.s === undefined
	) {
		throw new Error(
			'Serialized transaction is missing required signature fields',
		)
	}
...
export const formSendRawTransaction = async (
	ethereumClientService: EthereumClientService,
	sendRawTransactionParams: SendRawTransactionParams,
	website: Website,
	created: Date,
	transactionIdentifier: EthereumQuantity,
): Promise<WebsiteCreatedEthereumUnsignedTransaction> => {
	const serializedTransaction = dataStringWith0xStart(
		sendRawTransactionParams.params[0],
	)
	const parsedTransaction = parseSerializedTransaction(serializedTransaction)
	if (parsedTransaction.type !== 'eip1559')
		throw new Error('No support for non-1559 transactions')
	if (!isSerializedEip1559Transaction(serializedTransaction))
		throw new Error('Expected serialized EIP-1559 transaction')
	const from = await recoverSerializedEip1559TransactionAddress(
		serializedTransaction,
	)
	if (parsedTransaction.gas === undefined)
		throw new Error('Unable to parse gas from serialized transaction')
	if (parsedTransaction.nonce === undefined)
		throw new Error('Unable to parse nonce from serialized transaction')
	const transactionDetails = {
		from: EthereumAddress.parse(from),
		input: stringToUint8Array(parsedTransaction.data ?? '0x'),
		gas: parsedTransaction.gas,
		value: parsedTransaction.value,
		...(parsedTransaction.to === undefined || parsedTransaction.to === null
			? {}
			: { to: EthereumAddress.parse(parsedTransaction.to) }),
		...(parsedTransaction.maxPriorityFeePerGas === undefined
			? {}
			: { maxPriorityFeePerGas: parsedTransaction.maxPriorityFeePerGas }),
		...(parsedTransaction.maxFeePerGas === undefined
	...
	if (transactionDetails.maxFeePerGas === undefined)
		throw new Error('No support for non-1559 transactions')
...
export const formEthSendTransaction = async (
	ethereumClientService: EthereumClientService,
	...
	transactionIdentifier: EthereumQuantity,
	simulationMode = true,
): Promise<WebsiteCreatedEthereumUnsignedTransactionOrFailed> => {
	const simulationState = simulationMode
		? await getUpdatedSimulationState(ethereumClientService)
		: PASSTHROUGH_STATE
	const parentBlockPromise = silenceChromeUnCaughtPromise(
		ethereumClientService.getBlock(requestAbortController),
	) // we are getting the real block here, as we are not interested in the current block where this is going to be included, but the parent
	const transactionDetails = sendTransactionParams.params[0]
	if (activeAddress === undefined)
		throw new Error('Access to active address is denied')
	const from =
		simulationMode && transactionDetails.from !== undefined
			? transactionDetails.from
			: activeAddress
	const transactionCountPromise = silenceChromeUnCaughtPromise(
		getSimulatedTransactionCount(
	...
	const maxPriorityFeePerGas =
		transactionDetails.maxPriorityFeePerGas !== undefined &&
		transactionDetails.maxPriorityFeePerGas !== null
			? transactionDetails.maxPriorityFeePerGas
			: 10n ** 8n // 0.1 nanoEth/gas
	const transactionWithoutGas = {
		...
		maxFeePerGas:
			transactionDetails.maxFeePerGas !== undefined &&
			transactionDetails.maxFeePerGas !== null
				? transactionDetails.maxFeePerGas
				: parentBlock.baseFeePerGas * 2n + maxPriorityFeePerGas,
		maxPriorityFeePerGas,
		to: transactionDetails.to === undefined ? null : transactionDetails.to,
		value:
			transactionDetails.value !== undefined ? transactionDetails.value : 0n,
	...
	if (transactionDetails.gas === undefined) {
		try {
			const estimateGas = await simulateEstimateGas(
				ethereumClientService,
				requestAbortController,
				simulationState,
			...
			if ('error' in estimateGas)
				return { ...extraParams, ...estimateGas, success: false }
			return {
			...
			return {
				...extraParams,
				error: { code: 123456, message: 'Unknown Error', data: '0x' },
				success: false,
			}
		}
	}
...
const getPendingTransactionWindow = async (
	...
	if (firstPendingTransaction !== undefined) {
		const alreadyOpenWindow = await getPopupOrTabById(
			firstPendingTransaction.popupOrTabId,
		)
...
export async function openConfirmTransactionDialogForMessage(
	...
	if (activeAddress === undefined)
		return { type: 'result' as const, ...ERROR_INTERCEPTOR_NO_ACTIVE_ADDRESS }
	const uniqueRequestIdentifierString = getUniqueRequestIdentifierString(
		request.uniqueRequestIdentifier,
	)
	const messageIdentifier = EthereumQuantity.parse(
		keccak256(stringToBytes(uniqueRequestIdentifierString)),
	)
	...
	try {
		const visualizedPersonalSignRequest = await craftPersonalSignPopupMessage(
			ethereumClientService,
			undefined,
			signedMessageTransaction,
			ethereumClientService.getRpcEntry(),
		)
		await pendingConfirmationSemaphore.execute(async () => {
			...
			if (openedDialog === undefined)
				throw new Error('Failed to get pending transaction window!')
			...
			await updateConfirmTransactionView(
				ethereumClientService,
				tokenPriceService,
			)

			await updatePendingTransactionOrMessage(
				pendingMessage.uniqueRequestIdentifier,
				async (message) => {
					if (message.type !== 'SignableMessage') return message
					return modifyObject(message, {
						transactionOrMessageCreationStatus: 'Simulating' as const,
					})
				},
			)
			await updateConfirmTransactionView(
			...
			if (visualizedPersonalSignRequest.type === 'SafeTx') {
				await simulateGnosisSafeTransactionOnPass(
					ethereumClientService,
					tokenPriceService,
					visualizedPersonalSignRequest,
				)
		...
		return formRejectMessage(
			METAMASK_ERROR_BLANKET_ERROR,
			'Failed to process message signing request. See Interceptor for error message',
		)
	}
	...
	if (pendingTransactionData === undefined)
		return formRejectMessage(
			METAMASK_ERROR_BLANKET_ERROR,
			'The Interceptor failed to process the transaction',
		)
...
export async function openConfirmTransactionDialogForTransaction(
	...
	const transactionIdentifier = EthereumQuantity.parse(
		keccak256(stringToBytes(uniqueRequestIdentifierString)),
	)
	const created = new Date()
	const transactionToSimulatePromise =
		transactionParams.method === 'eth_sendTransaction'
	...
	const outcome = await pendingConfirmationSemaphore.execute(async () => {
		try {
			...
			if (openedDialog === undefined)
				return formRejectMessage(
					METAMASK_ERROR_FAILED_TO_PARSE_REQUEST,
					'Failed to get pending transaction window',
				)
			markPerformance(
				POPUP_PERFORMANCE_MARKS.backgroundTransactionConfirmPopupOpened,
			)
			...
			markPerformance(
				POPUP_PERFORMANCE_MARKS.backgroundTransactionSimulationStart,
			)
			const simulationResultsPromise = silenceChromeUnCaughtPromise(
				refreshConfirmTransactionSimulation(
					ethereumClientService,
					tokenPriceService,
					activeAddress,
			...
			markPerformance(
				POPUP_PERFORMANCE_MARKS.backgroundTransactionSimulationEnd,
			)
			await updatePendingTransactionOrMessage(
				pendingTransaction.uniqueRequestIdentifier,
				async (transaction) => {
					...
					if (
						transaction.transactionOrMessageCreationStatus === 'Simulated' ||
						transaction.transactionOrMessageCreationStatus ===
							'FailedToSimulate'
					) {
						if (
							'popupVisualisation' in transaction &&
							!shouldReplacePopupVisualisation(
								transaction.popupVisualisation,
								popupVisualisation,
							)
						)
							return transaction
			...
			return formRejectMessage(
				METAMASK_ERROR_FAILED_TO_PARSE_REQUEST,
				'The Interceptor failed to send transaction',
			)
		}
	})
	if (!('success' in outcome))
		return formRejectMessage(outcome.error.code, outcome.error.message)
	const pendingTransactionData =
		await getPendingTransactionOrMessageByidentifier(
			request.uniqueRequestIdentifier,
		)
