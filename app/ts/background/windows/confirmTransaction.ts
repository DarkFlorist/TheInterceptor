...
import { stringToBytes, keccak256, recoverAddress, parseTransaction as parseSerializedTransaction, serializeTransaction } from '../../utils/viem.js'
...
const shouldReplacePopupVisualisation = (currentPopupVisualisation: TimestampedPopupVisualisation | undefined, nextPopupVisualisation: TimestampedPopupVisualisation) => {
...
export const setGasLimitForTransaction = async (transactionIdentifier: BigInt, gasLimit: bigint) => {
			...
			return {
				operations: prevStack.operations.map((operation) => {
					...
					const originalRequestParameters = modifyObject(originalParams, {
						params: [modifyObject(originalParams.params[0], { gas: gasLimit })],
					})
					return modifyObject(operation, {
						preSimulationTransaction: modifyObject(operation.preSimulationTransaction, {
							originalRequestParameters,
							signedTransaction: modifyObject(operation.preSimulationTransaction.signedTransaction, { gas: gasLimit }),
						}),
					})
				}),
			}
	...
	await updatePendingTransactionOrMessage(pendingTransaction.uniqueRequestIdentifier, async (transaction) => {
		if (transaction.originalRequestParameters.method === 'eth_sendTransaction') {
			const originalRequestParameters = modifyObject(transaction.originalRequestParameters, {
				params: [
...
export async function resolvePendingTransactionOrMessage(ethereum: EthereumClientService, tokenPriceService: TokenPriceService, websiteTabConnections: WebsiteTabConnections, confirmation: TransactionConfirmation) {
	...
	if (confirmation.data.action === 'accept' && pendingTransactionOrMessage.simulationMode === false) {
		await updatePendingTransactionOrMessage(confirmation.data.uniqueRequestIdentifier, async (transaction) =>
	...
	if ((await getPendingTransactionsAndMessages()).length === 0)
		await tryFocusingTabOrWindow({
			type: 'tab',
			id: pendingTransactionOrMessage.uniqueRequestIdentifier.requestSocket.tabId,
		})
	...
	switch (pendingTransactionOrMessage.type) {
		case 'SignableMessage': {
			await updateInterceptorTransactionStack((prevStack: InterceptorTransactionStack) => ({
				operations: [
					...prevStack.operations,
					{
						...
						signedMessageTransaction: pendingTransactionOrMessage.signedMessageTransaction,
					},
				],
			}))
			...
			return reply({
				type: 'result',
				result: (await simulatePersonalSign(pendingTransactionOrMessage.originalRequestParameters, pendingTransactionOrMessage.signedMessageTransaction.fakeSignedFor)).signature,
			})
		}
		case 'Transaction': {
...
export const onCloseWindowOrTab = async (popupOrTabs: PopupOrTabId, ethereum: EthereumClientService, tokenPriceService: TokenPriceService, websiteTabConnections: WebsiteTabConnections) => {
			...
			if (error instanceof Error)
				return {
					...
					error: {
						code: 123456,
						message: error.message,
						data: 'data' in error && typeof error.data === 'string' ? error.data : '0x',
					},
					success: false,
				}
			return {
...
export async function openConfirmTransactionDialogForTransaction(
	...
	const transactionToSimulatePromise =
		transactionParams.method === 'eth_sendTransaction' ? formEthSendTransaction(ethereumClientService, undefined, activeAddress, website, transactionParams, created, transactionIdentifier, simulationMode) : formSendRawTransaction(ethereumClientService, transactionParams, website, created, transactionIdentifier)
