...
export async function resolveChainChange(
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	resetSimulationServices: ResetSimulationServices,
	websiteTabConnections: WebsiteTabConnections,
	confirmation: ChainChangeConfirmation,
) {
	...
	if (
		...
		!doesUniqueRequestIdentifiersMatch(
			...
			data.request.uniqueRequestIdentifier,
		)
	)
	...
	if (resolved.error !== undefined) {
		replyToInterceptedRequest(websiteTabConnections, {
			...
			uniqueRequestIdentifier: data.request.uniqueRequestIdentifier,
		})
	} else {
		replyToInterceptedRequest(websiteTabConnections, {
			type: 'result',
...
export async function resolveSignerChainChange(
	confirmation: SignerChainChangeConfirmation,
) {
...
export const openChangeChainDialog = async (
	...
	if (openedDialog !== undefined || pendForUserReply || pendForSignerReply)
	...
	const onCloseWindowOrTab = async (popupOrTab: PopupOrTabId) => {
		...
		resolveChainChange(
			...
			websiteTabConnections,
			rejectMessage(
				await getRpcNetworkForChain(params.params[0].chainId),
...
async function resolve(
	...
	if (reply.data.accept) {
		...
		if (signerReply.data[0].accept === false)
			return { error: signerReply.data[0].error } as const // forward signers error to the application
		if (signerReply.data[0].chainId === reply.data.rpcNetwork.chainId)
			return { result: null }
	}
	return userDeniedChange
}
