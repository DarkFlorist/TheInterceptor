...
export async function updateChainChangeViewWithPendingRequest() {
	...
	if (promise) await publishPopupMessageToOpenUiPorts({ method: 'popup_ChangeChainRequest', data: promise })
	return
}

export async function resolveChainChange(simulator: Simulator, pageSessions: PageSessionStore, confirmation: ChainChangeConfirmation) {
	...
	const resolved = await resolve(simulator, pageSessions, confirmation, data.simulationMode)
	if (resolved.error !== undefined) {
		replyToInterceptedRequest(pageSessions, { type: 'result', method: 'wallet_switchEthereumChain' as const, error: resolved.error, uniqueRequestIdentifier: data.request.uniqueRequestIdentifier })
	} else {
		replyToInterceptedRequest(pageSessions, { type: 'result', method: 'wallet_switchEthereumChain' as const, result: resolved.result, uniqueRequestIdentifier: data.request.uniqueRequestIdentifier })
	}
...
export const openChangeChainDialog = async (
	...
	const onCloseWindowOrTab = async (popupOrTab: PopupOrTabId) => { // check if user has closed the window on their own, if so, reject signature
		...
		resolveChainChange(simulator, pageSessions, rejectMessage(await getRpcNetworkForChain(params.params[0].chainId), request.uniqueRequestIdentifier))
	}
	...
	try {
		...
		if (openedDialog !== undefined) {
			...
			await resolveChainChange(simulator, pageSessions, rejectMessage(await getRpcNetworkForChain(params.params[0].chainId), request.uniqueRequestIdentifier))
		}
		pendForSignerReply = undefined
		...
		return resolve(simulator, pageSessions, reply, simulationMode)
	} finally {
...
async function resolve(simulator: Simulator, pageSessions: PageSessionStore, reply: ChainChangeConfirmation, simulationMode: boolean) {
	await setChainChangeConfirmationPromise(undefined)
	if (reply.data.accept) {
		if (simulationMode) {
			await changeActiveRpc(simulator, pageSessions, reply.data.rpcNetwork, simulationMode)
			return { result: null }
		}
