...
const importSimulationStackFailure = (message: string): ImportSimulationStackReply => ({
	type: 'ImportSimulationStackReply',
	ok: false,
	message,
})
...
export async function removeAddressBookEntry(ethereum: EthereumClientService, tokenPriceService: TokenPriceService, resetSimulationServices: ResetSimulationServices, websiteTabConnections: WebsiteTabConnections, removeAddressBookEntry: RemoveAddressBookEntry) {
	await updateUserAddressBookEntries((previousContacts) => previousContacts.filter((contact) => !(contact.address === removeAddressBookEntry.data.address && (contact.chainId === removeAddressBookEntry.data.chainId || (contact.chainId === undefined && removeAddressBookEntry.data.chainId === 1n)))))
...
export async function changeInterceptorAccess(ethereum: EthereumClientService, tokenPriceService: TokenPriceService, resetSimulationServices: ResetSimulationServices, websiteTabConnections: WebsiteTabConnections, accessChange: ChangeInterceptorAccess) {
	await updateWebsiteAccess((previousAccess) => {
		...
		for (const change of accessChange.data) {
			const hostname = getHostnameForWebsiteOrigin(change.newEntry.website.websiteOrigin)
			const previousOverrides = hostScopedOverrides.get(hostname)
			hostScopedOverrides.set(hostname, {
				interceptorDisabled: change.newEntry.interceptorDisabled ?? previousOverrides?.interceptorDisabled,
				declarativeNetRequestBlockMode: change.newEntry.declarativeNetRequestBlockMode ?? previousOverrides?.declarativeNetRequestBlockMode,
			})
		}
		return normalizeHostnameScopedWebsiteAccess(updatedWebsiteAccess, hostScopedOverrides).websiteAccess
	})
		...
		switch (params.data.type) {
			case 'Transaction': {
				...
				const shouldShiftNonceAfterRemoval = (transaction: typeof removedTransaction) => {
					return transactionWasFound && transaction.originalRequestParameters.method === 'eth_sendTransaction' && transaction.signedTransaction.from === removedTransaction.signedTransaction.from && transaction.signedTransaction.nonce > removedTransaction.signedTransaction.nonce
				}
			...
			case 'Message': {
				...
				return {
					operations: normalizeConsecutiveTimeManipulations(prevStack.operations).filter((operation) => !(operation.type === 'Message' && messageIdentifier === operation.signedMessageTransaction.messageIdentifier)),
				}
			}
			default:
...
export async function refreshPopupConfirmTransactionMetadata(ethereum: EthereumClientService, tokenPriceService: TokenPriceService, requestAbortController: AbortController | undefined) {
	...
	switch (first.type) {
		case 'SignableMessage': {
			...
			await Promise.all([sendPopupMessageToOpenWindows(serialize(UpdateConfirmTransactionDialogPendingTransactions, messagePendingTransactions), 'confirmTransaction'), sendPopupMessageToOpenWindows(serialize(UpdateConfirmTransactionDialog, message), 'confirmTransaction')])
			return
		}
		case 'Transaction': {
...
export async function refreshPopupConfirmTransactionSimulation(ethereum: EthereumClientService, tokenPriceService: TokenPriceService) {
	...
	const transactionToSimulate =
		firstTxn.originalRequestParameters.method === 'eth_sendTransaction'
			? await formEthSendTransaction(ethereum, undefined, firstTxn.activeAddress, firstTxn.transactionToSimulate.website, firstTxn.originalRequestParameters, firstTxn.created, firstTxn.transactionIdentifier, firstTxn.simulationMode)
...
export async function requestNewHomeData(ethereum: EthereumClientService, websiteTabConnections: WebsiteTabConnections, shouldRefreshSignerAccounts: boolean, requestAbortController: AbortController | undefined) {
...
export async function importSettings(settingsData: ImportSettings) {
	...
	if (!parsed.success) {
		await sendPopupMessageToOpenWindows({
			...
			data: {
				success: false,
				errorMessage: 'Failed to read the file. It is not a valid interceptor settings file',
			},
		})
		return
	}
...
export async function exportSettings() {
	...
	await sendPopupMessageToOpenWindows({
		...
		data: {
			fileContents: JSON.stringify(serialize(ExportedSettings, exportedSettings), undefined, 4),
		},
	})
}
	...
	const newStack = await updateInterceptorTransactionStack((prevStack: InterceptorTransactionStack) => {
		...
		const appendAfterIndex = normalizedPrevStack.operations.findIndex((operation) => {
			switch (operation.type) {
				case 'Transaction':
					return identifier.type === operation.type && identifier.transactionIdentifier === operation.preSimulationTransaction.transactionIdentifier
				case 'Message':
					return identifier.type === operation.type && identifier.messageIdentifier === operation.signedMessageTransaction.messageIdentifier
		...
		if (maybeExistingManipulator?.type === 'TimeManipulation') {
			...
			if (parsedRequest.data.blockTimeManipulation.type === 'No Delay')
				return {
					operations: normalizeConsecutiveTimeManipulations([...normalizedPrevStack.operations.slice(0, indexOfMaybeManipulator), ...normalizedPrevStack.operations.slice(indexOfMaybeManipulator + 1)]),
				}
		...
		return {
			operations: normalizeConsecutiveTimeManipulations([...normalizedPrevStack.operations.slice(0, indexOfMaybeManipulator), newManipulator, ...normalizedPrevStack.operations.slice(indexOfMaybeManipulator)]),
		}
	})
...
export async function requestMakeMeRichList(ethereumClientService: EthereumClientService, requestAbortController: AbortController | undefined) {
	...
	const fixedRichListPromises = Array.from(fixedAddressRichList.values()).map(async (element) => {
		try {
			return {
				...element,
				addressBookEntry: await identifyAddress(ethereumClientService, requestAbortController, element.address),
			}
		} catch (error) {
...
async function getCachedRichData() {
	const [makeCurrentAddressRich, fixedAddressRichList] = await Promise.all([getMakeCurrentAddressRich(), getFixedAddressRichList()])
	return {
		method: 'popup_requestMakeMeRichData' as const,
...
async function buildHomePageUpdate(
	...
	const richDataPromise = silenceChromeUnCaughtPromise(richDataSource === 'fresh' ? requestMakeMeRichList(ethereum, requestAbortController) : getCachedRichData())
	const tabId = await getLastKnownCurrentTabId()
			...
			switch (operation.type) {
				case 'Message':
					return modifyObject(operation, {
						signedMessageTransaction: modifyObject(operation.signedMessageTransaction, {
							website: {
				...
				case 'Transaction':
					return modifyObject(operation, {
						preSimulationTransaction: modifyObject(operation.preSimulationTransaction, {
							website: {
							...
							case 'Message':
								return modifyObject(operation, {
									signedMessageTransaction: modifyObject(operation.signedMessageTransaction, {
										...
										website: updateWebsiteDetails(operation.signedMessageTransaction.website),
									}),
								})
							case 'TimeManipulation':
								return operation
							case 'Transaction':
								return modifyObject(operation, {
									preSimulationTransaction: modifyObject(operation.preSimulationTransaction, {
										...
										website: updateWebsiteDetails(operation.preSimulationTransaction.website),
									}),
								})
							default:
								return assertNever(operation)
...
export async function requestSimulationMetadata(ethereumClientService: EthereumClientService) {
	...
	if (simulationState.kind === 'passthrough' || simulationState.value.success === false)
		return {
...
export async function requestIdentifyAddress(ethereumClientService: EthereumClientService, parsedRequest: RequestIdentifyAddress) {
	return {
		...
		data: {
			addressBookEntry: await identifyAddress(ethereumClientService, undefined, parsedRequest.data.address),
		},
	}
}
