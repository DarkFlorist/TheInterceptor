import {
	...
	refreshConfirmTransactionSimulation,
} from './background.js'
import {
	getSettings,
	...
	getPage,
	setPreSimulationBlockTimeManipulation,
...
import {
	type ChangeActiveAddress,
	type ModifyMakeMeRich,
	type ChangePage,
	type RemoveTransaction,
	type RequestAccountsFromSigner,
	type TransactionConfirmation,
	type InterceptorAccess,
	type ChangeInterceptorAccess,
	type ChainChangeConfirmation,
	type EnableSimulationMode,
	type ChangeActiveChain,
	type AddOrEditAddressBookEntry,
	type GetAddressBookData,
	type RemoveAddressBookEntry,
	type InterceptorAccessRefresh,
	type InterceptorAccessChangeAddress,
	type Settings,
	type ChangeSettings,
	type ImportSettings,
	type SetRpcList,
	UpdateHomePage,
	type SimulateGovernanceContractExecution,
	type ChangeAddOrModifyAddressWindowState,
	type OpenWebPage,
	type DisableInterceptor,
	type SetEnsNameForHash,
	UpdateConfirmTransactionDialog,
	UpdateConfirmTransactionDialogPendingTransactions,
	SimulateExecutionReply,
	type BlockOrAllowExternalRequests,
	type RemoveWebsiteAccess,
	type AllowOrPreventAddressAccessForWebsite,
	type RemoveWebsiteAddressAccess,
	type ForceSetGasLimitForTransaction,
	type RetrieveWebsiteAccess,
	type ChangePreSimulationBlockTimeManipulation,
	type SetTransactionOrMessageBlockTimeManipulator,
	type FetchSimulationStackRequestConfirmation,
	type ImportSimulationStack,
	type PopupReadyAndListeningPage,
} from '../types/interceptor-messages.js'
import {
	formEthSendTransaction,
	formSendRawTransaction,
	resolvePendingTransactionOrMessage,
	updateConfirmTransactionView,
	setGasLimitForTransaction,
	toPopupPendingTransactionOrSignableMessage,
...
import {
	...
	setInterceptorDisabledForWebsite,
...
import {
	findEntryWithSymbolOrName,
	getMetadataForAddressBookData,
} from './medataSearch.js'
import {
	getActiveAddressEntry,
	getActiveAddresses,
	identifyAddress,
...
import {
	...
	type InterceptorStackOperation,
...
import {
	fetchAbiFromBlockExplorer,
	isValidAbi,
} from '../simulation/services/EtherScanAbiFetcher.js'
import {
	checksummedAddress,
	generate256BitRandomBigInt,
	stringToAddress,
...
} from '../utils/contentScriptsUpdating.js'
...
import {
	buildWebsiteAccessPopupData,
	getAddressMetadataForAccess,
	sendWebsiteAccessChangedFromWebsiteAccess,
} from './websiteAccessPopup.js'
import {
	getCurrentSimulationInput,
	getMetadataForSimulation,
	simulateGnosisSafeMetaTransaction,
...
import {
	...
	updatePopupVisualisationState,
} from './popupVisualisationUpdater.js'
...
const getSimulationConductedTimestamp = (
	popupVisualisation: TimestampedPopupVisualisation,
...
const importSimulationStackFailure = (
	message: string,
): ImportSimulationStackReply => ({
	type: 'ImportSimulationStackReply',
	ok: false,
...
async function refreshSignerAccountsForTabIfNeeded(
	...
	const approvedConnection = Object.values(tabConnections.connections).find(
...
export async function confirmRequestAccess(
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	resetSimulationServices: ResetSimulationServices,
	websiteTabConnections: WebsiteTabConnections,
	confirmation: InterceptorAccess,
) {
...
export async function popupReadyAndListening(
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
...
async function getSignerAccount() {
	...
	const signerAccounts =
		tabId === undefined ? undefined : (await getTabState(tabId)).signerAccounts
...
export async function changeActiveAddress(
	...
	if (addressChange.data.activeAddress === 'signer') {
		...
		await setUseSignersAddressAsActiveAddress(
			addressChange.data.activeAddress === 'signer',
			signerAccount,
		)
...
export async function modifyMakeMeRich(
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	makeMeRichChange: ModifyMakeMeRich,
) {
	if (makeMeRichChange.data.address === 'CurrentAddress') {
		...
		if (makeMeRichChange.data.add) {
			...
			await setFixedMakeMeRichList(
				currentList.filter(
					(element) => element.address !== makeMeRichChange.data.address,
				),
			)
		}
	}
		...
		previousContacts.filter(
			(contact) =>
				!(
					...
					(contact.chainId === removeAddressBookEntry.data.chainId ||
						(contact.chainId === undefined &&
							removeAddressBookEntry.data.chainId === 1n))
				),
		),
	)
	if (removeAddressBookEntry.data.addressBookCategory === 'My Active Addresses')
...
export async function addOrModifyAddressBookEntry(
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	resetSimulationServices: ResetSimulationServices,
	websiteTabConnections: WebsiteTabConnections,
	entry: AddOrEditAddressBookEntry,
) {
	await updateUserAddressBookEntries((previousContacts) => {
		if (
			previousContacts.find(
				(previous) =>
					previous.address === entry.data.address &&
					(previous.chainId || 1n) === (entry.data.chainId || 1n),
			)
		) {
			return previousContacts.map((previous) =>
				previous.address === entry.data.address &&
				(previous.chainId || 1n) === (entry.data.chainId || 1n)
					? entry.data
					: previous,
	...
	if (entry.data.useAsActiveAddress)
		await updateWebsiteApprovalAccesses(
			ethereum,
			tokenPriceService,
			resetSimulationServices,
...
export async function changeInterceptorAccess(
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	resetSimulationServices: ResetSimulationServices,
	websiteTabConnections: WebsiteTabConnections,
	accessChange: ChangeInterceptorAccess,
) {
	await updateWebsiteAccess((previousAccess) => {
		const withEntriesRemoved = previousAccess.filter(
			(acc) =>
				accessChange.data.find(
					(change) =>
						change.newEntry.website.websiteOrigin === acc.website.websiteOrigin,
				)?.removed !== true,
		)
		const updatedWebsiteAccess = withEntriesRemoved.map((entry) => {
			const changeForEntry = accessChange.data.find(
				(change) =>
					change.newEntry.website.websiteOrigin === entry.website.websiteOrigin,
			)
			if (changeForEntry === undefined) return entry
			return changeForEntry.newEntry
		})
		const hostScopedOverrides = new Map<
			string,
			{
		...
		>()
		for (const change of accessChange.data) {
			const hostname = getHostnameForWebsiteOrigin(
				change.newEntry.website.websiteOrigin,
			)
			const previousOverrides = hostScopedOverrides.get(hostname)
			hostScopedOverrides.set(hostname, {
				interceptorDisabled:
					change.newEntry.interceptorDisabled ??
					previousOverrides?.interceptorDisabled,
				declarativeNetRequestBlockMode:
					change.newEntry.declarativeNetRequestBlockMode ??
					previousOverrides?.declarativeNetRequestBlockMode,
			})
		}
		return normalizeHostnameScopedWebsiteAccess(
			updatedWebsiteAccess,
			hostScopedOverrides,
		).websiteAccess
	})

	const interceptorDisablesChanged = accessChange.data
		.filter(
			(x) => x.newEntry.interceptorDisabled !== x.oldEntry.interceptorDisabled,
		)
		.map((x) => x)
	await Promise.all(
		interceptorDisablesChanged.map(async (disable) => {
			if (disable.newEntry.interceptorDisabled === undefined) return
			return await disableInterceptorForPage(
				websiteTabConnections,
				disable.newEntry.website,
				disable.newEntry.interceptorDisabled,
			)
		}),
	)
	...
	await sendPopupMessageToOpenWindows({
		method: 'popup_interceptor_access_changed',
	})
}

export const changePage = async (page: ChangePage) => await setPage(page.data)

export async function requestAccountsFromSigner(
	websiteTabConnections: WebsiteTabConnections,
	params: RequestAccountsFromSigner,
) {
	if (params.data) {
		sendMessageToApprovedWebsitePorts(websiteTabConnections, {
...
const normalizeConsecutiveTimeManipulations = (
	...
	return operations.filter(
		(operation, operationIndex) =>
			!(
				operationIndex > 0 &&
				operation.type === 'TimeManipulation' &&
				operations[operationIndex - 1]?.type === 'TimeManipulation'
			),
	)
}

export async function removeTransactionOrSignedMessage(
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	params: RemoveTransaction,
) {
		...
		(prevStack: InterceptorTransactionStack) => {
			switch (params.data.type) {
				case 'Transaction': {
					...
					const transactionToBeRemoved = prevStack.operations.find(
						(
							transaction,
						): transaction is Extract<
							InterceptorStackOperation,
							{ type: 'Transaction' }
						> =>
							transaction.type === 'Transaction' &&
							transaction.preSimulationTransaction.transactionIdentifier ===
								transactionIdentifier,
					)
					if (transactionToBeRemoved === undefined) return prevStack
					const removedTransaction =
						transactionToBeRemoved.preSimulationTransaction
					const shouldShiftNonceAfterRemoval = (
						transaction: typeof removedTransaction,
					) => {
						return (
							transactionWasFound &&
							transaction.originalRequestParameters.method ===
								'eth_sendTransaction' &&
							transaction.signedTransaction.from ===
								removedTransaction.signedTransaction.from &&
							transaction.signedTransaction.nonce >
								removedTransaction.signedTransaction.nonce
						)
					}
					...
					for (const operation of prevStack.operations) {
						if (
							...
							transactionIdentifier ===
								operation.preSimulationTransaction.transactionIdentifier
						) {
						...
						if (operation.type === 'Transaction') {
							...
							const shouldUpdateNonce =
								shouldShiftNonceAfterRemoval(transaction)
							const newTransaction = shouldUpdateNonce
								? modifyObject(transaction.signedTransaction, {
										nonce: transaction.signedTransaction.nonce - 1n,
									})
					...
					return {
						operations: normalizeConsecutiveTimeManipulations(newOperations),
					}
				}
				case 'Message': {
					const messageIdentifier = params.data.messageIdentifier
					return {
						operations: normalizeConsecutiveTimeManipulations(
							...
							(operation) =>
								!(
									...
									messageIdentifier ===
										operation.signedMessageTransaction.messageIdentifier
								),
						),
					}
				}
...
export async function refreshPopupConfirmTransactionMetadata(
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	requestAbortController: AbortController | undefined,
) {
	const currentBlockNumberPromise = ethereum.getBlockNumber(
		requestAbortController,
	)
	silenceChromeUnCaughtPromise(currentBlockNumberPromise)
	const rpcConnectionStatusPromise = silenceChromeUnCaughtPromise(
		getRpcConnectionStatus(),
	)
	const promises = await getPendingTransactionsAndMessages()
	const visualizedSimulatorStatePromise = silenceChromeUnCaughtPromise(
		updatePopupVisualisationIfNeeded(ethereum, tokenPriceService),
	)
	const first = promises[0]
	if (first === undefined) return
	switch (first.type) {
		case 'SignableMessage': {
			const visualizedPersonalSignRequestPromise =
				craftPersonalSignPopupMessage(
					ethereum,
					requestAbortController,
					first.signedMessageTransaction,
					ethereum.getRpcEntry(),
				)
			...
			const messagePendingTransactions: UpdateConfirmTransactionDialogPendingTransactions =
				{
					method:
						'popup_update_confirm_transaction_dialog_pending_transactions' as const,
					data: {
						pendingTransactionAndSignableMessages: [
							{
								...first,
								visualizedPersonalSignRequest:
									await visualizedPersonalSignRequestPromise,
								transactionOrMessageCreationStatus: 'Simulated' as const,
							},
							...promises.slice(1),
						].map(toPopupPendingTransactionOrSignableMessage),
						currentBlockNumber: await currentBlockNumberPromise,
						rpcConnectionStatus: await rpcConnectionStatusPromise,
					},
				}
			await Promise.all([
				...
				sendPopupMessageToOpenWindows(
					serialize(UpdateConfirmTransactionDialog, message),
					'confirmTransaction',
				),
			])
			return
		...
		case 'Transaction': {
			if (
				first.transactionOrMessageCreationStatus !== 'Simulated' ||
				first.popupVisualisation.statusCode === 'failed'
			)
				return
			try {
				const visualizedSimulationState = await visualizeSimulatorState(
					first.popupVisualisation.data.simulationState,
					ethereum,
					tokenPriceService,
					requestAbortController,
				)
						...
						data: {
							pendingTransactionAndSignableMessages: [
								modifyObject(first, {
									popupVisualisation: {
										statusCode: 'success',
										data: modifyObject(first.popupVisualisation.data, {
...
export async function refreshPopupConfirmTransactionSimulation(
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
) {
	...
	if (
		firstTxn === undefined ||
		firstTxn.type !== 'Transaction' ||
		(firstTxn.transactionOrMessageCreationStatus !== 'Simulated' &&
			firstTxn.transactionOrMessageCreationStatus !== 'FailedToSimulate')
	)
		return
	const transactionToSimulate =
		firstTxn.originalRequestParameters.method === 'eth_sendTransaction'
	...
	await updatePendingTransactionOrMessage(
		firstTxn.uniqueRequestIdentifier,
		async (transactionOrMessage) => {
			switch (transactionOrMessage.type) {
				case 'SignableMessage':
					throw new Error('Tried to refresh simulation of a message')
				case 'Transaction': {
					if (
						transactionOrMessage.transactionOrMessageCreationStatus !==
							'Simulated' &&
						transactionOrMessage.transactionOrMessageCreationStatus !==
							'FailedToSimulate'
					)
						return transactionOrMessage
					const currentTimestamp = getSimulationConductedTimestamp(
						transactionOrMessage.popupVisualisation,
					)
				...
				default:
					assertNever(transactionOrMessage)
			}
		},
	)
...
export async function popupChangeActiveRpc(
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	resetSimulationServices: ResetSimulationServices,
	websiteTabConnections: WebsiteTabConnections,
	params: ChangeActiveChain,
	settings: Settings,
) {
	return await changeActiveRpc(
		ethereum,
		tokenPriceService,
		resetSimulationServices,
		websiteTabConnections,
...
export async function changeChainDialog(
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	resetSimulationServices: ResetSimulationServices,
	websiteTabConnections: WebsiteTabConnections,
	chainChange: ChainChangeConfirmation,
) {
...
export async function enableSimulationMode(
	...
	if (settings.useSignersAddressAsActiveAddress || params.data === false) {
		...
		const tabId = await getLastKnownCurrentTabId()
		const chainToSwitch =
			tabId === undefined ? undefined : (await getTabState(tabId)).signerChain
		const networkToSwitch =
			chainToSwitch === undefined
				? (await getRpcList())[0]
				: await getPrimaryRpcForChain(chainToSwitch)
		await changeActiveAddressAndChain(
			ethereum,
			tokenPriceService,
			resetSimulationServices,
		...
		await changeActiveAddressAndChain(
			...
			{
				simulationMode: params.data,
				...(settings.activeRpcNetwork === selectedNetworkToSwitch
					? {}
					: { rpcNetwork: selectedNetworkToSwitch }),
			},
		)
	}
}
...
export const openNewTab = async (
	tabName: 'settingsView' | 'addressBook' | 'websiteAccess',
) => {
	...
	const tab = await updateTabIfExists(addressBookTab.id, {
		active: true,
		highlighted: true,
	})
...
export async function requestNewHomeData(
	...
	const updatedPage = await buildHomePageUpdate(
		ethereum,
		websiteTabConnections,
		{
			requestAbortController,
...
export async function refreshHomeData(
	...
	try {
		...
		if (currentSettings.simulationMode)
			await updateSimulationMetadata(ethereum, requestAbortController)
		...
		const settings = await getSettings()
		if (settings.activeRpcNetwork.httpsRpc !== undefined)
			await makeSureInterceptorIsNotSleeping(ethereum)
		const updatedPage = await buildHomePageUpdate(
			ethereum,
			websiteTabConnections,
			{
...
export async function settingsOpened() {
	const useTabsInsteadOfPopupPromise = silenceChromeUnCaughtPromise(
		getUseTabsInsteadOfPopup(),
	)
	const metamaskCompatibilityModePromise = silenceChromeUnCaughtPromise(
		getMetamaskCompatibilityMode(),
	)
...
export async function interceptorAccessChangeAddressOrRefresh(
	websiteTabConnections: WebsiteTabConnections,
	params: InterceptorAccessChangeAddress | InterceptorAccessRefresh,
) {
...
export async function changeSettings(
	...
	parsedRequest: ChangeSettings,
	requestAbortController: AbortController | undefined,
) {
	if (parsedRequest.data.useTabsInsteadOfPopup !== undefined)
		await setUseTabsInsteadOfPopup(parsedRequest.data.useTabsInsteadOfPopup)
	if (parsedRequest.data.metamaskCompatibilityMode !== undefined)
		await setMetamaskCompatibilityMode(
			parsedRequest.data.metamaskCompatibilityMode,
		)
...
export async function importSettings(settingsData: ImportSettings) {
	if (!isJSON(settingsData.data.fileContents)) {
		await sendPopupMessageToOpenWindows({
			...
			data: {
				success: false,
				errorMessage: 'Failed to read the file. It is not a valid JSON file.',
			},
		})
		return
	}
	const parsed = ExportedSettings.safeParse(
		JSON.parse(settingsData.data.fileContents),
	)
	if (!parsed.success) {
		await sendPopupMessageToOpenWindows({
			method: 'popup_initiate_export_settings_reply',
			data: {
				success: false,
				errorMessage:
					'Failed to read the file. It is not a valid interceptor settings file',
			},
		})
		return
	}
	...
	await sendPopupMessageToOpenWindows({
		...
		data: {
			fileContents: JSON.stringify(
				serialize(ExportedSettings, exportedSettings),
				undefined,
				4,
			),
		},
...
export async function setNewRpcList(
	resetSimulationServices: ResetSimulationServices,
	request: SetRpcList,
	settings: Settings,
) {
...
export async function simulateGovernanceContractExecutionOnPass(
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	request: SimulateGovernanceContractExecution,
) {
	const pendingTransactions = await getPendingTransactionsAndMessages()
	const transaction = pendingTransactions.find(
		(tx) =>
			tx.type === 'Transaction' &&
			tx.transactionIdentifier === request.data.transactionIdentifier,
	)
	if (transaction === undefined || transaction.type !== 'Transaction')
		throw new Error(
			`Could not find transactionIdentifier: ${request.data.transactionIdentifier}`,
		)
	const governanceContractExecutionVisualisation =
		await simulateGovernanceContractExecution(
			transaction,
			ethereum,
			tokenPriceService,
		)
	await sendPopupMessageToOpenWindows(
		serialize(SimulateExecutionReply, {
			...
			data: {
				...governanceContractExecutionVisualisation,
				transactionOrMessageIdentifier: request.data.transactionIdentifier,
			},
		}),
	)
}

export async function simulateGnosisSafeTransactionOnPass(
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	gnosisSafeMessage: VisualizedPersonalSignRequestSafeTx,
) {
	const gnosisTransactionExecutionVisualisation =
		await simulateGnosisSafeMetaTransaction(
			gnosisSafeMessage,
	...
	await sendPopupMessageToOpenWindows(
		serialize(SimulateExecutionReply, {
			method: 'popup_simulateExecutionReply' as const,
			data: {
				...gnosisTransactionExecutionVisualisation,
				transactionOrMessageIdentifier: gnosisSafeMessage.messageIdentifier,
			},
		}),
	)
}

const getErrorIfAnyWithIncompleteAddressBookEntry = async (
	ethereum: EthereumClientService,
	incompleteAddressBookEntry: IncompleteAddressBookEntry,
) => {
	// check for duplicates
	const duplicateEntry = await findEntryWithSymbolOrName(
		incompleteAddressBookEntry.symbol,
		incompleteAddressBookEntry.name,
		incompleteAddressBookEntry.chainId,
	)
	if (
		duplicateEntry !== undefined &&
		duplicateEntry.address !==
			stringToAddress(incompleteAddressBookEntry.address)
	) {
	...
	if (incompleteAddressBookEntry.address !== undefined) {
		...
		if (isAddress(trimmed)) {
			...
			if (incompleteAddressBookEntry.addingAddress) {
				const identifiedAddress = await identifyAddress(
					ethereum,
					undefined,
					address,
				)
				if (
					identifiedAddress.entrySource !== 'OnChain' &&
					identifiedAddress.entrySource !== 'FilledIn'
				) {
	...
	const trimmedAbi =
		incompleteAddressBookEntry.abi === undefined
			? undefined
			: incompleteAddressBookEntry.abi.trim()
	if (
		trimmedAbi !== undefined &&
		trimmedAbi.length !== 0 &&
		(!isJSON(trimmedAbi) || !isValidAbi(trimmedAbi))
	) {
...
export async function changeAddOrModifyAddressWindowState(
	ethereum: EthereumClientService,
	parsedRequest: ChangeAddOrModifyAddressWindowState,
) {
	const updatePage = async (newState: ModifyAddressWindowState) => {
		const currentPage = await getPage()
		if (
			(currentPage.page === 'AddNewAddress' ||
				currentPage.page === 'ModifyAddress') &&
			currentPage.state.windowStateId === parsedRequest.data.windowStateId
		) {
			await setPage({ page: currentPage.page, state: newState })
		}
	}
	await updatePage(parsedRequest.data.newState)
	const message = await getErrorIfAnyWithIncompleteAddressBookEntry(
		ethereum,
		parsedRequest.data.newState.incompleteAddressBookEntry,
	)

	const errorState =
		message === undefined ? undefined : { blockEditing: true, message }
	if (errorState?.message !== parsedRequest.data.newState.errorState?.message)
		await updatePage({ ...parsedRequest.data.newState, errorState })
	await sendPopupMessageToOpenWindows({
...
export async function requestAbiAndNameFromBlockExplorer(
	parsedRequest: RequestAbiAndNameFromBlockExplorer,
) {
	const etherscanReply = await fetchAbiFromBlockExplorer(
		parsedRequest.data.address,
...
export async function openWebPage(parsedRequest: OpenWebPage) {
	const allTabs = await browser.tabs.query({})
	const addressBookTab = allTabs.find(
		(tab) => tab.id === parsedRequest.data.websiteSocket.tabId,
	)
	...
	try {
		await browser.tabs.update(parsedRequest.data.websiteSocket.tabId, {
...
async function reloadConnectedTabs(
	websiteTabConnections: WebsiteTabConnections,
) {
	const tabIdsToRefesh = Array.from(websiteTabConnections.entries()).map(
		([tabId]) => tabId,
	)
	const currentTabId = await getLastKnownCurrentTabId()
	const withCurrentTabid =
		currentTabId === undefined
			? tabIdsToRefesh
			: [...tabIdsToRefesh, currentTabId]
...
async function disableInterceptorForPage(
	websiteTabConnections: WebsiteTabConnections,
	website: Website,
	interceptorDisabled: boolean,
) {
...
export async function disableInterceptor(
	...
	resetSimulationServices: ResetSimulationServices,
	websiteTabConnections: WebsiteTabConnections,
	parsedRequest: DisableInterceptor,
) {
	await disableInterceptorForPage(
		websiteTabConnections,
		parsedRequest.data.website,
		parsedRequest.data.interceptorDisabled,
	)
	await updateWebsiteApprovalAccesses(
		ethereum,
		tokenPriceService,
	...
	await sendPopupMessageToOpenWindows({
		method: 'popup_setDisableInterceptorReply' as const,
...
export async function retrieveWebsiteAccess(
	parsedRequest: RetrieveWebsiteAccess,
) {
	await sendPopupMessageToOpenWindows({
...
async function blockOrAllowWebsiteExternalRequests(
	websiteTabConnections: WebsiteTabConnections,
	website: Website,
	shouldBlock: boolean,
) {
	await updateWebsiteAccess((previousAccessList) => {
		return previousAccessList.map((access) => {
			if (
				!doWebsiteOriginsShareHostname(
					access.website.websiteOrigin,
					website.websiteOrigin,
				)
			)
				return access
			return modifyObject(access, {
				declarativeNetRequestBlockMode: shouldBlock ? 'block-all' : 'disabled',
			})
		})
	})
...
export async function blockOrAllowExternalRequests(
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	resetSimulationServices: ResetSimulationServices,
	websiteTabConnections: WebsiteTabConnections,
	parsedRequest: BlockOrAllowExternalRequests,
) {
	await blockOrAllowWebsiteExternalRequests(
		websiteTabConnections,
		parsedRequest.data.website,
		parsedRequest.data.shouldBlock,
	)
	const settings = await getSettings()
	await updateWebsiteApprovalAccesses(
		ethereum,
...
async function removeAddressAccessByAddress(
	...
	await updateWebsiteAccess((previousAccessList) => {
		return previousAccessList.map((access) => {
			if (
				access.website.websiteOrigin !== websiteOrigin ||
				!access.addressAccess
			)
				return access
			const strippedAddressAccess = access.addressAccess.filter(
				(addressAccess) => addressAccess.address !== address,
			)
...
export async function removeWebsiteAddressAccess(
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	resetSimulationServices: ResetSimulationServices,
	websiteTabConnections: WebsiteTabConnections,
	parsedRequest: RemoveWebsiteAddressAccess,
) {
	await removeAddressAccessByAddress(
		parsedRequest.data.websiteOrigin,
		parsedRequest.data.address,
	)
	await reloadConnectedTabs(websiteTabConnections)
	const settings = await getSettings()
...
async function setAdressAccessForWebsite(
	websiteOrigin: string,
	address: EthereumAddress,
	allowAccess: boolean,
) {
	await updateWebsiteAccess((previousAccessList) => {
		return previousAccessList.map((access) => {
			if (
				...
				return access
			const addressAccessList = access.addressAccess.map((addressAccess) =>
				addressAccess.address !== address
					? addressAccess
					: modifyObject(addressAccess, { access: allowAccess }),
			)
...
export async function allowOrPreventAddressAccessForWebsite(
	websiteTabConnections: WebsiteTabConnections,
	parsedRequest: AllowOrPreventAddressAccessForWebsite,
) {
...
export async function removeWebsiteAccess(
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	resetSimulationServices: ResetSimulationServices,
	websiteTabConnections: WebsiteTabConnections,
	parsedRequest: RemoveWebsiteAccess,
) {
	await updateWebsiteAccess((previousAccess) =>
		previousAccess.filter(
			(access) =>
				!doWebsiteOriginsShareHostname(
					access.website.websiteOrigin,
					parsedRequest.data.websiteOrigin,
...
export async function forceSetGasLimitForTransaction(
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	parsedRequest: ForceSetGasLimitForTransaction,
) {
	await setGasLimitForTransaction(
		parsedRequest.data.transactionIdentifier,
		parsedRequest.data.gasLimit,
	)
	await updatePopupVisualisationIfNeeded(
		ethereum,
		tokenPriceService,
...
export async function changePreSimulationBlockTimeManipulation(
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	parsedRequest: ChangePreSimulationBlockTimeManipulation,
) {
	await setPreSimulationBlockTimeManipulation(
		parsedRequest.data.blockTimeManipulation,
	)
	await updatePopupVisualisationIfNeeded(
		ethereum,
		tokenPriceService,
...
export async function setTransactionOrMessageBlockTimeManipulator(
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	parsedRequest: SetTransactionOrMessageBlockTimeManipulator,
) {
	const newStack = await updateInterceptorTransactionStack(
		(prevStack: InterceptorTransactionStack) => {
			const normalizedPrevStack = {
			...
			const appendAfterIndex = normalizedPrevStack.operations.findIndex(
				(operation) => {
			...
			const maybeExistingManipulator =
				normalizedPrevStack.operations[indexOfMaybeManipulator]
			if (maybeExistingManipulator?.type === 'TimeManipulation') {
				// no delay, so we can remove the manipulator
				if (parsedRequest.data.blockTimeManipulation.type === 'No Delay')
					return {
						operations: normalizeConsecutiveTimeManipulations([
							...normalizedPrevStack.operations.slice(
								0,
								indexOfMaybeManipulator,
							),
							...normalizedPrevStack.operations.slice(
								indexOfMaybeManipulator + 1,
							),
						]),
					}
				const newManipulator = {
					type: 'TimeManipulation',
					blockTimeManipulation: parsedRequest.data.blockTimeManipulation,
				} as const
				// replace manipulator
				return {
					operations: normalizeConsecutiveTimeManipulations(
						normalizedPrevStack.operations.map((operation, index) =>
							index === indexOfMaybeManipulator ? newManipulator : operation,
						),
					),
				}
			}
			...
			return {
				operations: normalizeConsecutiveTimeManipulations([
					...normalizedPrevStack.operations.slice(0, indexOfMaybeManipulator),
					newManipulator,
					...normalizedPrevStack.operations.slice(indexOfMaybeManipulator),
				]),
			}
		},
	)
	const secondToLastOperation =
		newStack.operations[newStack.operations.length - 2]
	if (
		secondToLastOperation === undefined ||
		secondToLastOperation.type === 'TimeManipulation'
	) {
		await updatePopupVisualisationIfNeeded(
			ethereum,
			tokenPriceService,
	...
	const appendIdentifier =
		parsedRequest.data.transactionOrMessageIdentifier.type === 'Transaction'
			? parsedRequest.data.transactionOrMessageIdentifier.transactionIdentifier
			: parsedRequest.data.transactionOrMessageIdentifier.messageIdentifier
	const operationIdentifier =
		secondToLastOperation.type === 'Transaction'
			? secondToLastOperation.preSimulationTransaction.transactionIdentifier
			: secondToLastOperation.signedMessageTransaction.messageIdentifier
...
export async function requestMakeMeRichList(
	ethereumClientService: EthereumClientService,
	requestAbortController: AbortController | undefined,
) {
	const makeMeRichPromise = silenceChromeUnCaughtPromise(
		getMakeCurrentAddressRich(),
	)
	const fixedAddressRichList = await getFixedAddressRichList()
	const fixedRichListPromises = Array.from(fixedAddressRichList.values()).map(
		async (element) => {
			try {
				return {
					...element,
					addressBookEntry: await identifyAddress(
				...
				await handleUnexpectedError(
					new Error(
						`Failed to identify rich list address ${address}: ${errorMessage}`,
					),
				)
				return {
					...element,
...
export const requestActiveAddresses = async () => ({
	method: 'popup_requestActiveAddresses' as const,
	activeAddresses: await getActiveAddresses(),
})

export const requestSimulationMode = async () => ({
	method: 'popup_requestSimulationMode' as const,
	simulationMode: (await getSettings()).simulationMode,
})

export const requestLatestUnexpectedError = async () => ({
	method: 'popup_requestLatestUnexpectedError' as const,
	latestUnexpectedError: await getLatestUnexpectedError(),
})
...
async function getCachedRichData() {
	...
	return {
		method: 'popup_requestMakeMeRichData' as const,
		richList: await Promise.all(
			fixedAddressRichList.map(async (element) => ({
				...element,
				addressBookEntry: await getActiveAddressEntry(element.address),
			})),
		),
...
async function buildHomePageUpdate(
	...
	const rpcEntriesPromise = silenceChromeUnCaughtPromise(getRpcList())
	const preSimulationBlockTimeManipulationPromise =
		silenceChromeUnCaughtPromise(getPreSimulationBlockTimeManipulation())
	const visualizedSimulatorStatePromise: Promise<CompleteVisualizedSimulation> =
		silenceChromeUnCaughtPromise(getPopupVisualisationState())
	const activeAddressesPromise = silenceChromeUnCaughtPromise(
		getActiveAddresses(),
	)
	const latestUnexpectedErrorPromise = silenceChromeUnCaughtPromise(
		getLatestUnexpectedError(),
	)
	...
	const tabId = await getLastKnownCurrentTabId()
	const tabStatePromise = silenceChromeUnCaughtPromise(
		tabId === undefined ? getTabState(-1) : getTabState(tabId),
	)
	const settings = await settingsPromise
	let tabState = await tabStatePromise
	tabState = await refreshSignerAccountsForTabIfNeeded(
		websiteTabConnections,
		tabId,
		tabState,
		shouldRefreshSignerAccounts,
	...
	const interceptorDisabled =
		websiteOrigin === undefined
			? false
			: hasAccess(settings.websiteAccess, websiteOrigin) ===
				'interceptorDisabled'
	...
	return {
		...
		data: {
			...
			websiteAccessAddressMetadata: await getAddressMetadataForAccess(
				settings.websiteAccess,
			),
			tabState,
			...
			preSimulationBlockTimeManipulation:
				await preSimulationBlockTimeManipulationPromise,
		},
	}
}

export async function fetchSimulationStackRequestConfirmation(
	ethereumClientService: EthereumClientService,
	websiteTabConnections: WebsiteTabConnections,
...
export async function handleUnexpectedErrorInWindow(
	parsedRequest: UnexpectedErrorOccured,
) {
...
export async function requestInterceptorSimulationInput(
	ethereumClientService: EthereumClientService,
) {
	const stackPromise = silenceChromeUnCaughtPromise(
		getInterceptorTransactionStack(),
	)
	const simulationInput = await getCurrentSimulationInput()
	const currentBlockNumberPromise = silenceChromeUnCaughtPromise(
		ethereumClientService.getBlockNumber(undefined),
	)
	const eth_simulateV1 = await ethereumClientService.ethSimulateV1Input(
		simulationInput,
		await currentBlockNumberPromise,
		undefined,
	)
	const stack = await stackPromise

	const interceptorSimulateStack = modifyObject(stack, {
		operations: stack.operations.map((operation) => {
			switch (operation.type) {
				case 'Message':
					return modifyObject(operation, {
						signedMessageTransaction: modifyObject(
							operation.signedMessageTransaction,
							{
								website: {
									...operation.signedMessageTransaction.website,
									icon: undefined,
									title: undefined,
								},
							},
				...
				case 'Transaction':
					return modifyObject(operation, {
						preSimulationTransaction: modifyObject(
							operation.preSimulationTransaction,
							{
								website: {
									...operation.preSimulationTransaction.website,
									icon: undefined,
									title: undefined,
								},
							},
	...
	return {
		...
		ethSimulateV1InputString: JSON.stringify(
...
export async function importSimulationStack(
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	parsedRequest: ImportSimulationStack,
): Promise<ImportSimulationStackReply> {
	if (parsedRequest.data.version !== '1.0.0')
		return importSimulationStackFailure(
			'Only simulation stack export version 1.0.0 is supported.',
		)
	if (parsedRequest.data.interceptorSimulateStack.operations.length === 0)
		return importSimulationStackSuccess()

	const importedStackBytes = estimateSerializedStateBytes(
		InterceptorTransactionStack,
		parsedRequest.data.interceptorSimulateStack,
	)
	console.info(
		`[simulation-stack import] received ${parsedRequest.data.interceptorSimulateStack.operations.length} operations (${formatEstimatedBytes(importedStackBytes)}).`,
	)

	const websiteAccess = await getWebsiteAccess()
	const updateWebsiteDetails = (website: Website) => {
		const websiteData = websiteAccess.find(
			(access) => access.website.websiteOrigin === website.websiteOrigin,
		)?.website
	...
	try {
		updatedStack = await updateInterceptorTransactionStack(
			(prevStack: InterceptorTransactionStack) => {
				const newOperations = [
					...prevStack.operations,
					...parsedRequest.data.interceptorSimulateStack.operations,
				]
								...
								case 'Message':
									return modifyObject(operation, {
										signedMessageTransaction: modifyObject(
											operation.signedMessageTransaction,
											{
												messageIdentifier: generate256BitRandomBigInt(),
								...
								case 'Transaction':
									return modifyObject(operation, {
										preSimulationTransaction: modifyObject(
											operation.preSimulationTransaction,
											{
												transactionIdentifier: generate256BitRandomBigInt(),
												website: updateWebsiteDetails(
													operation.preSimulationTransaction.website,
												),
											},
		...
		return importSimulationStackFailure(
			`Failed to store the imported simulation stack (${formatEstimatedBytes(importedStackBytes)}): ${getErrorMessage(error)}`,
		)
	}

	const updatedStackBytes = estimateSerializedStateBytes(
		InterceptorTransactionStack,
		updatedStack,
	)
	console.info(
		`[simulation-stack import] persisted transaction stack at ${formatEstimatedBytes(updatedStackBytes)}.`,
	)

	try {
		await updatePopupVisualisationState(
			ethereum,
			tokenPriceService,
			undefined,
			true,
		...
		const popupVisualisationBytes = estimateSerializedStateBytes(
			CompleteVisualizedSimulation,
			popupVisualisation,
		)
		console.info(
			`[simulation-stack import] persisted popup visualisation at ${formatEstimatedBytes(popupVisualisationBytes)}.`,
		)
	} catch (error) {
		return importSimulationStackFailure(
			`Imported stack was stored (${formatEstimatedBytes(updatedStackBytes)}), but updating the visualized simulation failed: ${getErrorMessage(error)}`,
		)
	}
...
export async function requestCompleteVisualizedSimulation(
	...
	const visualizedSimulatorState = await updatePopupVisualisationIfNeeded(
		ethereum,
		tokenPriceService,
		false,
		false,
	...
	return {
		method: 'popup_requestCompleteVisualizedSimulation' as const,
		visualizedSimulatorState,
	}
}

export async function requestSimulationMetadata(
	ethereumClientService: EthereumClientService,
) {
	const settings = await getSettings()
	const simulationState = settings.simulationMode
		? await getUpdatedSimulationState(ethereumClientService)
		: { kind: 'passthrough' as const }
	if (
		simulationState.kind === 'passthrough' ||
		simulationState.value.success === false
	)
		return {
			...
			metadata: {
				namedTokenIds: [],
				addressBookEntries: [],
				ens: { ensNameHashes: [], ensLabelHashes: [] },
			},
		}
	const eventsForEachBlockAndTransactionPromise = silenceChromeUnCaughtPromise(
		Promise.all(
			simulationState.value.simulatedBlocks.map((block) =>
				Promise.all(
					block.simulatedTransactions.map(async (simulatedTransaction) =>
						simulatedTransaction.ethSimulateV1CallResult.status === 'failure'
							? []
							: await parseEvents(
	...
	const parsedInputDataForEachBlockAndTransactionPromise =
		silenceChromeUnCaughtPromise(
			Promise.all(
				simulationState.value.simulatedBlocks.map((block) => {
					const transactions = getWebsiteCreatedEthereumUnsignedTransactions(
						block.simulatedTransactions,
					)
					return Promise.all(
	...
	const inputData = (
		await parsedInputDataForEachBlockAndTransactionPromise
	).flat()

	const metadata = await getMetadataForSimulation(
...
export async function requestIdentifyAddress(
	ethereumClientService: EthereumClientService,
	parsedRequest: RequestIdentifyAddress,
) {
	return {
		method: 'popup_requestIdentifyAddress' as const,
		data: {
			addressBookEntry: await identifyAddress(
				ethereumClientService,
				undefined,
