...
import { UI_COMMAND_ACTION, UI_EVENT_ACTION, UI_QUERY_ACTION, UI_SNAPSHOT_ACTION, UiPopupEventPayload, UiRole, getUiRoleFromPortName, isUiPortEnvelope } from '../messages/ui.js'
...
export function publishUiPopupEvent(message: MessageToPopupPayload, role: 'all' | 'confirmTransaction' = 'all') {
...
export async function onUiPortConnected(port: browser.runtime.Port, handleSnapshot: UiSnapshotHandler) {
	...
	port.onMessage.addListener((rawMessage: unknown) => {
		void Promise.resolve((async () => {
			...
			try {
				const payload = await uiRequestRouter.dispatch(rawMessage.action, { role: session.role, port }, rawMessage.payload)
