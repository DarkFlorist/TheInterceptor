...
export async function migrateWebsiteAccess() {
	const storageEntries: Partial<Record<'websiteAccess', unknown>> =
	...
	const sanitizedWebsiteAccess = sanitizeWebsiteAccess(
		parsedWebsiteAccess.value,
	)
	const normalizedWebsiteAccess = normalizeHostnameScopedWebsiteAccess(
		sanitizedWebsiteAccess,
	)
	if (
		sanitizedWebsiteAccess === parsedWebsiteAccess.value &&
		!normalizedWebsiteAccess.changed
	)
		return
