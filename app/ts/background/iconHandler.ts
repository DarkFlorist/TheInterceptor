...
async function getCachedWebsiteIcon(tabId: number, websiteOrigin: string) {
	...
	const storedWebsite = storedWebsiteAccess.find(
		(entry) => entry.website.websiteOrigin === websiteOrigin,
	)
	if (storedWebsite === undefined)
		return { cachedIcon: undefined, hasStoredWebsiteAccess: false as const }
	...
	if (currentWebsite?.websiteOrigin === websiteOrigin) {
		...
		if (currentIcon !== undefined)
			return { cachedIcon: currentIcon, hasStoredWebsiteAccess: true as const }
	}
	return {
		cachedIcon: sanitizeStoredWebsiteIcon(storedWebsite.website.icon),
		hasStoredWebsiteAccess: true as const,
	}
}

async function setInterceptorIcon(
	tabId: number,
	icon: TabIcon,
	iconReason: string,
) {
	...
	const { previousState, newState } = await updateTabState(
		tabId,
		(previousState: TabState) => {
			const previousTabIconDetails = previousState.tabIconDetails
			if (
				previousTabIconDetails.icon === tabIconDetails.icon &&
				previousTabIconDetails.iconReason === tabIconDetails.iconReason
			)
...
async function waitForLoadedTab(tabId: number) {
	...
	try {
		...
		try {
			timeout = setTimeout(
				() => waitForLoadedFuture.reject(new Error('timed out')),
				60000,
			)
...
export async function updateExtensionIcon(
	...
	const blockingWebsitePromise = areWeBlocking(
		websiteTabConnections,
		tabId,
		websiteOrigin,
	)
	silenceChromeUnCaughtPromise(blockingWebsitePromise)
	const addShieldIfNeeded = async (icon: TabIcon): Promise<TabIcon> =>
		(await blockingWebsitePromise) && icon !== ICON_INTERCEPTOR_DISABLED
			? TabIcon.parse(icon.replace('.png', '-shield.png'))
			: icon
	const setIcon = async (icon: TabIcon, iconReason: string) =>
		setInterceptorIcon(
			tabId,
			await addShieldIfNeeded(icon),
			(await blockingWebsitePromise)
				? `${iconReason} The Interceptor is blocking external requests made by the website.`
				: iconReason,
		)

	const settings = await getSettings()
	if (
		hasAccess(settings.websiteAccess, websiteOrigin) === 'interceptorDisabled'
	)
		return setIcon(
			ICON_INTERCEPTOR_DISABLED,
			`The Interceptor is disabled for ${websiteOrigin} by user request.`,
		)
	const activeAddress = await getActiveAddress(settings, tabId)
	if (activeAddress === undefined)
		return setIcon(ICON_NOT_ACTIVE, 'No active address selected.')
	...
	if (addressAccess === 'notFound')
		return setIcon(
			ICON_NOT_ACTIVE,
			`${websiteOrigin} has PENDING access request for ${activeAddress.name}!`,
		)
	if (addressAccess !== 'hasAccess') {
		if (hasAccess(settings.websiteAccess, websiteOrigin) === 'noAccess') {
			return setIcon(
				ICON_ACCESS_DENIED,
				`The access for ${websiteOrigin} has been DENIED!`,
			)
		}
		return setIcon(
			ICON_ACCESS_DENIED,
			`The access to ${activeAddress.name} for ${websiteOrigin} has been DENIED!`,
		)
	}
	if (settings.simulationMode)
		return setIcon(
			ICON_SIMULATING,
			'The Interceptor simulates your sent transactions.',
		)
	if (settings.activeRpcNetwork.httpsRpc === undefined)
		return setIcon(
			ICON_SIGNING_NOT_SUPPORTED,
			`The Interceptor is disabled while it's on an unsupported network`,
		)
	const tabState = await getTabState(tabId)
	return setIcon(
		ICON_SIGNING,
		`The Interceptor forwards your transactions to ${getPrettySignerName(tabState.signerName)} once sent.`,
	)
}
...
export async function retrieveWebsiteDetails(
	tabId: number,
	websiteOrigin?: string,
) {
	let loadedTab
	try {
		...
		console.warn(
			`Failed to load tab ${tabId} before reading website details`,
			error,
		)
	...
	if (websiteOrigin !== undefined) {
		const { cachedIcon, hasStoredWebsiteAccess } = await getCachedWebsiteIcon(
			tabId,
			websiteOrigin,
		)
		if (cachedIcon !== undefined)
			return { title: loadedTab?.title, icon: cachedIcon }
		if (!hasStoredWebsiteAccess)
			return { title: loadedTab?.title, icon: undefined }
	}
	...
	const failToLoadFavicon = (reason: string) => {
		console.warn(
			`Failed to load favicon for tab ${tabId} (${pageUrl}): ${reason}`,
		)
	...
	try {
		parsedFaviconUrl =
			tab?.url === undefined
				? new URL(faviconUrl)
				: new URL(faviconUrl, tab.url)
	} catch (error) {
		const reason =
			error instanceof Error
				? `${error.message}: ${faviconUrl}`
				: `invalid favicon URL ${faviconUrl}`
		return failToLoadFavicon(reason)
	}

	if (!ALLOWED_FAVICON_PROTOCOLS.has(parsedFaviconUrl.protocol)) {
		return failToLoadFavicon(
			`unsupported URL scheme ${parsedFaviconUrl.protocol}`,
		)
	}
	if (parsedFaviconUrl.protocol === 'data:') {
		...
		if (icon === undefined)
			return failToLoadFavicon(
				'favicon data URL was not an image or exceeded the size limit',
			)
		return {
			title: tab?.title,
			icon,
	...
	if (tab?.url === undefined)
		return failToLoadFavicon('page URL was unavailable for favicon validation')
	const pageOrigin = new URL(tab.url).origin
	if (parsedFaviconUrl.origin !== pageOrigin) {
		return failToLoadFavicon(
			`favicon origin ${parsedFaviconUrl.origin} did not match page origin ${pageOrigin}`,
		)
	}
	const faviconResult = await imageToUri(parsedFaviconUrl.toString())
	if (
		faviconResult.failureReason !== undefined ||
		faviconResult.data === undefined
	)
		return failToLoadFavicon(
			faviconResult.failureReason ?? 'favicon conversion failed',
		)
	return {
		title: tab?.title,
