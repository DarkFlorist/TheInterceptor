...
export async function updateExtensionIcon(tabId: number, websiteOrigin: string) {
	...
	if (settings.rpcNetwork.httpsRpc === undefined) return setInterceptorIcon(tabId, ICON_SIGNING_NOT_SUPPORTED, `Interceptor is on an unsupported network and simulation mode is disabled.`)
