	...
	try {
		const processRequest = async (): Promise<PopupReplyOption | void> => {
			switch (parsedRequest.method) {
				...
				case 'popup_requestNewHomeData': return await requestNewHomeData(ethereum, tokenPriceService, websiteTabConnections, simulationAbortController)
				case 'popup_refreshHomeData': return await refreshHomeData(ethereum, tokenPriceService, websiteTabConnections)
