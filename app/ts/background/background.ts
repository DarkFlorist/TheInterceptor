...
import {
	...
	getFilterChanges,
	getFilterLogs,
	handleIterceptorError,
	requestInterceptorSimulatorStack,
} from './simulationModeHanders.js'
import {
	changeActiveAddress,
	changePage,
	confirmDialog,
...
async function handleRPCRequest(
	...
	switch (parsedRequest.method) {
		...
		case 'eth_unsubscribe':
			return await unsubscribe(socket, parsedRequest)
		case 'eth_chainId':
			return await chainId(ethereum)
		case 'net_version':
		...
		case 'eth_simulateV1':
			return {
				...
				error: { code: 10000, message: 'Cannot call eth_simulateV1 directly' },
			}
		case 'wallet_addEthereumChain': {
			...
			return {
				...
				error: {
					code: 10000,
					message: 'wallet_addEthereumChain not implemented',
				},
			}
		}
		case 'eth_getStorageAt': {
			...
			return {
				type: 'result' as const,
				method: parsedRequest.method,
				error: { code: 10000, message: 'eth_getStorageAt not implemented' },
			}
		}
		case 'eth_getLogs':
			return await withExecutionSimulationState((simulationState) =>
		...
		case 'eth_sign':
			return {
				type: 'result' as const,
				method: parsedRequest.method,
				error: { code: 10000, message: 'eth_sign is deprecated' },
			}
...
const keepTrackOfPreviousAddressforRichList = async () => {
	...
	await setFixedMakeMeRichList([
		...richList,
		{
			address: previousActiveAddress,
			makingRich: false,
			type: 'PreviousActiveAddress' as const,
		},
	])
}
...
function getProviderHandler(method: string) {
	switch (method) {
		case 'signer_reply':
			return { method: 'signer_reply' as const, func: signerReply }
		case 'eth_accounts_reply':
			return { method: 'eth_accounts_reply' as const, func: ethAccountsReply }
		case 'signer_chainChanged':
			return {
				method: 'signer_chainChanged' as const,
				func: signerChainChanged,
			}
		case 'wallet_switchEthereumChain_reply':
			return {
				method: 'wallet_switchEthereumChain_reply' as const,
				func: walletSwitchEthereumChainReply,
			}
		case 'connected_to_signer':
			return {
				method: 'connected_to_signer' as const,
				func: connectedToSigner,
			}
		default:
			return { method: 'notProviderMethod' as const }
	}
}
...
export const handleInterceptedRequest = async (
...
async function handleContentScriptMessage(
	...
	try {
		...
		const getExecutionSimulationState = async () => {
			if (!settings.simulationMode) return PASSTHROUGH_STATE
			if (executionSimulationStatePromise === undefined)
...
async function gateKeepRequestBehindAccessDialog(
	ethereum: EthereumClientService,
	tokenPriceService: TokenPriceService,
	resetSimulationServices: ResetSimulationServices,
	websiteTabConnections: WebsiteTabConnections,
		...
		const processRequest = async (): Promise<PopupReplyOption | void> => {
			switch (parsedRequest.method) {
				...
				case 'popup_import_settings':
					return await importSettings(parsedRequest)
				...
				case 'popup_setEnsNameForHash':
					return await setEnsNameForHash(parsedRequest)
				case 'popup_openWebsiteAccess':
					return await openNewTab('websiteAccess')
				...
				case 'popup_requestSimulationMode':
					return await requestSimulationMode()
				...
				case 'popup_requestIdentifyAddress':
					return await requestIdentifyAddress(ethereum, parsedRequest)
				case 'popup_isMainPopupWindowOpen':
					return
				default:
					assertUnreachable(parsedRequest)
			}
		}
