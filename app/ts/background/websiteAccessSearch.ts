...
const createSearchPattern = (searchString: string) => {
	const unicodeEscapeString = (stringToEscape: string) =>
		`\\u{${stringToEscape.charCodeAt(0).toString(16)}}`
	const segments = searchString.trim().split('')
	return segments.length
		? new RegExp(`(?=(${segments.map(unicodeEscapeString).join('.*?')}))`, 'ui')
		: undefined
}
...
function selectLongerMatch<T extends SearchMatch>(
	a: T | undefined,
	b: T | undefined,
): T | undefined
function selectLongerMatch<T extends SearchMatch>(
...
function calculateWebsiteAccessScore(
	entry: WebsiteAccess,
	query: string,
): SearchScore<WebsiteAccess> {
	const urlMatch = computeSearchMatch(
		query,
		entry.website.websiteOrigin.toLowerCase(),
	)
	const titleMatch = entry.website.title
		? computeSearchMatch(query, entry.website.title.toLowerCase())
		: undefined
	const addressMatches =
		entry.addressAccess?.map((addr: WebsiteAddressAccess) =>
			computeSearchMatch(query, addressString(addr.address).toLowerCase()),
		) || []
