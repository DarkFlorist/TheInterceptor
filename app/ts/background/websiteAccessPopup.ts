...
export async function getAddressMetadataForAccess(
	websiteAccess: WebsiteAccessArray,
): Promise<AddressBookEntries> {
	const addresses = websiteAccess.flatMap((x) =>
		x.addressAccess === undefined
			? []
			: x.addressAccess.map((addr) => addr.address),
	)
	const addressSet = new Set(addresses)
	return await Promise.all(
		Array.from(addressSet).map((x) => getActiveAddressEntry(x)),
	)
}

export async function buildWebsiteAccessPopupDataFromWebsiteAccess(
	websiteAccess: WebsiteAccessArray,
	query: string,
) {
	const filteredWebsiteAccess = searchWebsiteAccess(query, websiteAccess)
	const addressAccessMetadata = await getAddressMetadataForAccess(
		filteredWebsiteAccess,
	)
...
export async function buildWebsiteAccessPopupData(query: string) {
	...
	return await buildWebsiteAccessPopupDataFromWebsiteAccess(
		settings.websiteAccess,
		query,
	)
}

export async function sendWebsiteAccessChangedFromWebsiteAccess(
	websiteAccess: WebsiteAccessArray,
) {
