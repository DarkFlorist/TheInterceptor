...
## Built-UI signing walkthrough

Run `bun run test:chrome-direct-signing-flow` after building Chrome. This uses an isolated profile, public test keys, scripted native-shaped HID replies, QR pixels delivered through a synthetic camera stream, and a local RPC fixture. It clicks through all three methods for Ledger and AirGap. Transactions start at a seeded explanation/review boundary; approval, independent signature verification, explicit broadcast, durable application reply, receipt confirmation, rejection/disconnection retry, a wrong AirGap response, offline-page reload, and ambiguous submission reconciliation run through production handlers. It asserts that signing alone never broadcasts and that reconciliation does not send a second transaction. It does not claim to run a physical device or the Vault app.

`bun run test:chrome-signing-communication` exercises browser-wallet transaction forwarding and rejection from the originating website. Add `--messages`, `--ledger`, or `--airgap` to exercise personal and typed messages from website access approval through both confirmation pages and back to the site's request promise. The hardware variants inject no browser wallet. Their device responses are fixtures, while browser-wallet message signatures use a public test key and are independently verified. The browser transaction fixture returns a fixed hash and retains browser-owned broadcasting; its explanation uses the configured default RPC, not the local transaction fixture above.

Set `SIGNING_FLOW_SCREENSHOTS=/absolute/output/directory` on the communication harness to capture the live confirmation and completion states. The direct-flow harness always captures its stages, defaulting to `/tmp/interceptor-signing-flow`. Captures activate the actual window before waiting for rendering, including when signature delivery has focused the originating site. Full-page captures use the layout's actual width and report viewport, fonts, and device scale.

The walkthrough found and fixed missing browser-account refresh before signing with a saved binding, double serialization of typed data at both request admission and browser-signature verification, an unusable retry after a rejected QR response, stale submission UI after an RPC error, and a cancellation action on already-completed messages. Confirmations now keep mode, origin, network, acting address and saved wallet visible; direct signing shows readable messages, exact bytes, decoded call data/access lists, and compact domain/message sections with full type definitions available.

The visual-polish checks also assert that completed transactions no longer offer reconciliation and that completed messages/transactions offer Return to application. The website-driven hardware message walkthrough clicks that action and verifies the original tab becomes active. Return checks the saved tab and the same host/port identity used by website access management; a missing or navigated tab leaves an error on the signing page. Typed-data field rendering preserves scalar text and nested JSON, with the full original definitions still expandable.

The screenshot harness also checks native keyboard activation of copy buttons and nested typed-data disclosures, error focus and camera retry traversal, accessible names and live-region semantics in Chrome’s accessibility tree, and actual tab zoom at 200% without horizontal overflow. Clipboard success/failure uses a fixture to assert the exact copied value; native clipboard integration and spoken output from a screen reader are not established by those checks. Completed-message headings describe already-signed data. Full addresses/hashes remain visible beside the copy buttons.
